// This file is auto-generated by @hey-api/openapi-ts

export const AIAPICredentialAdminPublicSchema = {
    properties: {
        id: {
            type: 'string',
            format: 'uuid',
            title: 'Id'
        },
        user_id: {
            type: 'string',
            format: 'uuid',
            title: 'User Id'
        },
        user_email: {
            anyOf: [
                {
                    type: 'string'
                },
                {
                    type: 'null'
                }
            ],
            title: 'User Email'
        },
        user_full_name: {
            anyOf: [
                {
                    type: 'string'
                },
                {
                    type: 'null'
                }
            ],
            title: 'User Full Name'
        },
        request_id: {
            type: 'string',
            format: 'uuid',
            title: 'Request Id'
        },
        base_url: {
            type: 'string',
            title: 'Base Url'
        },
        api_key_prefix: {
            type: 'string',
            title: 'Api Key Prefix'
        },
        api_key_name: {
            type: 'string',
            title: 'Api Key Name'
        },
        rate_limit: {
            anyOf: [
                {
                    type: 'integer'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Rate Limit'
        },
        status: {
            type: 'string',
            enum: [
                'active',
                'inactive'
            ],
            title: 'Status'
        },
        inactive_reason: {
            anyOf: [
                {
                    type: 'string',
                    enum: [
                        'revoked',
                        'expired'
                    ]
                },
                {
                    type: 'null'
                }
            ],
            title: 'Inactive Reason'
        },
        expires_at: {
            anyOf: [
                {
                    type: 'string',
                    format: 'date-time'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Expires At'
        },
        revoked_at: {
            anyOf: [
                {
                    type: 'string',
                    format: 'date-time'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Revoked At'
        },
        created_at: {
            type: 'string',
            format: 'date-time',
            title: 'Created At'
        }
    },
    type: 'object',
    required: [
        'id',
        'user_id',
        'request_id',
        'base_url',
        'api_key_prefix',
        'api_key_name',
        'status',
        'created_at'
    ],
    title: 'AIAPICredentialAdminPublic'
} as const;

export const AIAPICredentialPublicSchema = {
    properties: {
        id: {
            type: 'string',
            format: 'uuid',
            title: 'Id'
        },
        request_id: {
            type: 'string',
            format: 'uuid',
            title: 'Request Id'
        },
        base_url: {
            type: 'string',
            title: 'Base Url'
        },
        api_key: {
            type: 'string',
            title: 'Api Key'
        },
        api_key_prefix: {
            type: 'string',
            title: 'Api Key Prefix'
        },
        api_key_name: {
            type: 'string',
            title: 'Api Key Name'
        },
        rate_limit: {
            anyOf: [
                {
                    type: 'integer'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Rate Limit'
        },
        expires_at: {
            anyOf: [
                {
                    type: 'string',
                    format: 'date-time'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Expires At'
        },
        revoked_at: {
            anyOf: [
                {
                    type: 'string',
                    format: 'date-time'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Revoked At'
        },
        created_at: {
            type: 'string',
            format: 'date-time',
            title: 'Created At'
        }
    },
    type: 'object',
    required: [
        'id',
        'request_id',
        'base_url',
        'api_key',
        'api_key_prefix',
        'api_key_name',
        'created_at'
    ],
    title: 'AIAPICredentialPublic'
} as const;

export const AIAPICredentialUpdateSchema = {
    properties: {
        api_key_name: {
            type: 'string',
            maxLength: 20,
            minLength: 1,
            title: 'Api Key Name'
        }
    },
    type: 'object',
    required: [
        'api_key_name'
    ],
    title: 'AIAPICredentialUpdate'
} as const;

export const AIAPICredentialsAdminPublicSchema = {
    properties: {
        data: {
            items: {
                $ref: '#/components/schemas/AIAPICredentialAdminPublic'
            },
            type: 'array',
            title: 'Data'
        },
        count: {
            type: 'integer',
            title: 'Count'
        }
    },
    type: 'object',
    required: [
        'data',
        'count'
    ],
    title: 'AIAPICredentialsAdminPublic'
} as const;

export const AIAPICredentialsPublicSchema = {
    properties: {
        data: {
            items: {
                $ref: '#/components/schemas/AIAPICredentialPublic'
            },
            type: 'array',
            title: 'Data'
        },
        count: {
            type: 'integer',
            title: 'Count'
        }
    },
    type: 'object',
    required: [
        'data',
        'count'
    ],
    title: 'AIAPICredentialsPublic'
} as const;

export const AIAPIRequestCreateSchema = {
    properties: {
        purpose: {
            type: 'string',
            maxLength: 2000,
            minLength: 10,
            title: 'Purpose'
        },
        api_key_name: {
            type: 'string',
            maxLength: 20,
            minLength: 1,
            title: 'Api Key Name',
            default: 'test'
        },
        duration: {
            type: 'string',
            maxLength: 20,
            title: 'Duration',
            default: 'never'
        }
    },
    type: 'object',
    required: [
        'purpose'
    ],
    title: 'AIAPIRequestCreate'
} as const;

export const AIAPIRequestPublicSchema = {
    properties: {
        id: {
            type: 'string',
            format: 'uuid',
            title: 'Id'
        },
        user_id: {
            type: 'string',
            format: 'uuid',
            title: 'User Id'
        },
        user_email: {
            anyOf: [
                {
                    type: 'string'
                },
                {
                    type: 'null'
                }
            ],
            title: 'User Email'
        },
        user_full_name: {
            anyOf: [
                {
                    type: 'string'
                },
                {
                    type: 'null'
                }
            ],
            title: 'User Full Name'
        },
        purpose: {
            type: 'string',
            title: 'Purpose'
        },
        api_key_name: {
            type: 'string',
            title: 'Api Key Name'
        },
        duration: {
            type: 'string',
            title: 'Duration'
        },
        rate_limit: {
            anyOf: [
                {
                    type: 'integer'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Rate Limit'
        },
        status: {
            $ref: '#/components/schemas/AIAPIRequestStatus'
        },
        reviewer_id: {
            anyOf: [
                {
                    type: 'string',
                    format: 'uuid'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Reviewer Id'
        },
        reviewer_email: {
            anyOf: [
                {
                    type: 'string'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Reviewer Email'
        },
        review_comment: {
            anyOf: [
                {
                    type: 'string'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Review Comment'
        },
        reviewed_at: {
            anyOf: [
                {
                    type: 'string',
                    format: 'date-time'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Reviewed At'
        },
        created_at: {
            type: 'string',
            format: 'date-time',
            title: 'Created At'
        }
    },
    type: 'object',
    required: [
        'id',
        'user_id',
        'purpose',
        'api_key_name',
        'duration',
        'status',
        'created_at'
    ],
    title: 'AIAPIRequestPublic'
} as const;

export const AIAPIRequestReviewSchema = {
    properties: {
        status: {
            $ref: '#/components/schemas/AIAPIRequestStatus'
        },
        review_comment: {
            anyOf: [
                {
                    type: 'string',
                    maxLength: 2000
                },
                {
                    type: 'null'
                }
            ],
            title: 'Review Comment'
        }
    },
    type: 'object',
    required: [
        'status'
    ],
    title: 'AIAPIRequestReview'
} as const;

export const AIAPIRequestStatusSchema = {
    type: 'string',
    enum: [
        'pending',
        'approved',
        'rejected'
    ],
    title: 'AIAPIRequestStatus'
} as const;

export const AIAPIRequestsPublicSchema = {
    properties: {
        data: {
            items: {
                $ref: '#/components/schemas/AIAPIRequestPublic'
            },
            type: 'array',
            title: 'Data'
        },
        count: {
            type: 'integer',
            title: 'Count'
        }
    },
    type: 'object',
    required: [
        'data',
        'count'
    ],
    title: 'AIAPIRequestsPublic'
} as const;

export const AIMonitoringStatsSchema = {
    properties: {
        proxy_total_calls: {
            type: 'integer',
            title: 'Proxy Total Calls'
        },
        proxy_total_input_tokens: {
            type: 'integer',
            title: 'Proxy Total Input Tokens'
        },
        proxy_total_output_tokens: {
            type: 'integer',
            title: 'Proxy Total Output Tokens'
        },
        template_total_calls: {
            type: 'integer',
            title: 'Template Total Calls'
        },
        template_total_input_tokens: {
            type: 'integer',
            title: 'Template Total Input Tokens'
        },
        template_total_output_tokens: {
            type: 'integer',
            title: 'Template Total Output Tokens'
        },
        successful_calls: {
            type: 'integer',
            title: 'Successful Calls'
        },
        failed_calls: {
            type: 'integer',
            title: 'Failed Calls'
        },
        success_rate: {
            type: 'integer',
            title: 'Success Rate'
        },
        avg_latency_ms: {
            type: 'integer',
            title: 'Avg Latency Ms'
        },
        active_users: {
            type: 'integer',
            title: 'Active Users'
        },
        models_used: {
            items: {
                type: 'string'
            },
            type: 'array',
            title: 'Models Used'
        }
    },
    type: 'object',
    required: [
        'proxy_total_calls',
        'proxy_total_input_tokens',
        'proxy_total_output_tokens',
        'template_total_calls',
        'template_total_input_tokens',
        'template_total_output_tokens',
        'successful_calls',
        'failed_calls',
        'success_rate',
        'avg_latency_ms',
        'active_users',
        'models_used'
    ],
    title: 'AIMonitoringStats',
    description: 'Admin 全局 AI 統計'
} as const;

export const AIProxyCallRecordSchema = {
    properties: {
        id: {
            type: 'string',
            format: 'uuid',
            title: 'Id'
        },
        user_id: {
            type: 'string',
            format: 'uuid',
            title: 'User Id'
        },
        user_email: {
            anyOf: [
                {
                    type: 'string'
                },
                {
                    type: 'null'
                }
            ],
            title: 'User Email'
        },
        user_full_name: {
            anyOf: [
                {
                    type: 'string'
                },
                {
                    type: 'null'
                }
            ],
            title: 'User Full Name'
        },
        credential_id: {
            type: 'string',
            format: 'uuid',
            title: 'Credential Id'
        },
        model_name: {
            type: 'string',
            title: 'Model Name'
        },
        request_type: {
            type: 'string',
            title: 'Request Type'
        },
        input_tokens: {
            type: 'integer',
            title: 'Input Tokens'
        },
        output_tokens: {
            type: 'integer',
            title: 'Output Tokens'
        },
        request_duration_ms: {
            anyOf: [
                {
                    type: 'integer'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Request Duration Ms'
        },
        status: {
            type: 'string',
            title: 'Status'
        },
        error_message: {
            anyOf: [
                {
                    type: 'string'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Error Message'
        },
        created_at: {
            type: 'string',
            format: 'date-time',
            title: 'Created At'
        }
    },
    type: 'object',
    required: [
        'id',
        'user_id',
        'credential_id',
        'model_name',
        'request_type',
        'input_tokens',
        'output_tokens',
        'status',
        'created_at'
    ],
    title: 'AIProxyCallRecord',
    description: '單筆 Proxy 呼叫紀錄'
} as const;

export const AIProxyCallsResponseSchema = {
    properties: {
        data: {
            items: {
                $ref: '#/components/schemas/AIProxyCallRecord'
            },
            type: 'array',
            title: 'Data'
        },
        count: {
            type: 'integer',
            title: 'Count'
        }
    },
    type: 'object',
    required: [
        'data',
        'count'
    ],
    title: 'AIProxyCallsResponse',
    description: 'Proxy 呼叫清單回應'
} as const;

export const AITemplateCallRecordSchema = {
    properties: {
        id: {
            type: 'string',
            format: 'uuid',
            title: 'Id'
        },
        user_id: {
            type: 'string',
            format: 'uuid',
            title: 'User Id'
        },
        user_email: {
            anyOf: [
                {
                    type: 'string'
                },
                {
                    type: 'null'
                }
            ],
            title: 'User Email'
        },
        user_full_name: {
            anyOf: [
                {
                    type: 'string'
                },
                {
                    type: 'null'
                }
            ],
            title: 'User Full Name'
        },
        call_type: {
            type: 'string',
            title: 'Call Type'
        },
        model_name: {
            type: 'string',
            title: 'Model Name'
        },
        preset: {
            anyOf: [
                {
                    type: 'string'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Preset'
        },
        input_tokens: {
            type: 'integer',
            title: 'Input Tokens'
        },
        output_tokens: {
            type: 'integer',
            title: 'Output Tokens'
        },
        request_duration_ms: {
            anyOf: [
                {
                    type: 'integer'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Request Duration Ms'
        },
        status: {
            type: 'string',
            title: 'Status'
        },
        error_message: {
            anyOf: [
                {
                    type: 'string'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Error Message'
        },
        created_at: {
            type: 'string',
            format: 'date-time',
            title: 'Created At'
        }
    },
    type: 'object',
    required: [
        'id',
        'user_id',
        'call_type',
        'model_name',
        'input_tokens',
        'output_tokens',
        'status',
        'created_at'
    ],
    title: 'AITemplateCallRecord',
    description: '單筆 Template 呼叫紀錄'
} as const;

export const AITemplateCallsResponseSchema = {
    properties: {
        data: {
            items: {
                $ref: '#/components/schemas/AITemplateCallRecord'
            },
            type: 'array',
            title: 'Data'
        },
        count: {
            type: 'integer',
            title: 'Count'
        }
    },
    type: 'object',
    required: [
        'data',
        'count'
    ],
    title: 'AITemplateCallsResponse',
    description: 'Template 呼叫清單回應'
} as const;

export const AIUserUsageSummarySchema = {
    properties: {
        user_id: {
            type: 'string',
            format: 'uuid',
            title: 'User Id'
        },
        user_email: {
            anyOf: [
                {
                    type: 'string'
                },
                {
                    type: 'null'
                }
            ],
            title: 'User Email'
        },
        user_full_name: {
            anyOf: [
                {
                    type: 'string'
                },
                {
                    type: 'null'
                }
            ],
            title: 'User Full Name'
        },
        proxy_calls: {
            type: 'integer',
            title: 'Proxy Calls'
        },
        proxy_input_tokens: {
            type: 'integer',
            title: 'Proxy Input Tokens'
        },
        proxy_output_tokens: {
            type: 'integer',
            title: 'Proxy Output Tokens'
        },
        template_calls: {
            type: 'integer',
            title: 'Template Calls'
        },
        template_input_tokens: {
            type: 'integer',
            title: 'Template Input Tokens'
        },
        template_output_tokens: {
            type: 'integer',
            title: 'Template Output Tokens'
        }
    },
    type: 'object',
    required: [
        'user_id',
        'proxy_calls',
        'proxy_input_tokens',
        'proxy_output_tokens',
        'template_calls',
        'template_input_tokens',
        'template_output_tokens'
    ],
    title: 'AIUserUsageSummary',
    description: '單一使用者的 AI 用量彙總'
} as const;

export const AIUsersUsageResponseSchema = {
    properties: {
        data: {
            items: {
                $ref: '#/components/schemas/AIUserUsageSummary'
            },
            type: 'array',
            title: 'Data'
        },
        count: {
            type: 'integer',
            title: 'Count'
        }
    },
    type: 'object',
    required: [
        'data',
        'count'
    ],
    title: 'AIUsersUsageResponse',
    description: '使用者用量彙總回應'
} as const;

export const AiMetricsSchema = {
    properties: {
        prompt_tokens: {
            type: 'integer',
            minimum: 0,
            title: 'Prompt Tokens',
            default: 0
        },
        completion_tokens: {
            type: 'integer',
            minimum: 0,
            title: 'Completion Tokens',
            default: 0
        },
        total_tokens: {
            type: 'integer',
            minimum: 0,
            title: 'Total Tokens',
            default: 0
        },
        elapsed_seconds: {
            type: 'number',
            minimum: 0,
            title: 'Elapsed Seconds',
            default: 0
        },
        tokens_per_second: {
            type: 'number',
            minimum: 0,
            title: 'Tokens Per Second',
            default: 0
        }
    },
    type: 'object',
    title: 'AiMetrics'
} as const;

export const AuditActionSchema = {
    type: 'string',
    enum: [
        'spec_change_request',
        'spec_change_apply',
        'snapshot_create',
        'snapshot_delete',
        'snapshot_rollback',
        'config_update',
        'vm_create',
        'lxc_create',
        'resource_start',
        'resource_stop',
        'resource_reboot',
        'resource_shutdown',
        'resource_reset',
        'resource_delete',
        'resource_extend_session',
        'vm_request_submit',
        'vm_request_submit_auto_approved',
        'vm_request_review',
        'ai_api_request_submit',
        'ai_api_request_review',
        'user_create',
        'user_update',
        'user_delete',
        'group_create',
        'group_delete',
        'group_member_add',
        'group_member_remove',
        'batch_provision_vm',
        'batch_provision_lxc',
        'script_deploy',
        'login_success',
        'login_failed',
        'login_google_success',
        'login_google_failed',
        'password_change',
        'password_recovery_request',
        'password_reset',
        'firewall_layout_update',
        'firewall_connection_create',
        'firewall_connection_delete',
        'firewall_rule_create',
        'firewall_rule_update',
        'firewall_rule_delete',
        'nat_rule_delete',
        'nat_rule_sync',
        'reverse_proxy_rule_delete',
        'reverse_proxy_rule_sync',
        'gateway_config_update',
        'gateway_keypair_generate',
        'gateway_config_write',
        'gateway_service_control',
        'cloudflare_config_update',
        'cloudflare_zone_create',
        'cloudflare_dns_record_create',
        'cloudflare_dns_record_update',
        'cloudflare_dns_record_delete',
        'proxmox_config_update',
        'proxmox_node_update',
        'proxmox_storage_update',
        'proxmox_sync_nodes',
        'proxmox_sync_now',
        'migration_job_retry',
        'migration_job_cancel',
        'spec_direct_update',
        'ai_api_credential_rotate',
        'ai_api_credential_delete',
        'ai_api_credential_update'
    ],
    title: 'AuditAction',
    description: '審計操作類型'
} as const;

export const AuditActionMetaSchema = {
    properties: {
        value: {
            type: 'string',
            title: 'Value'
        },
        category: {
            type: 'string',
            title: 'Category'
        }
    },
    type: 'object',
    required: [
        'value',
        'category'
    ],
    title: 'AuditActionMeta',
    description: '單一 action 元資訊（給前端 select 使用）'
} as const;

export const AuditLogPublicSchema = {
    properties: {
        id: {
            type: 'string',
            format: 'uuid',
            title: 'Id'
        },
        user_id: {
            anyOf: [
                {
                    type: 'string',
                    format: 'uuid'
                },
                {
                    type: 'null'
                }
            ],
            title: 'User Id'
        },
        user_email: {
            anyOf: [
                {
                    type: 'string'
                },
                {
                    type: 'null'
                }
            ],
            title: 'User Email'
        },
        user_full_name: {
            anyOf: [
                {
                    type: 'string'
                },
                {
                    type: 'null'
                }
            ],
            title: 'User Full Name'
        },
        vmid: {
            anyOf: [
                {
                    type: 'integer'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Vmid'
        },
        resource_vmid: {
            anyOf: [
                {
                    type: 'integer'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Resource Vmid'
        },
        action: {
            $ref: '#/components/schemas/AuditAction'
        },
        details: {
            type: 'string',
            title: 'Details'
        },
        ip_address: {
            anyOf: [
                {
                    type: 'string'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Ip Address'
        },
        user_agent: {
            anyOf: [
                {
                    type: 'string'
                },
                {
                    type: 'null'
                }
            ],
            title: 'User Agent'
        },
        created_at: {
            type: 'string',
            format: 'date-time',
            title: 'Created At'
        }
    },
    type: 'object',
    required: [
        'id',
        'user_id',
        'vmid',
        'action',
        'details',
        'ip_address',
        'user_agent',
        'created_at'
    ],
    title: 'AuditLogPublic',
    description: '公開的審計日誌'
} as const;

export const AuditLogStatsSchema = {
    properties: {
        total: {
            type: 'integer',
            title: 'Total'
        },
        danger: {
            type: 'integer',
            title: 'Danger'
        },
        login_failed: {
            type: 'integer',
            title: 'Login Failed'
        },
        active_users: {
            type: 'integer',
            title: 'Active Users'
        }
    },
    type: 'object',
    required: [
        'total',
        'danger',
        'login_failed',
        'active_users'
    ],
    title: 'AuditLogStats',
    description: '審計日誌儀表板統計卡片資料'
} as const;

export const AuditLogsPublicSchema = {
    properties: {
        data: {
            items: {
                $ref: '#/components/schemas/AuditLogPublic'
            },
            type: 'array',
            title: 'Data'
        },
        count: {
            type: 'integer',
            title: 'Count'
        }
    },
    type: 'object',
    required: [
        'data',
        'count'
    ],
    title: 'AuditLogsPublic',
    description: '審計日誌列表'
} as const;

export const AuditUserOptionSchema = {
    properties: {
        id: {
            type: 'string',
            format: 'uuid',
            title: 'Id'
        },
        email: {
            type: 'string',
            title: 'Email'
        },
        full_name: {
            anyOf: [
                {
                    type: 'string'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Full Name'
        }
    },
    type: 'object',
    required: [
        'id',
        'email'
    ],
    title: 'AuditUserOption',
    description: '操作者下拉選單選項'
} as const;

export const BatchActionRequestSchema = {
    properties: {
        vmids: {
            items: {
                type: 'integer'
            },
            type: 'array',
            maxItems: 100,
            minItems: 1,
            title: 'Vmids',
            description: 'VM IDs to operate on'
        },
        action: {
            type: 'string',
            title: 'Action',
            description: 'Action: start, stop, shutdown, reboot, reset, delete'
        }
    },
    type: 'object',
    required: [
        'vmids',
        'action'
    ],
    title: 'BatchActionRequest',
    description: '批次操作請求'
} as const;

export const BatchActionResponseSchema = {
    properties: {
        total: {
            type: 'integer',
            title: 'Total'
        },
        succeeded: {
            type: 'integer',
            title: 'Succeeded'
        },
        failed: {
            type: 'integer',
            title: 'Failed'
        },
        results: {
            items: {
                $ref: '#/components/schemas/BatchActionResultItem'
            },
            type: 'array',
            title: 'Results'
        }
    },
    type: 'object',
    required: [
        'total',
        'succeeded',
        'failed',
        'results'
    ],
    title: 'BatchActionResponse',
    description: '批次操作回應'
} as const;

export const BatchActionResultItemSchema = {
    properties: {
        vmid: {
            type: 'integer',
            title: 'Vmid'
        },
        success: {
            type: 'boolean',
            title: 'Success'
        },
        message: {
            type: 'string',
            title: 'Message'
        }
    },
    type: 'object',
    required: [
        'vmid',
        'success',
        'message'
    ],
    title: 'BatchActionResultItem',
    description: '單一 VM 的批次操作結果'
} as const;

export const BatchProvisionJobPublicSchema = {
    properties: {
        id: {
            type: 'string',
            format: 'uuid',
            title: 'Id'
        },
        group_id: {
            type: 'string',
            format: 'uuid',
            title: 'Group Id'
        },
        group_name: {
            anyOf: [
                {
                    type: 'string'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Group Name'
        },
        resource_type: {
            type: 'string',
            title: 'Resource Type'
        },
        hostname_prefix: {
            type: 'string',
            title: 'Hostname Prefix'
        },
        status: {
            type: 'string',
            title: 'Status'
        },
        total: {
            type: 'integer',
            title: 'Total'
        },
        done: {
            type: 'integer',
            title: 'Done'
        },
        failed_count: {
            type: 'integer',
            title: 'Failed Count'
        },
        created_at: {
            type: 'string',
            format: 'date-time',
            title: 'Created At'
        },
        finished_at: {
            anyOf: [
                {
                    type: 'string',
                    format: 'date-time'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Finished At'
        },
        initiated_by: {
            anyOf: [
                {
                    type: 'string',
                    format: 'uuid'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Initiated By'
        },
        initiated_by_email: {
            anyOf: [
                {
                    type: 'string'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Initiated By Email'
        },
        initiated_by_name: {
            anyOf: [
                {
                    type: 'string'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Initiated By Name'
        },
        reviewer_id: {
            anyOf: [
                {
                    type: 'string',
                    format: 'uuid'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Reviewer Id'
        },
        reviewer_email: {
            anyOf: [
                {
                    type: 'string'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Reviewer Email'
        },
        reviewed_at: {
            anyOf: [
                {
                    type: 'string',
                    format: 'date-time'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Reviewed At'
        },
        review_comment: {
            anyOf: [
                {
                    type: 'string'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Review Comment'
        },
        recurrence_rule: {
            anyOf: [
                {
                    type: 'string'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Recurrence Rule'
        },
        recurrence_duration_minutes: {
            anyOf: [
                {
                    type: 'integer'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Recurrence Duration Minutes'
        },
        schedule_timezone: {
            anyOf: [
                {
                    type: 'string'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Schedule Timezone'
        },
        spec: {
            $ref: '#/components/schemas/BatchProvisionJobSpec'
        },
        tasks: {
            items: {
                $ref: '#/components/schemas/BatchProvisionTaskPublic'
            },
            type: 'array',
            title: 'Tasks'
        }
    },
    type: 'object',
    required: [
        'id',
        'group_id',
        'resource_type',
        'hostname_prefix',
        'status',
        'total',
        'done',
        'failed_count',
        'created_at',
        'finished_at',
        'spec',
        'tasks'
    ],
    title: 'BatchProvisionJobPublic'
} as const;

export const BatchProvisionJobSpecSchema = {
    properties: {
        cores: {
            anyOf: [
                {
                    type: 'integer'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Cores'
        },
        memory: {
            anyOf: [
                {
                    type: 'integer'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Memory'
        },
        disk_size: {
            anyOf: [
                {
                    type: 'integer'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Disk Size'
        },
        rootfs_size: {
            anyOf: [
                {
                    type: 'integer'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Rootfs Size'
        },
        ostemplate: {
            anyOf: [
                {
                    type: 'string'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Ostemplate'
        },
        template_id: {
            anyOf: [
                {
                    type: 'integer'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Template Id'
        },
        username: {
            anyOf: [
                {
                    type: 'string'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Username'
        },
        environment_type: {
            anyOf: [
                {
                    type: 'string'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Environment Type'
        },
        os_info: {
            anyOf: [
                {
                    type: 'string'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Os Info'
        },
        expiry_date: {
            anyOf: [
                {
                    type: 'string'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Expiry Date'
        }
    },
    type: 'object',
    title: 'BatchProvisionJobSpec',
    description: 'Spec parameters that apply to every member\'s resource. Reflects the\nJSON stored in ``BatchProvisionJob.template_params``.'
} as const;

export const BatchProvisionRequestSchema = {
    properties: {
        resource_type: {
            type: 'string',
            pattern: '^(lxc|qemu)$',
            title: 'Resource Type'
        },
        hostname_prefix: {
            type: 'string',
            maxLength: 50,
            minLength: 1,
            pattern: '^[a-zA-Z0-9][a-zA-Z0-9-]*$',
            title: 'Hostname Prefix',
            description: 'Hostname prefix: ASCII letters, digits, hyphens; cannot start with hyphen'
        },
        password: {
            type: 'string',
            minLength: 6,
            title: 'Password'
        },
        cores: {
            type: 'integer',
            maximum: 32,
            minimum: 1,
            title: 'Cores',
            default: 2
        },
        memory: {
            type: 'integer',
            maximum: 65536,
            minimum: 128,
            title: 'Memory',
            default: 2048
        },
        environment_type: {
            type: 'string',
            title: 'Environment Type',
            default: '批次部署'
        },
        os_info: {
            anyOf: [
                {
                    type: 'string'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Os Info'
        },
        expiry_date: {
            anyOf: [
                {
                    type: 'string',
                    format: 'date'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Expiry Date'
        },
        ostemplate: {
            anyOf: [
                {
                    type: 'string'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Ostemplate'
        },
        rootfs_size: {
            anyOf: [
                {
                    type: 'integer',
                    maximum: 1000,
                    minimum: 1
                },
                {
                    type: 'null'
                }
            ],
            title: 'Rootfs Size',
            default: 8
        },
        template_id: {
            anyOf: [
                {
                    type: 'integer'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Template Id'
        },
        username: {
            anyOf: [
                {
                    type: 'string'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Username'
        },
        disk_size: {
            anyOf: [
                {
                    type: 'integer',
                    maximum: 1000,
                    minimum: 10
                },
                {
                    type: 'null'
                }
            ],
            title: 'Disk Size',
            default: 20
        },
        recurrence_rule: {
            anyOf: [
                {
                    type: 'string',
                    maxLength: 500
                },
                {
                    type: 'null'
                }
            ],
            title: 'Recurrence Rule'
        },
        recurrence_duration_minutes: {
            anyOf: [
                {
                    type: 'integer',
                    maximum: 10080,
                    minimum: 15
                },
                {
                    type: 'null'
                }
            ],
            title: 'Recurrence Duration Minutes'
        },
        schedule_timezone: {
            anyOf: [
                {
                    type: 'string',
                    maxLength: 64
                },
                {
                    type: 'null'
                }
            ],
            title: 'Schedule Timezone'
        }
    },
    type: 'object',
    required: [
        'resource_type',
        'hostname_prefix',
        'password'
    ],
    title: 'BatchProvisionRequest',
    description: 'Payload used to create one batch provisioning job.'
} as const;

export const BatchProvisionReviewRequestSchema = {
    properties: {
        decision: {
            type: 'string',
            pattern: '^(approved|rejected)$',
            title: 'Decision'
        },
        review_comment: {
            anyOf: [
                {
                    type: 'string',
                    maxLength: 500
                },
                {
                    type: 'null'
                }
            ],
            title: 'Review Comment'
        }
    },
    type: 'object',
    required: [
        'decision'
    ],
    title: 'BatchProvisionReviewRequest',
    description: 'Payload for admin approve/reject of a pending batch.'
} as const;

export const BatchProvisionTaskPublicSchema = {
    properties: {
        id: {
            type: 'string',
            format: 'uuid',
            title: 'Id'
        },
        user_id: {
            type: 'string',
            format: 'uuid',
            title: 'User Id'
        },
        user_email: {
            anyOf: [
                {
                    type: 'string'
                },
                {
                    type: 'null'
                }
            ],
            title: 'User Email'
        },
        user_name: {
            anyOf: [
                {
                    type: 'string'
                },
                {
                    type: 'null'
                }
            ],
            title: 'User Name'
        },
        member_index: {
            type: 'integer',
            title: 'Member Index'
        },
        vmid: {
            anyOf: [
                {
                    type: 'integer'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Vmid'
        },
        status: {
            type: 'string',
            title: 'Status'
        },
        error: {
            anyOf: [
                {
                    type: 'string'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Error'
        },
        started_at: {
            anyOf: [
                {
                    type: 'string',
                    format: 'date-time'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Started At'
        },
        finished_at: {
            anyOf: [
                {
                    type: 'string',
                    format: 'date-time'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Finished At'
        }
    },
    type: 'object',
    required: [
        'id',
        'user_id',
        'user_email',
        'user_name',
        'member_index',
        'vmid',
        'status',
        'error',
        'started_at',
        'finished_at'
    ],
    title: 'BatchProvisionTaskPublic'
} as const;

export const Body_groups_import_members_from_csvSchema = {
    properties: {
        file: {
            type: 'string',
            contentMediaType: 'application/octet-stream',
            title: 'File'
        }
    },
    type: 'object',
    required: [
        'file'
    ],
    title: 'Body_groups-import_members_from_csv'
} as const;

export const Body_login_login_access_tokenSchema = {
    properties: {
        grant_type: {
            anyOf: [
                {
                    type: 'string',
                    pattern: '^password$'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Grant Type'
        },
        username: {
            type: 'string',
            title: 'Username'
        },
        password: {
            type: 'string',
            format: 'password',
            title: 'Password'
        },
        scope: {
            type: 'string',
            title: 'Scope',
            default: ''
        },
        client_id: {
            anyOf: [
                {
                    type: 'string'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Client Id'
        },
        client_secret: {
            anyOf: [
                {
                    type: 'string'
                },
                {
                    type: 'null'
                }
            ],
            format: 'password',
            title: 'Client Secret'
        }
    },
    type: 'object',
    required: [
        'username',
        'password'
    ],
    title: 'Body_login-login_access_token'
} as const;

export const Body_proxmox_config_parse_certSchema = {
    properties: {
        pem: {
            type: 'string',
            title: 'Pem'
        }
    },
    type: 'object',
    required: [
        'pem'
    ],
    title: 'Body_proxmox-config-parse_cert'
} as const;

export const Body_rubric_upload_rubricSchema = {
    properties: {
        file: {
            type: 'string',
            contentMediaType: 'application/octet-stream',
            title: 'File'
        },
        template_key: {
            type: 'string',
            title: 'Template Key',
            default: 'linux'
        }
    },
    type: 'object',
    required: [
        'file'
    ],
    title: 'Body_rubric-upload_rubric'
} as const;

export const Body_teacher_judge_upload_group_teacher_judge_fileSchema = {
    properties: {
        file: {
            type: 'string',
            contentMediaType: 'application/octet-stream',
            title: 'File'
        },
        template_key: {
            type: 'string',
            title: 'Template Key',
            default: 'linux'
        },
        conflict_strategy: {
            anyOf: [
                {
                    type: 'string'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Conflict Strategy'
        }
    },
    type: 'object',
    required: [
        'file'
    ],
    title: 'Body_teacher-judge-upload_group_teacher_judge_file'
} as const;

export const CertParseResultSchema = {
    properties: {
        valid: {
            type: 'boolean',
            title: 'Valid'
        },
        fingerprint: {
            anyOf: [
                {
                    type: 'string'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Fingerprint'
        },
        subject: {
            anyOf: [
                {
                    type: 'string'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Subject'
        },
        issuer: {
            anyOf: [
                {
                    type: 'string'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Issuer'
        },
        not_before: {
            anyOf: [
                {
                    type: 'string'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Not Before'
        },
        not_after: {
            anyOf: [
                {
                    type: 'string'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Not After'
        },
        error: {
            anyOf: [
                {
                    type: 'string'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Error'
        }
    },
    type: 'object',
    required: [
        'valid'
    ],
    title: 'CertParseResult',
    description: '解析憑證 PEM 的結果'
} as const;

export const ChatCompletionChoiceSchema = {
    properties: {
        index: {
            type: 'integer',
            title: 'Index'
        },
        message: {
            $ref: '#/components/schemas/ChatMessage-Output'
        },
        finish_reason: {
            anyOf: [
                {
                    type: 'string'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Finish Reason'
        }
    },
    type: 'object',
    required: [
        'index',
        'message'
    ],
    title: 'ChatCompletionChoice',
    description: '聊天补全选择'
} as const;

export const ChatCompletionRequestSchema = {
    properties: {
        model: {
            type: 'string',
            title: 'Model',
            description: '模型名称，如 gpt-oss-20B'
        },
        messages: {
            items: {
                $ref: '#/components/schemas/app__schemas__ai_proxy__ChatMessage'
            },
            type: 'array',
            title: 'Messages',
            description: '对话历史'
        },
        max_tokens: {
            anyOf: [
                {
                    type: 'integer'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Max Tokens',
            description: '最大生成 tokens',
            default: 2048
        },
        temperature: {
            anyOf: [
                {
                    type: 'number',
                    maximum: 2,
                    minimum: 0
                },
                {
                    type: 'null'
                }
            ],
            title: 'Temperature',
            description: '采样温度',
            default: 0.8
        },
        top_p: {
            anyOf: [
                {
                    type: 'number',
                    maximum: 1,
                    minimum: 0
                },
                {
                    type: 'null'
                }
            ],
            title: 'Top P',
            description: '核采样',
            default: 0.95
        },
        stream: {
            anyOf: [
                {
                    type: 'boolean'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Stream',
            description: '是否流式响应',
            default: false
        },
        n: {
            anyOf: [
                {
                    type: 'integer'
                },
                {
                    type: 'null'
                }
            ],
            title: 'N',
            description: '生成多少个补全',
            default: 1
        },
        stop: {
            anyOf: [
                {
                    type: 'string'
                },
                {
                    items: {
                        type: 'string'
                    },
                    type: 'array'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Stop',
            description: '停止序列'
        },
        presence_penalty: {
            anyOf: [
                {
                    type: 'number'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Presence Penalty',
            description: '存在惩罚',
            default: 0
        },
        frequency_penalty: {
            anyOf: [
                {
                    type: 'number'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Frequency Penalty',
            description: '频率惩罚',
            default: 0
        }
    },
    type: 'object',
    required: [
        'model',
        'messages'
    ],
    title: 'ChatCompletionRequest',
    description: '聊天补全请求（OpenAI 兼容）'
} as const;

export const ChatCompletionResponseSchema = {
    properties: {
        id: {
            type: 'string',
            title: 'Id'
        },
        object: {
            type: 'string',
            title: 'Object',
            default: 'chat.completion'
        },
        created: {
            type: 'integer',
            title: 'Created'
        },
        model: {
            type: 'string',
            title: 'Model'
        },
        choices: {
            items: {
                $ref: '#/components/schemas/ChatCompletionChoice'
            },
            type: 'array',
            title: 'Choices'
        },
        usage: {
            $ref: '#/components/schemas/UsageInfo'
        },
        duration_ms: {
            anyOf: [
                {
                    type: 'integer'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Duration Ms'
        }
    },
    type: 'object',
    required: [
        'id',
        'created',
        'model',
        'choices',
        'usage'
    ],
    title: 'ChatCompletionResponse',
    description: '聊天补全响应（OpenAI 兼容）'
} as const;

export const ChatMessage_OutputSchema = {
    properties: {
        role: {
            type: 'string',
            title: 'Role',
            description: '消息角色: system, user, assistant'
        },
        content: {
            type: 'string',
            title: 'Content',
            description: '消息内容'
        }
    },
    type: 'object',
    required: [
        'role',
        'content'
    ],
    title: 'ChatMessage',
    description: '聊天消息'
} as const;

export const CloudflareConfigPublicSchema = {
    properties: {
        account_id: {
            anyOf: [
                {
                    type: 'string'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Account Id'
        },
        is_configured: {
            type: 'boolean',
            title: 'Is Configured'
        },
        has_api_token: {
            type: 'boolean',
            title: 'Has Api Token'
        },
        has_default_dns_target: {
            type: 'boolean',
            title: 'Has Default Dns Target'
        },
        default_dns_target_type: {
            anyOf: [
                {
                    type: 'string'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Default Dns Target Type'
        },
        default_dns_target_value: {
            anyOf: [
                {
                    type: 'string'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Default Dns Target Value'
        },
        updated_at: {
            anyOf: [
                {
                    type: 'string',
                    format: 'date-time'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Updated At'
        },
        last_verified_at: {
            anyOf: [
                {
                    type: 'string',
                    format: 'date-time'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Last Verified At'
        }
    },
    type: 'object',
    required: [
        'is_configured',
        'has_api_token',
        'has_default_dns_target'
    ],
    title: 'CloudflareConfigPublic'
} as const;

export const CloudflareConfigUpdateSchema = {
    properties: {
        account_id: {
            anyOf: [
                {
                    type: 'string',
                    maxLength: 128
                },
                {
                    type: 'null'
                }
            ],
            title: 'Account Id'
        },
        api_token: {
            anyOf: [
                {
                    type: 'string',
                    minLength: 20
                },
                {
                    type: 'null'
                }
            ],
            title: 'Api Token'
        },
        default_dns_target_type: {
            anyOf: [
                {
                    type: 'string',
                    maxLength: 16
                },
                {
                    type: 'null'
                }
            ],
            title: 'Default Dns Target Type'
        },
        default_dns_target_value: {
            anyOf: [
                {
                    type: 'string',
                    maxLength: 255
                },
                {
                    type: 'null'
                }
            ],
            title: 'Default Dns Target Value'
        }
    },
    type: 'object',
    title: 'CloudflareConfigUpdate'
} as const;

export const CloudflareConnectionTestResultSchema = {
    properties: {
        success: {
            type: 'boolean',
            title: 'Success'
        },
        message: {
            type: 'string',
            title: 'Message'
        },
        token_status: {
            anyOf: [
                {
                    type: 'string'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Token Status'
        }
    },
    type: 'object',
    required: [
        'success',
        'message'
    ],
    title: 'CloudflareConnectionTestResult'
} as const;

export const CloudflareDNSRecordCreateSchema = {
    properties: {
        type: {
            type: 'string',
            maxLength: 32,
            minLength: 1,
            title: 'Type'
        },
        name: {
            type: 'string',
            maxLength: 255,
            minLength: 1,
            title: 'Name'
        },
        content: {
            type: 'string',
            maxLength: 4096,
            minLength: 1,
            title: 'Content'
        },
        ttl: {
            type: 'integer',
            maximum: 86400,
            minimum: 1,
            title: 'Ttl',
            default: 1
        },
        proxied: {
            anyOf: [
                {
                    type: 'boolean'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Proxied'
        },
        comment: {
            anyOf: [
                {
                    type: 'string',
                    maxLength: 500
                },
                {
                    type: 'null'
                }
            ],
            title: 'Comment'
        },
        priority: {
            anyOf: [
                {
                    type: 'integer',
                    maximum: 65535,
                    minimum: 0
                },
                {
                    type: 'null'
                }
            ],
            title: 'Priority'
        }
    },
    type: 'object',
    required: [
        'type',
        'name',
        'content'
    ],
    title: 'CloudflareDNSRecordCreate'
} as const;

export const CloudflareDNSRecordPublicSchema = {
    properties: {
        id: {
            type: 'string',
            title: 'Id'
        },
        zone_id: {
            type: 'string',
            title: 'Zone Id'
        },
        type: {
            type: 'string',
            title: 'Type'
        },
        name: {
            type: 'string',
            title: 'Name'
        },
        content: {
            type: 'string',
            title: 'Content'
        },
        ttl: {
            type: 'integer',
            title: 'Ttl'
        },
        proxied: {
            anyOf: [
                {
                    type: 'boolean'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Proxied'
        },
        proxiable: {
            anyOf: [
                {
                    type: 'boolean'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Proxiable'
        },
        comment: {
            anyOf: [
                {
                    type: 'string'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Comment'
        },
        priority: {
            anyOf: [
                {
                    type: 'integer'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Priority'
        },
        tags: {
            items: {
                type: 'string'
            },
            type: 'array',
            title: 'Tags'
        },
        created_on: {
            anyOf: [
                {
                    type: 'string',
                    format: 'date-time'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Created On'
        },
        modified_on: {
            anyOf: [
                {
                    type: 'string',
                    format: 'date-time'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Modified On'
        }
    },
    type: 'object',
    required: [
        'id',
        'zone_id',
        'type',
        'name',
        'content',
        'ttl'
    ],
    title: 'CloudflareDNSRecordPublic'
} as const;

export const CloudflareDNSRecordUpdateSchema = {
    properties: {
        type: {
            type: 'string',
            maxLength: 32,
            minLength: 1,
            title: 'Type'
        },
        name: {
            type: 'string',
            maxLength: 255,
            minLength: 1,
            title: 'Name'
        },
        content: {
            type: 'string',
            maxLength: 4096,
            minLength: 1,
            title: 'Content'
        },
        ttl: {
            type: 'integer',
            maximum: 86400,
            minimum: 1,
            title: 'Ttl',
            default: 1
        },
        proxied: {
            anyOf: [
                {
                    type: 'boolean'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Proxied'
        },
        comment: {
            anyOf: [
                {
                    type: 'string',
                    maxLength: 500
                },
                {
                    type: 'null'
                }
            ],
            title: 'Comment'
        },
        priority: {
            anyOf: [
                {
                    type: 'integer',
                    maximum: 65535,
                    minimum: 0
                },
                {
                    type: 'null'
                }
            ],
            title: 'Priority'
        }
    },
    type: 'object',
    required: [
        'type',
        'name',
        'content'
    ],
    title: 'CloudflareDNSRecordUpdate'
} as const;

export const CloudflareDNSRecordsPublicSchema = {
    properties: {
        items: {
            items: {
                $ref: '#/components/schemas/CloudflareDNSRecordPublic'
            },
            type: 'array',
            title: 'Items'
        },
        page_info: {
            $ref: '#/components/schemas/CloudflarePageInfoPublic'
        }
    },
    type: 'object',
    required: [
        'items',
        'page_info'
    ],
    title: 'CloudflareDNSRecordsPublic'
} as const;

export const CloudflarePageInfoPublicSchema = {
    properties: {
        page: {
            type: 'integer',
            title: 'Page',
            default: 1
        },
        per_page: {
            type: 'integer',
            title: 'Per Page',
            default: 50
        },
        count: {
            type: 'integer',
            title: 'Count',
            default: 0
        },
        total_count: {
            type: 'integer',
            title: 'Total Count',
            default: 0
        },
        total_pages: {
            type: 'integer',
            title: 'Total Pages',
            default: 1
        }
    },
    type: 'object',
    title: 'CloudflarePageInfoPublic'
} as const;

export const CloudflareZoneCreateSchema = {
    properties: {
        name: {
            type: 'string',
            maxLength: 255,
            minLength: 1,
            title: 'Name'
        },
        account_id: {
            anyOf: [
                {
                    type: 'string',
                    maxLength: 128
                },
                {
                    type: 'null'
                }
            ],
            title: 'Account Id'
        },
        jump_start: {
            type: 'boolean',
            title: 'Jump Start',
            default: false
        }
    },
    type: 'object',
    required: [
        'name'
    ],
    title: 'CloudflareZoneCreate'
} as const;

export const CloudflareZonePublicSchema = {
    properties: {
        id: {
            type: 'string',
            title: 'Id'
        },
        name: {
            type: 'string',
            title: 'Name'
        },
        status: {
            type: 'string',
            title: 'Status'
        },
        paused: {
            type: 'boolean',
            title: 'Paused',
            default: false
        },
        type: {
            anyOf: [
                {
                    type: 'string'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Type'
        },
        development_mode: {
            anyOf: [
                {
                    type: 'integer'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Development Mode'
        },
        name_servers: {
            items: {
                type: 'string'
            },
            type: 'array',
            title: 'Name Servers'
        },
        original_name_servers: {
            items: {
                type: 'string'
            },
            type: 'array',
            title: 'Original Name Servers'
        },
        created_on: {
            anyOf: [
                {
                    type: 'string',
                    format: 'date-time'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Created On'
        },
        modified_on: {
            anyOf: [
                {
                    type: 'string',
                    format: 'date-time'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Modified On'
        },
        activated_on: {
            anyOf: [
                {
                    type: 'string',
                    format: 'date-time'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Activated On'
        }
    },
    type: 'object',
    required: [
        'id',
        'name',
        'status'
    ],
    title: 'CloudflareZonePublic'
} as const;

export const CloudflareZonesPublicSchema = {
    properties: {
        items: {
            items: {
                $ref: '#/components/schemas/CloudflareZonePublic'
            },
            type: 'array',
            title: 'Items'
        },
        page_info: {
            $ref: '#/components/schemas/CloudflarePageInfoPublic'
        }
    },
    type: 'object',
    required: [
        'items',
        'page_info'
    ],
    title: 'CloudflareZonesPublic'
} as const;

export const ClusterInfoSchema = {
    properties: {
        cluster_name: {
            anyOf: [
                {
                    type: 'string'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Cluster Name'
        },
        is_cluster: {
            type: 'boolean',
            title: 'Is Cluster'
        },
        node_count: {
            type: 'integer',
            title: 'Node Count'
        },
        quorate: {
            type: 'boolean',
            title: 'Quorate'
        },
        cluster_version: {
            anyOf: [
                {
                    type: 'integer'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Cluster Version'
        }
    },
    type: 'object',
    required: [
        'is_cluster',
        'node_count',
        'quorate'
    ],
    title: 'ClusterInfo'
} as const;

export const ClusterPreviewResultSchema = {
    properties: {
        success: {
            type: 'boolean',
            title: 'Success'
        },
        is_cluster: {
            type: 'boolean',
            title: 'Is Cluster'
        },
        nodes: {
            items: {
                $ref: '#/components/schemas/ProxmoxNodePublic'
            },
            type: 'array',
            title: 'Nodes'
        },
        error: {
            anyOf: [
                {
                    type: 'string'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Error'
        }
    },
    type: 'object',
    required: [
        'success',
        'is_cluster',
        'nodes'
    ],
    title: 'ClusterPreviewResult',
    description: '偵測叢集節點的預覽結果（不儲存）'
} as const;

export const ClusterStatsPublicSchema = {
    properties: {
        nodes: {
            items: {
                $ref: '#/components/schemas/NodeStatsPublic'
            },
            type: 'array',
            title: 'Nodes'
        },
        total_cpu_cores: {
            type: 'integer',
            title: 'Total Cpu Cores'
        },
        used_cpu_cores: {
            type: 'number',
            title: 'Used Cpu Cores'
        },
        total_mem_gb: {
            type: 'number',
            title: 'Total Mem Gb'
        },
        used_mem_gb: {
            type: 'number',
            title: 'Used Mem Gb'
        },
        total_disk_gb: {
            type: 'number',
            title: 'Total Disk Gb'
        },
        used_disk_gb: {
            type: 'number',
            title: 'Used Disk Gb'
        },
        online_count: {
            type: 'integer',
            title: 'Online Count'
        },
        offline_count: {
            type: 'integer',
            title: 'Offline Count'
        },
        total_vm_count: {
            type: 'integer',
            title: 'Total Vm Count'
        }
    },
    type: 'object',
    required: [
        'nodes',
        'total_cpu_cores',
        'used_cpu_cores',
        'total_mem_gb',
        'used_mem_gb',
        'total_disk_gb',
        'used_disk_gb',
        'online_count',
        'offline_count',
        'total_vm_count'
    ],
    title: 'ClusterStatsPublic',
    description: '整個叢集的資源加總與各節點狀態'
} as const;

export const ConnectionCreateSchema = {
    properties: {
        source_vmid: {
            anyOf: [
                {
                    type: 'integer'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Source Vmid',
            description: '來源 VM ID；None 代表網關（Internet 入站）'
        },
        target_vmid: {
            anyOf: [
                {
                    type: 'integer'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Target Vmid',
            description: '目標 VM ID；None 代表網關（上網）'
        },
        ports: {
            items: {
                $ref: '#/components/schemas/PortSpec'
            },
            type: 'array',
            title: 'Ports',
            description: '允許通過的端口列表'
        },
        direction: {
            type: 'string',
            enum: [
                'one_way',
                'bidirectional'
            ],
            title: 'Direction',
            description: '連線方向：one_way（單向）或 bidirectional（雙向）',
            default: 'one_way'
        }
    },
    type: 'object',
    required: [
        'source_vmid',
        'ports'
    ],
    title: 'ConnectionCreate',
    description: '建立 VM 間連線（或 VM 到網關，或 Internet 入站）'
} as const;

export const ConnectionDeleteSchema = {
    properties: {
        source_vmid: {
            anyOf: [
                {
                    type: 'integer'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Source Vmid',
            description: '來源 VM ID；None 代表網關'
        },
        target_vmid: {
            anyOf: [
                {
                    type: 'integer'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Target Vmid',
            description: '目標 VM ID；None 代表網關'
        },
        ports: {
            anyOf: [
                {
                    items: {
                        $ref: '#/components/schemas/PortSpec'
                    },
                    type: 'array'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Ports',
            description: '要刪除的端口；None 代表刪除全部連線'
        }
    },
    type: 'object',
    required: [
        'source_vmid'
    ],
    title: 'ConnectionDelete',
    description: '刪除 VM 間連線'
} as const;

export const CsvImportResultSchema = {
    properties: {
        created: {
            items: {
                type: 'string'
            },
            type: 'array',
            title: 'Created',
            default: []
        },
        already_existed: {
            items: {
                type: 'string'
            },
            type: 'array',
            title: 'Already Existed',
            default: []
        },
        added_to_group: {
            type: 'integer',
            title: 'Added To Group',
            default: 0
        },
        errors: {
            items: {
                type: 'string'
            },
            type: 'array',
            title: 'Errors',
            default: []
        }
    },
    type: 'object',
    title: 'CsvImportResult',
    description: 'CSV 大量匯入結果'
} as const;

export const CurrentStatsResponseSchema = {
    properties: {
        cpu: {
            anyOf: [
                {
                    type: 'number'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Cpu',
            description: 'CPU usage (0-1)'
        },
        maxcpu: {
            anyOf: [
                {
                    type: 'integer'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Maxcpu',
            description: 'CPU cores'
        },
        mem: {
            anyOf: [
                {
                    type: 'integer'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Mem',
            description: 'Memory usage (bytes)'
        },
        maxmem: {
            anyOf: [
                {
                    type: 'integer'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Maxmem',
            description: 'Max memory (bytes)'
        },
        disk: {
            anyOf: [
                {
                    type: 'integer'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Disk',
            description: 'Disk usage (bytes)'
        },
        maxdisk: {
            anyOf: [
                {
                    type: 'integer'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Maxdisk',
            description: 'Max disk (bytes)'
        },
        netin: {
            anyOf: [
                {
                    type: 'integer'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Netin',
            description: 'Network in (bytes)'
        },
        netout: {
            anyOf: [
                {
                    type: 'integer'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Netout',
            description: 'Network out (bytes)'
        },
        uptime: {
            anyOf: [
                {
                    type: 'integer'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Uptime',
            description: 'Uptime (seconds)'
        },
        status: {
            type: 'string',
            title: 'Status',
            description: 'Status'
        }
    },
    type: 'object',
    required: [
        'status'
    ],
    title: 'CurrentStatsResponse',
    description: '資源即時狀態'
} as const;

export const DeletionRequestCreatedSchema = {
    properties: {
        id: {
            type: 'string',
            format: 'uuid',
            title: 'Id'
        },
        vmid: {
            type: 'integer',
            title: 'Vmid'
        },
        status: {
            $ref: '#/components/schemas/DeletionRequestStatus'
        },
        message: {
            type: 'string',
            title: 'Message',
            default: 'Deletion request queued'
        }
    },
    type: 'object',
    required: [
        'id',
        'vmid',
        'status'
    ],
    title: 'DeletionRequestCreated',
    description: 'Response when accepting a delete request (HTTP 202).'
} as const;

export const DeletionRequestPublicSchema = {
    properties: {
        id: {
            type: 'string',
            format: 'uuid',
            title: 'Id'
        },
        user_id: {
            type: 'string',
            format: 'uuid',
            title: 'User Id'
        },
        vmid: {
            type: 'integer',
            title: 'Vmid'
        },
        name: {
            anyOf: [
                {
                    type: 'string'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Name'
        },
        node: {
            anyOf: [
                {
                    type: 'string'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Node'
        },
        resource_type: {
            anyOf: [
                {
                    type: 'string'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Resource Type'
        },
        purge: {
            type: 'boolean',
            title: 'Purge'
        },
        force: {
            type: 'boolean',
            title: 'Force'
        },
        status: {
            $ref: '#/components/schemas/DeletionRequestStatus'
        },
        error_message: {
            anyOf: [
                {
                    type: 'string'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Error Message'
        },
        created_at: {
            type: 'string',
            format: 'date-time',
            title: 'Created At'
        },
        started_at: {
            anyOf: [
                {
                    type: 'string',
                    format: 'date-time'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Started At'
        },
        completed_at: {
            anyOf: [
                {
                    type: 'string',
                    format: 'date-time'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Completed At'
        },
        user_email: {
            anyOf: [
                {
                    type: 'string'
                },
                {
                    type: 'null'
                }
            ],
            title: 'User Email'
        },
        user_full_name: {
            anyOf: [
                {
                    type: 'string'
                },
                {
                    type: 'null'
                }
            ],
            title: 'User Full Name'
        }
    },
    type: 'object',
    required: [
        'id',
        'user_id',
        'vmid',
        'purge',
        'force',
        'status',
        'created_at'
    ],
    title: 'DeletionRequestPublic'
} as const;

export const DeletionRequestStatusSchema = {
    type: 'string',
    enum: [
        'pending',
        'running',
        'completed',
        'failed',
        'cancelled'
    ],
    title: 'DeletionRequestStatus'
} as const;

export const DeletionRequestsPublicSchema = {
    properties: {
        data: {
            items: {
                $ref: '#/components/schemas/DeletionRequestPublic'
            },
            type: 'array',
            title: 'Data'
        },
        count: {
            type: 'integer',
            title: 'Count'
        }
    },
    type: 'object',
    required: [
        'data',
        'count'
    ],
    title: 'DeletionRequestsPublic'
} as const;

export const DependencyStatusSchema = {
    properties: {
        status: {
            type: 'string',
            enum: [
                'ok',
                'error',
                'skipped'
            ],
            title: 'Status'
        },
        detail: {
            anyOf: [
                {
                    type: 'string'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Detail'
        },
        latency_ms: {
            anyOf: [
                {
                    type: 'number'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Latency Ms'
        }
    },
    type: 'object',
    required: [
        'status'
    ],
    title: 'DependencyStatus'
} as const;

export const DeviceApproveRequestSchema = {
    properties: {
        device_code: {
            type: 'string',
            title: 'Device Code'
        }
    },
    type: 'object',
    required: [
        'device_code'
    ],
    title: 'DeviceApproveRequest'
} as const;

export const DeviceCodeResponseSchema = {
    properties: {
        device_code: {
            type: 'string',
            title: 'Device Code'
        },
        login_url: {
            type: 'string',
            title: 'Login Url'
        },
        expires_in: {
            type: 'integer',
            title: 'Expires In'
        }
    },
    type: 'object',
    required: [
        'device_code',
        'login_url',
        'expires_in'
    ],
    title: 'DeviceCodeResponse'
} as const;

export const DeviceNodeSchema = {
    properties: {
        node: {
            type: 'string',
            title: 'Node'
        },
        maxcpu: {
            type: 'integer',
            title: 'Maxcpu',
            default: 0
        },
        cpu_usage_ratio: {
            type: 'number',
            maximum: 1,
            minimum: 0,
            title: 'Cpu Usage Ratio',
            default: 0
        },
        maxmem_gb: {
            type: 'number',
            minimum: 0,
            title: 'Maxmem Gb',
            default: 0
        },
        mem_usage_ratio: {
            type: 'number',
            maximum: 1,
            minimum: 0,
            title: 'Mem Usage Ratio',
            default: 0
        },
        gpu_count: {
            type: 'integer',
            minimum: 0,
            title: 'Gpu Count',
            default: 0
        }
    },
    type: 'object',
    required: [
        'node'
    ],
    title: 'DeviceNode'
} as const;

export const DevicePollResponseSchema = {
    properties: {
        status: {
            type: 'string',
            title: 'Status'
        },
        access_token: {
            anyOf: [
                {
                    type: 'string'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Access Token'
        }
    },
    type: 'object',
    required: [
        'status'
    ],
    title: 'DevicePollResponse'
} as const;

export const DirectSpecUpdateRequestSchema = {
    properties: {
        cores: {
            anyOf: [
                {
                    type: 'integer',
                    maximum: 32,
                    minimum: 1
                },
                {
                    type: 'null'
                }
            ],
            title: 'Cores',
            description: 'CPU cores'
        },
        memory: {
            anyOf: [
                {
                    type: 'integer',
                    maximum: 65536,
                    minimum: 512
                },
                {
                    type: 'null'
                }
            ],
            title: 'Memory',
            description: 'Memory (MB)'
        },
        disk_size: {
            anyOf: [
                {
                    type: 'string',
                    pattern: '^\\+\\d+G$'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Disk Size',
            description: 'Disk size increment (e.g. "+10G")'
        }
    },
    type: 'object',
    title: 'DirectSpecUpdateRequest',
    description: '管理員直接調整規格'
} as const;

export const ExtendSessionResponseSchema = {
    properties: {
        vmid: {
            type: 'integer',
            title: 'Vmid'
        },
        auto_stop_at: {
            type: 'string',
            format: 'date-time',
            title: 'Auto Stop At'
        },
        extended_minutes: {
            type: 'integer',
            title: 'Extended Minutes'
        }
    },
    type: 'object',
    required: [
        'vmid',
        'auto_stop_at',
        'extended_minutes'
    ],
    title: 'ExtendSessionResponse',
    description: 'Returned after a successful session extension.'
} as const;

export const FirewallOptionsPublicSchema = {
    properties: {
        enable: {
            type: 'boolean',
            title: 'Enable'
        },
        policy_in: {
            type: 'string',
            title: 'Policy In'
        },
        policy_out: {
            type: 'string',
            title: 'Policy Out'
        }
    },
    type: 'object',
    required: [
        'enable',
        'policy_in',
        'policy_out'
    ],
    title: 'FirewallOptionsPublic',
    description: '防火牆選項（回應）'
} as const;

export const FirewallRuleCreateSchema = {
    properties: {
        type: {
            type: 'string',
            enum: [
                'in',
                'out'
            ],
            title: 'Type',
            description: '規則方向'
        },
        action: {
            type: 'string',
            enum: [
                'ACCEPT',
                'DROP',
                'REJECT'
            ],
            title: 'Action',
            description: '動作'
        },
        source: {
            anyOf: [
                {
                    type: 'string'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Source',
            description: '來源 IP/CIDR'
        },
        dest: {
            anyOf: [
                {
                    type: 'string'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Dest',
            description: '目標 IP/CIDR'
        },
        proto: {
            anyOf: [
                {
                    type: 'string'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Proto',
            description: '協定 (tcp/udp/icmp)'
        },
        dport: {
            anyOf: [
                {
                    type: 'string'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Dport',
            description: '目標端口或範圍'
        },
        sport: {
            anyOf: [
                {
                    type: 'string'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Sport',
            description: '來源端口或範圍'
        },
        enable: {
            type: 'integer',
            title: 'Enable',
            description: '是否啟用 (1=是, 0=否)',
            default: 1
        },
        comment: {
            anyOf: [
                {
                    type: 'string'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Comment',
            description: '備註'
        }
    },
    type: 'object',
    required: [
        'type',
        'action'
    ],
    title: 'FirewallRuleCreate',
    description: '建立防火牆規則（原始 Proxmox 規則）'
} as const;

export const FirewallRulePublicSchema = {
    properties: {
        pos: {
            type: 'integer',
            title: 'Pos'
        },
        type: {
            type: 'string',
            title: 'Type'
        },
        action: {
            type: 'string',
            title: 'Action'
        },
        source: {
            anyOf: [
                {
                    type: 'string'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Source'
        },
        dest: {
            anyOf: [
                {
                    type: 'string'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Dest'
        },
        proto: {
            anyOf: [
                {
                    type: 'string'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Proto'
        },
        dport: {
            anyOf: [
                {
                    type: 'string'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Dport'
        },
        sport: {
            anyOf: [
                {
                    type: 'string'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Sport'
        },
        enable: {
            type: 'integer',
            title: 'Enable',
            default: 1
        },
        comment: {
            anyOf: [
                {
                    type: 'string'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Comment'
        },
        is_managed: {
            type: 'boolean',
            title: 'Is Managed',
            description: '是否由 SkyLab 管理（comment 含 SkyLab: 前綴）',
            default: false
        }
    },
    type: 'object',
    required: [
        'pos',
        'type',
        'action'
    ],
    title: 'FirewallRulePublic',
    description: '防火牆規則（回應）'
} as const;

export const FirewallRuleUpdateSchema = {
    properties: {
        action: {
            anyOf: [
                {
                    type: 'string',
                    enum: [
                        'ACCEPT',
                        'DROP',
                        'REJECT'
                    ]
                },
                {
                    type: 'null'
                }
            ],
            title: 'Action'
        },
        source: {
            anyOf: [
                {
                    type: 'string'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Source'
        },
        dest: {
            anyOf: [
                {
                    type: 'string'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Dest'
        },
        proto: {
            anyOf: [
                {
                    type: 'string'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Proto'
        },
        dport: {
            anyOf: [
                {
                    type: 'string'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Dport'
        },
        sport: {
            anyOf: [
                {
                    type: 'string'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Sport'
        },
        enable: {
            anyOf: [
                {
                    type: 'integer'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Enable'
        },
        comment: {
            anyOf: [
                {
                    type: 'string'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Comment'
        }
    },
    type: 'object',
    title: 'FirewallRuleUpdate',
    description: '更新防火牆規則'
} as const;

export const GPUDeviceMapSchema = {
    properties: {
        node: {
            type: 'string',
            title: 'Node'
        },
        path: {
            type: 'string',
            title: 'Path'
        },
        id: {
            type: 'string',
            title: 'Id',
            default: ''
        },
        subsystem_id: {
            anyOf: [
                {
                    type: 'string'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Subsystem Id'
        },
        iommu_group: {
            anyOf: [
                {
                    type: 'integer'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Iommu Group'
        },
        description: {
            anyOf: [
                {
                    type: 'string'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Description'
        },
        is_mdev: {
            type: 'boolean',
            title: 'Is Mdev',
            default: false
        }
    },
    type: 'object',
    required: [
        'node',
        'path'
    ],
    title: 'GPUDeviceMap',
    description: 'A single node-level mapping entry for a GPU resource mapping.'
} as const;

export const GPUMappingCreateSchema = {
    properties: {
        id: {
            type: 'string',
            maxLength: 128,
            minLength: 1,
            title: 'Id',
            description: 'Mapping name'
        },
        description: {
            type: 'string',
            title: 'Description',
            default: ''
        },
        map: {
            items: {
                type: 'string'
            },
            type: 'array',
            minItems: 1,
            title: 'Map',
            description: 'List of map entries, e.g. \'node=pve1,path=0000:01:00.0\''
        }
    },
    type: 'object',
    required: [
        'id',
        'map'
    ],
    title: 'GPUMappingCreate',
    description: 'Create a new PCI resource mapping.'
} as const;

export const GPUMappingDetailSchema = {
    properties: {
        id: {
            type: 'string',
            title: 'Id',
            description: 'Mapping logical ID (name)'
        },
        description: {
            type: 'string',
            title: 'Description',
            default: ''
        },
        maps: {
            items: {
                $ref: '#/components/schemas/GPUDeviceMap'
            },
            type: 'array',
            title: 'Maps'
        },
        physical_gpu_count: {
            type: 'integer',
            title: 'Physical Gpu Count',
            description: 'Estimated physical GPU count (by unique PCI bus)',
            default: 0
        },
        device_count: {
            type: 'integer',
            title: 'Device Count',
            description: 'Total assignable device/VF slots',
            default: 0
        },
        used_count: {
            type: 'integer',
            title: 'Used Count',
            default: 0
        },
        available_count: {
            type: 'integer',
            title: 'Available Count',
            default: 0
        },
        is_sriov: {
            type: 'boolean',
            title: 'Is Sriov',
            description: 'True if SR-IOV detected (multiple devices on same PCI bus)',
            default: false
        },
        has_mdev: {
            type: 'boolean',
            title: 'Has Mdev',
            description: 'True if any device uses mediated devices',
            default: false
        },
        total_vram_mb: {
            type: 'integer',
            title: 'Total Vram Mb',
            description: 'Total physical VRAM in MB',
            default: 0
        },
        used_vram_mb: {
            type: 'integer',
            title: 'Used Vram Mb',
            description: 'Allocated VRAM in MB (sum of assigned vGPU/passthrough)',
            default: 0
        },
        used_by: {
            items: {
                $ref: '#/components/schemas/GPUUsageInfo'
            },
            type: 'array',
            title: 'Used By'
        }
    },
    type: 'object',
    required: [
        'id'
    ],
    title: 'GPUMappingDetail',
    description: 'Detail view with usage information.'
} as const;

export const GPUMappingsPublicSchema = {
    properties: {
        data: {
            items: {
                $ref: '#/components/schemas/GPUMappingDetail'
            },
            type: 'array',
            title: 'Data'
        },
        count: {
            type: 'integer',
            title: 'Count'
        }
    },
    type: 'object',
    required: [
        'data',
        'count'
    ],
    title: 'GPUMappingsPublic',
    description: 'List of GPU mappings.'
} as const;

export const GPUOptionContextSchema = {
    properties: {
        mapping_id: {
            type: 'string',
            title: 'Mapping Id'
        },
        description: {
            type: 'string',
            title: 'Description',
            default: ''
        },
        model: {
            type: 'string',
            title: 'Model',
            default: ''
        },
        vram: {
            type: 'string',
            title: 'Vram',
            default: ''
        },
        node: {
            type: 'string',
            title: 'Node',
            default: ''
        },
        available_count: {
            type: 'integer',
            title: 'Available Count',
            default: 0
        },
        device_count: {
            type: 'integer',
            title: 'Device Count',
            default: 0
        },
        used_count: {
            type: 'integer',
            title: 'Used Count',
            default: 0
        },
        total_vram_mb: {
            type: 'integer',
            title: 'Total Vram Mb',
            default: 0
        },
        used_vram_mb: {
            type: 'integer',
            title: 'Used Vram Mb',
            default: 0
        },
        has_mdev: {
            type: 'boolean',
            title: 'Has Mdev',
            default: false
        },
        is_sriov: {
            type: 'boolean',
            title: 'Is Sriov',
            default: false
        }
    },
    type: 'object',
    required: [
        'mapping_id'
    ],
    title: 'GPUOptionContext'
} as const;

export const GPUSummarySchema = {
    properties: {
        mapping_id: {
            type: 'string',
            title: 'Mapping Id'
        },
        description: {
            type: 'string',
            title: 'Description',
            default: ''
        },
        model: {
            type: 'string',
            title: 'Model',
            default: ''
        },
        vram: {
            type: 'string',
            title: 'Vram',
            default: ''
        },
        node: {
            type: 'string',
            title: 'Node',
            default: ''
        },
        physical_gpu_count: {
            type: 'integer',
            title: 'Physical Gpu Count',
            default: 1
        },
        device_count: {
            type: 'integer',
            title: 'Device Count',
            default: 1
        },
        used_count: {
            type: 'integer',
            title: 'Used Count',
            default: 0
        },
        available_count: {
            type: 'integer',
            title: 'Available Count',
            default: 1
        },
        is_sriov: {
            type: 'boolean',
            title: 'Is Sriov',
            default: false
        },
        has_mdev: {
            type: 'boolean',
            title: 'Has Mdev',
            default: false
        },
        total_vram_mb: {
            type: 'integer',
            title: 'Total Vram Mb',
            default: 0
        },
        used_vram_mb: {
            type: 'integer',
            title: 'Used Vram Mb',
            default: 0
        }
    },
    type: 'object',
    required: [
        'mapping_id'
    ],
    title: 'GPUSummary',
    description: 'A simplified GPU option for the application form selector.'
} as const;

export const GPUUsageInfoSchema = {
    properties: {
        vmid: {
            type: 'integer',
            title: 'Vmid'
        },
        vm_name: {
            type: 'string',
            title: 'Vm Name',
            default: ''
        },
        node: {
            type: 'string',
            title: 'Node',
            default: ''
        },
        status: {
            type: 'string',
            title: 'Status',
            default: ''
        },
        mdev_type: {
            type: 'string',
            title: 'Mdev Type',
            default: ''
        },
        allocated_vram_mb: {
            type: 'integer',
            title: 'Allocated Vram Mb',
            default: 0
        }
    },
    type: 'object',
    required: [
        'vmid'
    ],
    title: 'GPUUsageInfo',
    description: 'Information about a VM using a GPU mapping.'
} as const;

export const GatewayConfigPublicSchema = {
    properties: {
        host: {
            type: 'string',
            title: 'Host'
        },
        ssh_port: {
            type: 'integer',
            title: 'Ssh Port'
        },
        ssh_user: {
            type: 'string',
            title: 'Ssh User'
        },
        public_key: {
            type: 'string',
            title: 'Public Key'
        },
        is_configured: {
            type: 'boolean',
            title: 'Is Configured'
        }
    },
    type: 'object',
    required: [
        'host',
        'ssh_port',
        'ssh_user',
        'public_key',
        'is_configured'
    ],
    title: 'GatewayConfigPublic'
} as const;

export const GatewayConfigUpdateSchema = {
    properties: {
        host: {
            type: 'string',
            maxLength: 255,
            title: 'Host'
        },
        ssh_port: {
            type: 'integer',
            maximum: 65535,
            minimum: 1,
            title: 'Ssh Port',
            default: 22
        },
        ssh_user: {
            type: 'string',
            maxLength: 64,
            title: 'Ssh User',
            default: 'root'
        }
    },
    type: 'object',
    required: [
        'host'
    ],
    title: 'GatewayConfigUpdate'
} as const;

export const GatewayConnectionTestResultSchema = {
    properties: {
        success: {
            type: 'boolean',
            title: 'Success'
        },
        message: {
            type: 'string',
            title: 'Message'
        }
    },
    type: 'object',
    required: [
        'success',
        'message'
    ],
    title: 'GatewayConnectionTestResult'
} as const;

export const GatewayServiceVersionInfoSchema = {
    properties: {
        service: {
            type: 'string',
            enum: [
                'haproxy',
                'traefik',
                'frps',
                'frpc'
            ],
            title: 'Service'
        },
        current_version: {
            anyOf: [
                {
                    type: 'string'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Current Version'
        },
        target_version: {
            anyOf: [
                {
                    type: 'string'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Target Version'
        },
        update_available: {
            anyOf: [
                {
                    type: 'boolean'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Update Available'
        },
        source: {
            type: 'string',
            title: 'Source'
        },
        detection_error: {
            anyOf: [
                {
                    type: 'string'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Detection Error'
        }
    },
    type: 'object',
    required: [
        'service',
        'source'
    ],
    title: 'GatewayServiceVersionInfo'
} as const;

export const GatewayServiceVersionsResultSchema = {
    properties: {
        items: {
            items: {
                $ref: '#/components/schemas/GatewayServiceVersionInfo'
            },
            type: 'array',
            title: 'Items'
        },
        checked_at: {
            type: 'string',
            format: 'date-time',
            title: 'Checked At'
        }
    },
    type: 'object',
    required: [
        'items',
        'checked_at'
    ],
    title: 'GatewayServiceVersionsResult'
} as const;

export const GoogleLoginRequestSchema = {
    properties: {
        id_token: {
            type: 'string',
            title: 'Id Token'
        }
    },
    type: 'object',
    required: [
        'id_token'
    ],
    title: 'GoogleLoginRequest'
} as const;

export const GroupCreateSchema = {
    properties: {
        name: {
            type: 'string',
            maxLength: 255,
            minLength: 1,
            title: 'Name'
        },
        description: {
            anyOf: [
                {
                    type: 'string',
                    maxLength: 1000
                },
                {
                    type: 'null'
                }
            ],
            title: 'Description'
        }
    },
    type: 'object',
    required: [
        'name'
    ],
    title: 'GroupCreate',
    description: '建立群組'
} as const;

export const GroupDetailPublicSchema = {
    properties: {
        id: {
            type: 'string',
            format: 'uuid',
            title: 'Id'
        },
        name: {
            type: 'string',
            title: 'Name'
        },
        description: {
            anyOf: [
                {
                    type: 'string'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Description'
        },
        owner_id: {
            type: 'string',
            format: 'uuid',
            title: 'Owner Id'
        },
        created_at: {
            anyOf: [
                {
                    type: 'string',
                    format: 'date-time'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Created At'
        },
        members: {
            items: {
                $ref: '#/components/schemas/GroupMemberPublic'
            },
            type: 'array',
            title: 'Members',
            default: []
        }
    },
    type: 'object',
    required: [
        'id',
        'name',
        'owner_id'
    ],
    title: 'GroupDetailPublic',
    description: '群組詳情（含成員列表）'
} as const;

export const GroupMemberAddSchema = {
    properties: {
        emails: {
            items: {
                type: 'string',
                format: 'email'
            },
            type: 'array',
            title: 'Emails'
        }
    },
    type: 'object',
    required: [
        'emails'
    ],
    title: 'GroupMemberAdd',
    description: '新增群組成員'
} as const;

export const GroupMemberPublicSchema = {
    properties: {
        user_id: {
            type: 'string',
            format: 'uuid',
            title: 'User Id'
        },
        email: {
            type: 'string',
            format: 'email',
            title: 'Email'
        },
        full_name: {
            anyOf: [
                {
                    type: 'string'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Full Name'
        },
        added_at: {
            anyOf: [
                {
                    type: 'string',
                    format: 'date-time'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Added At'
        },
        vmid: {
            anyOf: [
                {
                    type: 'integer'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Vmid'
        },
        vm_status: {
            anyOf: [
                {
                    type: 'string'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Vm Status'
        },
        vm_type: {
            anyOf: [
                {
                    type: 'string'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Vm Type'
        },
        vm_cpu_usage_pct: {
            anyOf: [
                {
                    type: 'number'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Vm Cpu Usage Pct'
        },
        vm_ram_usage_pct: {
            anyOf: [
                {
                    type: 'number'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Vm Ram Usage Pct'
        },
        vm_disk_usage_pct: {
            anyOf: [
                {
                    type: 'number'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Vm Disk Usage Pct'
        }
    },
    type: 'object',
    required: [
        'user_id',
        'email'
    ],
    title: 'GroupMemberPublic',
    description: '群組成員資料'
} as const;

export const GroupPublicSchema = {
    properties: {
        id: {
            type: 'string',
            format: 'uuid',
            title: 'Id'
        },
        name: {
            type: 'string',
            title: 'Name'
        },
        description: {
            anyOf: [
                {
                    type: 'string'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Description'
        },
        owner_id: {
            type: 'string',
            format: 'uuid',
            title: 'Owner Id'
        },
        created_at: {
            anyOf: [
                {
                    type: 'string',
                    format: 'date-time'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Created At'
        },
        member_count: {
            type: 'integer',
            title: 'Member Count',
            default: 0
        }
    },
    type: 'object',
    required: [
        'id',
        'name',
        'owner_id'
    ],
    title: 'GroupPublic',
    description: 'API 回傳的群組資料'
} as const;

export const GroupsPublicSchema = {
    properties: {
        data: {
            items: {
                $ref: '#/components/schemas/GroupPublic'
            },
            type: 'array',
            title: 'Data'
        },
        count: {
            type: 'integer',
            title: 'Count'
        }
    },
    type: 'object',
    required: [
        'data',
        'count'
    ],
    title: 'GroupsPublic',
    description: '群組列表回應'
} as const;

export const HTTPValidationErrorSchema = {
    properties: {
        detail: {
            items: {
                $ref: '#/components/schemas/ValidationError'
            },
            type: 'array',
            title: 'Detail'
        }
    },
    type: 'object',
    title: 'HTTPValidationError'
} as const;

export const HealthResponseSchema = {
    properties: {
        overall: {
            type: 'string',
            enum: [
                'ok',
                'degraded',
                'error'
            ],
            title: 'Overall'
        },
        db: {
            $ref: '#/components/schemas/DependencyStatus'
        },
        redis: {
            $ref: '#/components/schemas/DependencyStatus'
        },
        proxmox: {
            $ref: '#/components/schemas/DependencyStatus'
        }
    },
    type: 'object',
    required: [
        'overall',
        'db',
        'redis',
        'proxmox'
    ],
    title: 'HealthResponse'
} as const;

export const IpAllocationListResponseSchema = {
    properties: {
        allocations: {
            items: {
                $ref: '#/components/schemas/IpAllocationPublic'
            },
            type: 'array',
            title: 'Allocations'
        },
        total: {
            type: 'integer',
            title: 'Total'
        }
    },
    type: 'object',
    required: [
        'allocations',
        'total'
    ],
    title: 'IpAllocationListResponse',
    description: 'IP 分配列表回傳'
} as const;

export const IpAllocationPublicSchema = {
    properties: {
        ip_address: {
            type: 'string',
            title: 'Ip Address'
        },
        purpose: {
            type: 'string',
            title: 'Purpose'
        },
        vmid: {
            anyOf: [
                {
                    type: 'integer'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Vmid'
        },
        description: {
            anyOf: [
                {
                    type: 'string'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Description'
        },
        allocated_at: {
            type: 'string',
            format: 'date-time',
            title: 'Allocated At'
        }
    },
    type: 'object',
    required: [
        'ip_address',
        'purpose',
        'vmid',
        'description',
        'allocated_at'
    ],
    title: 'IpAllocationPublic',
    description: 'IP 分配記錄公開格式'
} as const;

export const JobDetailSchema = {
    properties: {
        item: {
            $ref: '#/components/schemas/JobItem'
        },
        output: {
            anyOf: [
                {
                    type: 'string'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Output',
            description: '完整文字輸出（script_deploy 的 stdout/stderr；migration 的 last_error）'
        },
        error: {
            anyOf: [
                {
                    type: 'string'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Error'
        },
        extra: {
            additionalProperties: true,
            type: 'object',
            title: 'Extra',
            description: 'kind 特定附加資訊（如 spec_change diff、vm_request 規格、migration 時間戳記）'
        }
    },
    type: 'object',
    required: [
        'item'
    ],
    title: 'JobDetail',
    description: 'Job 詳細資訊，包含 kind 特定的額外欄位（如腳本輸出、規格 diff 等）。'
} as const;

export const JobItemSchema = {
    properties: {
        id: {
            type: 'string',
            title: 'Id',
            description: '複合 ID，格式：<kind>:<source_id>'
        },
        kind: {
            $ref: '#/components/schemas/JobKind'
        },
        title: {
            type: 'string',
            title: 'Title'
        },
        status: {
            $ref: '#/components/schemas/JobStatus'
        },
        progress: {
            anyOf: [
                {
                    type: 'integer',
                    maximum: 100,
                    minimum: 0
                },
                {
                    type: 'null'
                }
            ],
            title: 'Progress',
            description: '0-100, 若不可估算則為 null'
        },
        message: {
            anyOf: [
                {
                    type: 'string'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Message'
        },
        user_id: {
            anyOf: [
                {
                    type: 'string',
                    format: 'uuid'
                },
                {
                    type: 'null'
                }
            ],
            title: 'User Id'
        },
        user_email: {
            anyOf: [
                {
                    type: 'string'
                },
                {
                    type: 'null'
                }
            ],
            title: 'User Email'
        },
        created_at: {
            type: 'string',
            format: 'date-time',
            title: 'Created At'
        },
        updated_at: {
            type: 'string',
            format: 'date-time',
            title: 'Updated At'
        },
        completed_at: {
            anyOf: [
                {
                    type: 'string',
                    format: 'date-time'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Completed At'
        },
        detail_url: {
            anyOf: [
                {
                    type: 'string'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Detail Url',
            description: '前端可點擊跳轉的相對路徑（例：/jobs?focus=migration:xxx）'
        },
        meta: {
            additionalProperties: true,
            type: 'object',
            title: 'Meta'
        }
    },
    type: 'object',
    required: [
        'id',
        'kind',
        'title',
        'status',
        'created_at',
        'updated_at'
    ],
    title: 'JobItem',
    description: '統一的 Job 顯示模型。'
} as const;

export const JobKindSchema = {
    type: 'string',
    enum: [
        'migration',
        'script_deploy',
        'vm_request',
        'spec_change',
        'deletion'
    ],
    title: 'JobKind'
} as const;

export const JobStatusSchema = {
    type: 'string',
    enum: [
        'pending',
        'running',
        'completed',
        'failed',
        'blocked',
        'cancelled'
    ],
    title: 'JobStatus',
    description: '正規化狀態。各來源的原始狀態會 map 到此枚舉。'
} as const;

export const JobsListResponseSchema = {
    properties: {
        items: {
            items: {
                $ref: '#/components/schemas/JobItem'
            },
            type: 'array',
            title: 'Items'
        },
        total: {
            type: 'integer',
            title: 'Total'
        },
        active_count: {
            type: 'integer',
            title: 'Active Count'
        }
    },
    type: 'object',
    required: [
        'items',
        'total',
        'active_count'
    ],
    title: 'JobsListResponse'
} as const;

export const LXCCreateRequestSchema = {
    properties: {
        hostname: {
            type: 'string',
            maxLength: 63,
            minLength: 1,
            title: 'Hostname'
        },
        ostemplate: {
            type: 'string',
            title: 'Ostemplate'
        },
        cores: {
            type: 'integer',
            maximum: 32,
            minimum: 1,
            title: 'Cores',
            default: 1
        },
        memory: {
            type: 'integer',
            maximum: 65536,
            minimum: 128,
            title: 'Memory',
            default: 512
        },
        rootfs_size: {
            type: 'integer',
            maximum: 1000,
            minimum: 1,
            title: 'Rootfs Size',
            default: 8
        },
        password: {
            type: 'string',
            minLength: 6,
            title: 'Password'
        },
        storage: {
            type: 'string',
            title: 'Storage',
            default: 'local-lvm'
        },
        environment_type: {
            type: 'string',
            title: 'Environment Type'
        },
        os_info: {
            anyOf: [
                {
                    type: 'string'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Os Info'
        },
        expiry_date: {
            anyOf: [
                {
                    type: 'string',
                    format: 'date'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Expiry Date'
        },
        start: {
            type: 'boolean',
            title: 'Start',
            default: true
        },
        unprivileged: {
            type: 'boolean',
            title: 'Unprivileged',
            default: true
        },
        service_template_slug: {
            anyOf: [
                {
                    type: 'string'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Service Template Slug'
        }
    },
    type: 'object',
    required: [
        'hostname',
        'ostemplate',
        'password',
        'environment_type'
    ],
    title: 'LXCCreateRequest',
    description: '建立 LXC 容器'
} as const;

export const LXCCreateResponseSchema = {
    properties: {
        vmid: {
            type: 'integer',
            title: 'Vmid'
        },
        upid: {
            type: 'string',
            title: 'Upid'
        },
        message: {
            type: 'string',
            title: 'Message'
        }
    },
    type: 'object',
    required: [
        'vmid',
        'upid',
        'message'
    ],
    title: 'LXCCreateResponse',
    description: '建立 LXC 回應'
} as const;

export const LayoutNodeUpdateSchema = {
    properties: {
        vmid: {
            anyOf: [
                {
                    type: 'integer'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Vmid',
            description: 'VM ID；None 代表 gateway'
        },
        node_type: {
            type: 'string',
            enum: [
                'vm',
                'gateway'
            ],
            title: 'Node Type',
            description: '節點類型'
        },
        position_x: {
            type: 'number',
            title: 'Position X',
            description: 'X 座標'
        },
        position_y: {
            type: 'number',
            title: 'Position Y',
            description: 'Y 座標'
        }
    },
    type: 'object',
    required: [
        'node_type',
        'position_x',
        'position_y'
    ],
    title: 'LayoutNodeUpdate',
    description: '更新節點位置'
} as const;

export const LayoutUpdateSchema = {
    properties: {
        nodes: {
            items: {
                $ref: '#/components/schemas/LayoutNodeUpdate'
            },
            type: 'array',
            title: 'Nodes'
        }
    },
    type: 'object',
    required: [
        'nodes'
    ],
    title: 'LayoutUpdate',
    description: '批次更新圖形佈局'
} as const;

export const MachineCurrentStatusSchema = {
    properties: {
        node: {
            type: 'string',
            title: 'Node'
        },
        status: {
            type: 'string',
            title: 'Status'
        },
        candidate: {
            type: 'boolean',
            title: 'Candidate'
        },
        running_resources: {
            type: 'integer',
            minimum: 0,
            title: 'Running Resources',
            default: 0
        },
        cpu_usage_ratio: {
            type: 'number',
            minimum: 0,
            title: 'Cpu Usage Ratio',
            default: 0
        },
        memory_usage_ratio: {
            type: 'number',
            minimum: 0,
            title: 'Memory Usage Ratio',
            default: 0
        },
        disk_usage_ratio: {
            type: 'number',
            minimum: 0,
            title: 'Disk Usage Ratio',
            default: 0
        },
        allocatable_cpu_cores: {
            type: 'number',
            minimum: 0,
            title: 'Allocatable Cpu Cores',
            default: 0
        },
        allocatable_memory_gb: {
            type: 'number',
            minimum: 0,
            title: 'Allocatable Memory Gb',
            default: 0
        },
        allocatable_disk_gb: {
            type: 'number',
            minimum: 0,
            title: 'Allocatable Disk Gb',
            default: 0
        },
        gpu_count: {
            type: 'integer',
            minimum: 0,
            title: 'Gpu Count',
            default: 0
        }
    },
    type: 'object',
    required: [
        'node',
        'status',
        'candidate'
    ],
    title: 'MachineCurrentStatus'
} as const;

export const MessageSchema = {
    properties: {
        message: {
            type: 'string',
            title: 'Message'
        }
    },
    type: 'object',
    required: [
        'message'
    ],
    title: 'Message',
    description: '通用訊息回應'
} as const;

export const MigrationJobPublicSchema = {
    properties: {
        id: {
            type: 'string',
            format: 'uuid',
            title: 'Id'
        },
        request_id: {
            type: 'string',
            format: 'uuid',
            title: 'Request Id'
        },
        vmid: {
            anyOf: [
                {
                    type: 'integer'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Vmid'
        },
        source_node: {
            anyOf: [
                {
                    type: 'string'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Source Node'
        },
        target_node: {
            type: 'string',
            title: 'Target Node'
        },
        status: {
            $ref: '#/components/schemas/VMMigrationJobStatus'
        },
        rebalance_epoch: {
            type: 'integer',
            title: 'Rebalance Epoch',
            default: 0
        },
        attempt_count: {
            type: 'integer',
            title: 'Attempt Count',
            default: 0
        },
        last_error: {
            anyOf: [
                {
                    type: 'string'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Last Error'
        },
        requested_at: {
            type: 'string',
            format: 'date-time',
            title: 'Requested At'
        },
        available_at: {
            anyOf: [
                {
                    type: 'string',
                    format: 'date-time'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Available At'
        },
        claimed_by: {
            anyOf: [
                {
                    type: 'string'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Claimed By'
        },
        claimed_at: {
            anyOf: [
                {
                    type: 'string',
                    format: 'date-time'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Claimed At'
        },
        claim_expires_at: {
            anyOf: [
                {
                    type: 'string',
                    format: 'date-time'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Claim Expires At'
        },
        started_at: {
            anyOf: [
                {
                    type: 'string',
                    format: 'date-time'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Started At'
        },
        finished_at: {
            anyOf: [
                {
                    type: 'string',
                    format: 'date-time'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Finished At'
        },
        updated_at: {
            type: 'string',
            format: 'date-time',
            title: 'Updated At'
        }
    },
    type: 'object',
    required: [
        'id',
        'request_id',
        'target_node',
        'status',
        'requested_at',
        'updated_at'
    ],
    title: 'MigrationJobPublic'
} as const;

export const MigrationJobsPublicSchema = {
    properties: {
        data: {
            items: {
                $ref: '#/components/schemas/MigrationJobPublic'
            },
            type: 'array',
            title: 'Data'
        },
        count: {
            type: 'integer',
            title: 'Count'
        }
    },
    type: 'object',
    required: [
        'data',
        'count'
    ],
    title: 'MigrationJobsPublic'
} as const;

export const MigrationStatsPublicSchema = {
    properties: {
        total_jobs: {
            type: 'integer',
            title: 'Total Jobs',
            default: 0
        },
        by_status: {
            additionalProperties: {
                type: 'integer'
            },
            type: 'object',
            title: 'By Status'
        },
        avg_duration_seconds: {
            type: 'number',
            title: 'Avg Duration Seconds',
            default: 0
        },
        success_rate: {
            type: 'number',
            title: 'Success Rate',
            default: 0
        }
    },
    type: 'object',
    title: 'MigrationStatsPublic'
} as const;

export const ModelInfoSchema = {
    properties: {
        id: {
            type: 'string',
            title: 'Id'
        },
        object: {
            type: 'string',
            title: 'Object',
            default: 'model'
        },
        created: {
            anyOf: [
                {
                    type: 'integer'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Created'
        },
        owned_by: {
            type: 'string',
            title: 'Owned By',
            default: 'SkyLab'
        }
    },
    type: 'object',
    required: [
        'id'
    ],
    title: 'ModelInfo',
    description: '模型信息'
} as const;

export const ModelsResponseSchema = {
    properties: {
        data: {
            items: {
                $ref: '#/components/schemas/ModelInfo'
            },
            type: 'array',
            title: 'Data'
        },
        object: {
            type: 'string',
            title: 'Object',
            default: 'list'
        }
    },
    type: 'object',
    required: [
        'data'
    ],
    title: 'ModelsResponse',
    description: '模型列表响应'
} as const;

export const NATRulePublicSchema = {
    properties: {
        id: {
            type: 'string',
            format: 'uuid',
            title: 'Id'
        },
        ssh_host: {
            type: 'string',
            title: 'Ssh Host'
        },
        vmid: {
            type: 'integer',
            title: 'Vmid'
        },
        vm_ip: {
            type: 'string',
            title: 'Vm Ip'
        },
        external_port: {
            type: 'integer',
            title: 'External Port'
        },
        internal_port: {
            type: 'integer',
            title: 'Internal Port'
        },
        protocol: {
            type: 'string',
            title: 'Protocol'
        },
        created_at: {
            type: 'string',
            format: 'date-time',
            title: 'Created At'
        }
    },
    type: 'object',
    required: [
        'id',
        'ssh_host',
        'vmid',
        'vm_ip',
        'external_port',
        'internal_port',
        'protocol',
        'created_at'
    ],
    title: 'NATRulePublic',
    description: 'NAT 端口轉發規則（回應）'
} as const;

export const NavigationResolveRequestSchema = {
    properties: {
        query: {
            type: 'string',
            maxLength: 2000,
            minLength: 2,
            title: 'Query'
        }
    },
    type: 'object',
    required: [
        'query'
    ],
    title: 'NavigationResolveRequest'
} as const;

export const NavigationResolveResponseSchema = {
    properties: {
        intent: {
            type: 'string',
            title: 'Intent'
        },
        confidence: {
            type: 'number',
            maximum: 1,
            minimum: 0,
            title: 'Confidence',
            default: 0
        },
        action: {
            type: 'string',
            enum: [
                'navigate',
                'suggest',
                'clarify'
            ],
            title: 'Action'
        },
        primary: {
            anyOf: [
                {
                    $ref: '#/components/schemas/NavigationTarget'
                },
                {
                    type: 'null'
                }
            ]
        },
        suggestions: {
            items: {
                $ref: '#/components/schemas/NavigationTarget'
            },
            type: 'array',
            title: 'Suggestions'
        },
        clarification_question: {
            anyOf: [
                {
                    type: 'string'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Clarification Question'
        }
    },
    type: 'object',
    required: [
        'intent',
        'action'
    ],
    title: 'NavigationResolveResponse'
} as const;

export const NavigationTargetSchema = {
    properties: {
        title: {
            type: 'string',
            title: 'Title'
        },
        path: {
            type: 'string',
            title: 'Path'
        },
        reason: {
            type: 'string',
            title: 'Reason',
            default: ''
        }
    },
    type: 'object',
    required: [
        'title',
        'path'
    ],
    title: 'NavigationTarget'
} as const;

export const NetworkInterfaceSchema = {
    properties: {
        vmid: {
            type: 'integer',
            title: 'Vmid'
        },
        name: {
            type: 'string',
            title: 'Name'
        },
        inet: {
            anyOf: [
                {
                    type: 'string'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Inet'
        },
        inet6: {
            anyOf: [
                {
                    type: 'string'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Inet6'
        },
        hwaddr: {
            anyOf: [
                {
                    type: 'string'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Hwaddr'
        }
    },
    type: 'object',
    required: [
        'vmid',
        'name'
    ],
    title: 'NetworkInterface'
} as const;

export const NewPasswordSchema = {
    properties: {
        token: {
            type: 'string',
            title: 'Token'
        },
        new_password: {
            type: 'string',
            maxLength: 128,
            minLength: 8,
            title: 'New Password'
        }
    },
    type: 'object',
    required: [
        'token',
        'new_password'
    ],
    title: 'NewPassword',
    description: '重設密碼請求'
} as const;

export const NodeInfoSchema = {
    properties: {
        node: {
            type: 'string',
            title: 'Node'
        },
        status: {
            type: 'string',
            title: 'Status'
        },
        cpu_usage: {
            type: 'number',
            title: 'Cpu Usage'
        },
        cpu_cores: {
            type: 'integer',
            title: 'Cpu Cores'
        },
        mem_used_bytes: {
            type: 'integer',
            title: 'Mem Used Bytes'
        },
        mem_total_bytes: {
            type: 'integer',
            title: 'Mem Total Bytes'
        },
        mem_used_pct: {
            type: 'number',
            title: 'Mem Used Pct'
        },
        disk_used_bytes: {
            type: 'integer',
            title: 'Disk Used Bytes'
        },
        disk_total_bytes: {
            type: 'integer',
            title: 'Disk Total Bytes'
        },
        disk_used_pct: {
            type: 'number',
            title: 'Disk Used Pct'
        },
        uptime_seconds: {
            anyOf: [
                {
                    type: 'integer'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Uptime Seconds'
        }
    },
    type: 'object',
    required: [
        'node',
        'status',
        'cpu_usage',
        'cpu_cores',
        'mem_used_bytes',
        'mem_total_bytes',
        'mem_used_pct',
        'disk_used_bytes',
        'disk_total_bytes',
        'disk_used_pct'
    ],
    title: 'NodeInfo'
} as const;

export const NodeSchemaSchema = {
    properties: {
        node: {
            type: 'string',
            title: 'Node'
        },
        status: {
            type: 'string',
            title: 'Status'
        },
        cpu: {
            anyOf: [
                {
                    type: 'number'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Cpu'
        },
        maxcpu: {
            anyOf: [
                {
                    type: 'integer'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Maxcpu'
        },
        mem: {
            anyOf: [
                {
                    type: 'integer'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Mem'
        },
        maxmem: {
            anyOf: [
                {
                    type: 'integer'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Maxmem'
        },
        uptime: {
            anyOf: [
                {
                    type: 'integer'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Uptime'
        }
    },
    type: 'object',
    required: [
        'node',
        'status'
    ],
    title: 'NodeSchema',
    description: 'Proxmox 節點資訊'
} as const;

export const NodeStatsPublicSchema = {
    properties: {
        name: {
            type: 'string',
            title: 'Name'
        },
        status: {
            type: 'string',
            title: 'Status'
        },
        cpu_usage_pct: {
            type: 'number',
            title: 'Cpu Usage Pct'
        },
        cpu_cores: {
            type: 'integer',
            title: 'Cpu Cores'
        },
        mem_used_gb: {
            type: 'number',
            title: 'Mem Used Gb'
        },
        mem_total_gb: {
            type: 'number',
            title: 'Mem Total Gb'
        },
        disk_used_gb: {
            type: 'number',
            title: 'Disk Used Gb'
        },
        disk_total_gb: {
            type: 'number',
            title: 'Disk Total Gb'
        },
        vm_count: {
            type: 'integer',
            title: 'Vm Count',
            default: 0
        }
    },
    type: 'object',
    required: [
        'name',
        'status',
        'cpu_usage_pct',
        'cpu_cores',
        'mem_used_gb',
        'mem_total_gb',
        'disk_used_gb',
        'disk_total_gb'
    ],
    title: 'NodeStatsPublic',
    description: '單一節點的即時資源使用狀態'
} as const;

export const PlacementAdvisorResponseSchema = {
    properties: {
        generated_at: {
            type: 'string',
            format: 'date-time',
            title: 'Generated At'
        },
        reply: {
            type: 'string',
            title: 'Reply'
        },
        machines_to_open: {
            items: {
                $ref: '#/components/schemas/RecommendedMachine'
            },
            type: 'array',
            title: 'Machines To Open'
        },
        reasons: {
            items: {
                type: 'string'
            },
            type: 'array',
            title: 'Reasons'
        },
        current_status: {
            items: {
                $ref: '#/components/schemas/MachineCurrentStatus'
            },
            type: 'array',
            title: 'Current Status'
        },
        ai_used: {
            type: 'boolean',
            title: 'Ai Used',
            default: false
        },
        model: {
            anyOf: [
                {
                    type: 'string'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Model'
        },
        warning: {
            anyOf: [
                {
                    type: 'string'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Warning'
        },
        ai_metrics: {
            anyOf: [
                {
                    $ref: '#/components/schemas/AiMetrics'
                },
                {
                    type: 'null'
                }
            ]
        }
    },
    type: 'object',
    required: [
        'reply'
    ],
    title: 'PlacementAdvisorResponse'
} as const;

export const PlacementRequestSchema = {
    properties: {
        resource_type: {
            type: 'string',
            enum: [
                'lxc',
                'vm'
            ],
            title: 'Resource Type',
            default: 'vm'
        },
        cpu_cores: {
            type: 'integer',
            maximum: 256,
            minimum: 1,
            title: 'Cpu Cores',
            default: 2
        },
        memory_mb: {
            type: 'integer',
            maximum: 1048576,
            minimum: 128,
            title: 'Memory Mb',
            default: 2048
        },
        disk_gb: {
            type: 'integer',
            maximum: 65536,
            minimum: 1,
            title: 'Disk Gb',
            default: 20
        },
        instance_count: {
            type: 'integer',
            maximum: 100,
            minimum: 1,
            title: 'Instance Count',
            default: 1
        },
        gpu_required: {
            type: 'integer',
            maximum: 16,
            minimum: 0,
            title: 'Gpu Required',
            default: 0
        }
    },
    type: 'object',
    title: 'PlacementRequest'
} as const;

export const PortSpecSchema = {
    properties: {
        port: {
            type: 'integer',
            maximum: 65535,
            minimum: 0,
            title: 'Port',
            description: '端口號；0 表示無端口協定'
        },
        protocol: {
            type: 'string',
            title: 'Protocol',
            description: '協定 (tcp/udp/icmp/esp/ah/...)',
            default: 'tcp'
        },
        external_port: {
            anyOf: [
                {
                    type: 'integer',
                    maximum: 65535,
                    minimum: 1
                },
                {
                    type: 'null'
                }
            ],
            title: 'External Port',
            description: '外網入站 port（Port 轉發用）'
        },
        domain: {
            anyOf: [
                {
                    type: 'string',
                    maxLength: 255
                },
                {
                    type: 'null'
                }
            ],
            title: 'Domain',
            description: '對外網域名稱（反向代理用）'
        },
        enable_https: {
            type: 'boolean',
            title: 'Enable Https',
            description: '反向代理是否啟用 HTTPS（Let\'s Encrypt）',
            default: true
        }
    },
    type: 'object',
    required: [
        'port'
    ],
    title: 'PortSpec',
    description: '端口規格（port=0 表示無端口協定，如 icmp/esp 等）\n\n三種入站存取模式：\n- domain 有值 → 反向代理（Traefik）\n- external_port 有值 → Port 轉發（haproxy）\n- 兩者皆無 → 僅開放防火牆'
} as const;

export const PrivateUserCreateSchema = {
    properties: {
        email: {
            type: 'string',
            title: 'Email'
        },
        password: {
            type: 'string',
            title: 'Password'
        },
        full_name: {
            type: 'string',
            title: 'Full Name'
        },
        is_verified: {
            type: 'boolean',
            title: 'Is Verified',
            default: false
        }
    },
    type: 'object',
    required: [
        'email',
        'password',
        'full_name'
    ],
    title: 'PrivateUserCreate'
} as const;

export const ProxmoxConfigPublicSchema = {
    properties: {
        host: {
            type: 'string',
            title: 'Host'
        },
        user: {
            type: 'string',
            title: 'User'
        },
        verify_ssl: {
            type: 'boolean',
            title: 'Verify Ssl'
        },
        iso_storage: {
            type: 'string',
            title: 'Iso Storage'
        },
        data_storage: {
            type: 'string',
            title: 'Data Storage'
        },
        api_timeout: {
            type: 'integer',
            title: 'Api Timeout'
        },
        task_check_interval: {
            type: 'integer',
            title: 'Task Check Interval'
        },
        pool_name: {
            type: 'string',
            title: 'Pool Name'
        },
        gateway_ip: {
            anyOf: [
                {
                    type: 'string'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Gateway Ip'
        },
        local_subnet: {
            anyOf: [
                {
                    type: 'string'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Local Subnet'
        },
        default_node: {
            anyOf: [
                {
                    type: 'string'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Default Node'
        },
        placement_strategy: {
            type: 'string',
            title: 'Placement Strategy',
            default: 'priority_dominant_share'
        },
        cpu_overcommit_ratio: {
            type: 'number',
            title: 'Cpu Overcommit Ratio',
            default: 2
        },
        disk_overcommit_ratio: {
            type: 'number',
            title: 'Disk Overcommit Ratio',
            default: 1
        },
        migration_enabled: {
            type: 'boolean',
            title: 'Migration Enabled',
            default: true
        },
        migration_max_per_rebalance: {
            type: 'integer',
            title: 'Migration Max Per Rebalance',
            default: 2
        },
        migration_min_interval_minutes: {
            type: 'integer',
            title: 'Migration Min Interval Minutes',
            default: 60
        },
        migration_retry_limit: {
            type: 'integer',
            title: 'Migration Retry Limit',
            default: 3
        },
        rebalance_migration_cost: {
            type: 'number',
            title: 'Rebalance Migration Cost',
            default: 0.15
        },
        rebalance_peak_cpu_margin: {
            type: 'number',
            title: 'Rebalance Peak Cpu Margin',
            default: 1.1
        },
        rebalance_peak_memory_margin: {
            type: 'number',
            title: 'Rebalance Peak Memory Margin',
            default: 1.05
        },
        rebalance_loadavg_warn_per_core: {
            type: 'number',
            title: 'Rebalance Loadavg Warn Per Core',
            default: 0.8
        },
        rebalance_loadavg_max_per_core: {
            type: 'number',
            title: 'Rebalance Loadavg Max Per Core',
            default: 1.5
        },
        rebalance_loadavg_penalty_weight: {
            type: 'number',
            title: 'Rebalance Loadavg Penalty Weight',
            default: 0.9
        },
        rebalance_disk_contention_warn_share: {
            type: 'number',
            title: 'Rebalance Disk Contention Warn Share',
            default: 0.7
        },
        rebalance_disk_contention_high_share: {
            type: 'number',
            title: 'Rebalance Disk Contention High Share',
            default: 0.9
        },
        rebalance_disk_penalty_weight: {
            type: 'number',
            title: 'Rebalance Disk Penalty Weight',
            default: 0.75
        },
        rebalance_search_max_relocations: {
            type: 'integer',
            title: 'Rebalance Search Max Relocations',
            default: 2
        },
        rebalance_search_depth: {
            type: 'integer',
            title: 'Rebalance Search Depth',
            default: 3
        },
        migration_worker_concurrency: {
            type: 'integer',
            title: 'Migration Worker Concurrency',
            default: 2
        },
        migration_job_claim_timeout_seconds: {
            type: 'integer',
            title: 'Migration Job Claim Timeout Seconds',
            default: 300
        },
        migration_retry_backoff_seconds: {
            type: 'integer',
            title: 'Migration Retry Backoff Seconds',
            default: 120
        },
        migration_lxc_live_enabled: {
            type: 'boolean',
            title: 'Migration Lxc Live Enabled',
            default: false
        },
        rebalance_cpu_peak_warn_share: {
            type: 'number',
            title: 'Rebalance Cpu Peak Warn Share',
            default: 0.7
        },
        rebalance_cpu_peak_high_share: {
            type: 'number',
            title: 'Rebalance Cpu Peak High Share',
            default: 1.2
        },
        rebalance_memory_peak_warn_share: {
            type: 'number',
            title: 'Rebalance Memory Peak Warn Share',
            default: 0.8
        },
        rebalance_memory_peak_high_share: {
            type: 'number',
            title: 'Rebalance Memory Peak High Share',
            default: 0.85
        },
        rebalance_resource_weight_cpu: {
            type: 'number',
            title: 'Rebalance Resource Weight Cpu',
            default: 1
        },
        rebalance_resource_weight_memory: {
            type: 'number',
            title: 'Rebalance Resource Weight Memory',
            default: 1
        },
        rebalance_resource_weight_disk: {
            type: 'number',
            title: 'Rebalance Resource Weight Disk',
            default: 1
        },
        scheduled_boot_batch_size: {
            type: 'integer',
            title: 'Scheduled Boot Batch Size',
            default: 5
        },
        scheduled_boot_batch_interval_seconds: {
            type: 'integer',
            title: 'Scheduled Boot Batch Interval Seconds',
            default: 10
        },
        scheduled_boot_lead_time_minutes: {
            type: 'integer',
            title: 'Scheduled Boot Lead Time Minutes',
            default: 5
        },
        window_grace_period_minutes: {
            type: 'integer',
            title: 'Window Grace Period Minutes',
            default: 30
        },
        practice_session_hours: {
            type: 'integer',
            title: 'Practice Session Hours',
            default: 3
        },
        practice_warning_minutes: {
            type: 'integer',
            title: 'Practice Warning Minutes',
            default: 30
        },
        updated_at: {
            anyOf: [
                {
                    type: 'string',
                    format: 'date-time'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Updated At'
        },
        is_configured: {
            type: 'boolean',
            title: 'Is Configured'
        },
        has_ca_cert: {
            type: 'boolean',
            title: 'Has Ca Cert'
        },
        ca_fingerprint: {
            anyOf: [
                {
                    type: 'string'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Ca Fingerprint'
        }
    },
    type: 'object',
    required: [
        'host',
        'user',
        'verify_ssl',
        'iso_storage',
        'data_storage',
        'api_timeout',
        'task_check_interval',
        'pool_name',
        'is_configured',
        'has_ca_cert'
    ],
    title: 'ProxmoxConfigPublic',
    description: '回傳給前端的 Proxmox 設定（不含密碼與憑證原文）'
} as const;

export const ProxmoxConfigUpdateSchema = {
    properties: {
        host: {
            type: 'string',
            title: 'Host'
        },
        user: {
            type: 'string',
            title: 'User'
        },
        password: {
            anyOf: [
                {
                    type: 'string'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Password'
        },
        verify_ssl: {
            type: 'boolean',
            title: 'Verify Ssl',
            default: false
        },
        iso_storage: {
            type: 'string',
            title: 'Iso Storage',
            default: 'local'
        },
        data_storage: {
            type: 'string',
            title: 'Data Storage',
            default: 'local-lvm'
        },
        api_timeout: {
            type: 'integer',
            maximum: 300,
            minimum: 1,
            title: 'Api Timeout',
            default: 30
        },
        task_check_interval: {
            type: 'integer',
            maximum: 60,
            minimum: 1,
            title: 'Task Check Interval',
            default: 2
        },
        pool_name: {
            type: 'string',
            title: 'Pool Name',
            default: 'SkyLab'
        },
        ca_cert: {
            anyOf: [
                {
                    type: 'string'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Ca Cert'
        },
        gateway_ip: {
            anyOf: [
                {
                    type: 'string'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Gateway Ip'
        },
        local_subnet: {
            anyOf: [
                {
                    type: 'string'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Local Subnet'
        },
        default_node: {
            anyOf: [
                {
                    type: 'string'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Default Node'
        },
        placement_strategy: {
            type: 'string',
            title: 'Placement Strategy',
            default: 'priority_dominant_share'
        },
        cpu_overcommit_ratio: {
            type: 'number',
            maximum: 8,
            minimum: 1,
            title: 'Cpu Overcommit Ratio',
            default: 2
        },
        disk_overcommit_ratio: {
            type: 'number',
            maximum: 5,
            minimum: 1,
            title: 'Disk Overcommit Ratio',
            default: 1
        },
        migration_enabled: {
            type: 'boolean',
            title: 'Migration Enabled',
            default: true
        },
        migration_max_per_rebalance: {
            type: 'integer',
            maximum: 20,
            minimum: 0,
            title: 'Migration Max Per Rebalance',
            default: 2
        },
        migration_min_interval_minutes: {
            type: 'integer',
            maximum: 10080,
            minimum: 0,
            title: 'Migration Min Interval Minutes',
            default: 60
        },
        migration_retry_limit: {
            type: 'integer',
            maximum: 10,
            minimum: 0,
            title: 'Migration Retry Limit',
            default: 3
        },
        rebalance_migration_cost: {
            type: 'number',
            maximum: 5,
            minimum: 0,
            title: 'Rebalance Migration Cost',
            default: 0.15
        },
        rebalance_peak_cpu_margin: {
            type: 'number',
            maximum: 2,
            minimum: 1,
            title: 'Rebalance Peak Cpu Margin',
            default: 1.1
        },
        rebalance_peak_memory_margin: {
            type: 'number',
            maximum: 2,
            minimum: 1,
            title: 'Rebalance Peak Memory Margin',
            default: 1.05
        },
        rebalance_loadavg_warn_per_core: {
            type: 'number',
            maximum: 4,
            minimum: 0,
            title: 'Rebalance Loadavg Warn Per Core',
            default: 0.8
        },
        rebalance_loadavg_max_per_core: {
            type: 'number',
            maximum: 8,
            minimum: 0.1,
            title: 'Rebalance Loadavg Max Per Core',
            default: 1.5
        },
        rebalance_loadavg_penalty_weight: {
            type: 'number',
            maximum: 5,
            minimum: 0,
            title: 'Rebalance Loadavg Penalty Weight',
            default: 0.9
        },
        rebalance_disk_contention_warn_share: {
            type: 'number',
            maximum: 1.5,
            minimum: 0,
            title: 'Rebalance Disk Contention Warn Share',
            default: 0.7
        },
        rebalance_disk_contention_high_share: {
            type: 'number',
            maximum: 2,
            minimum: 0.1,
            title: 'Rebalance Disk Contention High Share',
            default: 0.9
        },
        rebalance_disk_penalty_weight: {
            type: 'number',
            maximum: 5,
            minimum: 0,
            title: 'Rebalance Disk Penalty Weight',
            default: 0.75
        },
        rebalance_search_max_relocations: {
            type: 'integer',
            maximum: 10,
            minimum: 0,
            title: 'Rebalance Search Max Relocations',
            default: 2
        },
        rebalance_search_depth: {
            type: 'integer',
            maximum: 10,
            minimum: 0,
            title: 'Rebalance Search Depth',
            default: 3
        },
        migration_worker_concurrency: {
            type: 'integer',
            maximum: 20,
            minimum: 1,
            title: 'Migration Worker Concurrency',
            default: 2
        },
        migration_job_claim_timeout_seconds: {
            type: 'integer',
            maximum: 86400,
            minimum: 30,
            title: 'Migration Job Claim Timeout Seconds',
            default: 300
        },
        migration_retry_backoff_seconds: {
            type: 'integer',
            maximum: 86400,
            minimum: 0,
            title: 'Migration Retry Backoff Seconds',
            default: 120
        },
        migration_lxc_live_enabled: {
            type: 'boolean',
            title: 'Migration Lxc Live Enabled',
            default: false
        },
        rebalance_cpu_peak_warn_share: {
            type: 'number',
            maximum: 2,
            minimum: 0,
            title: 'Rebalance Cpu Peak Warn Share',
            default: 0.7
        },
        rebalance_cpu_peak_high_share: {
            type: 'number',
            maximum: 3,
            minimum: 0.1,
            title: 'Rebalance Cpu Peak High Share',
            default: 1.2
        },
        rebalance_memory_peak_warn_share: {
            type: 'number',
            maximum: 2,
            minimum: 0,
            title: 'Rebalance Memory Peak Warn Share',
            default: 0.8
        },
        rebalance_memory_peak_high_share: {
            type: 'number',
            maximum: 3,
            minimum: 0.1,
            title: 'Rebalance Memory Peak High Share',
            default: 0.85
        },
        rebalance_resource_weight_cpu: {
            type: 'number',
            maximum: 10,
            minimum: 0,
            title: 'Rebalance Resource Weight Cpu',
            default: 1
        },
        rebalance_resource_weight_memory: {
            type: 'number',
            maximum: 10,
            minimum: 0,
            title: 'Rebalance Resource Weight Memory',
            default: 1
        },
        rebalance_resource_weight_disk: {
            type: 'number',
            maximum: 10,
            minimum: 0,
            title: 'Rebalance Resource Weight Disk',
            default: 1
        },
        scheduled_boot_batch_size: {
            type: 'integer',
            maximum: 100,
            minimum: 1,
            title: 'Scheduled Boot Batch Size',
            default: 5
        },
        scheduled_boot_batch_interval_seconds: {
            type: 'integer',
            maximum: 600,
            minimum: 0,
            title: 'Scheduled Boot Batch Interval Seconds',
            default: 10
        },
        scheduled_boot_lead_time_minutes: {
            type: 'integer',
            maximum: 120,
            minimum: 0,
            title: 'Scheduled Boot Lead Time Minutes',
            default: 5
        },
        window_grace_period_minutes: {
            type: 'integer',
            maximum: 240,
            minimum: 0,
            title: 'Window Grace Period Minutes',
            default: 30
        },
        practice_session_hours: {
            type: 'integer',
            maximum: 24,
            minimum: 1,
            title: 'Practice Session Hours',
            default: 3
        },
        practice_warning_minutes: {
            type: 'integer',
            maximum: 120,
            minimum: 1,
            title: 'Practice Warning Minutes',
            default: 30
        }
    },
    type: 'object',
    required: [
        'host',
        'user'
    ],
    title: 'ProxmoxConfigUpdate',
    description: '更新 Proxmox 設定的請求 schema'
} as const;

export const ProxmoxConnectionTestResultSchema = {
    properties: {
        success: {
            type: 'boolean',
            title: 'Success'
        },
        message: {
            type: 'string',
            title: 'Message'
        }
    },
    type: 'object',
    required: [
        'success',
        'message'
    ],
    title: 'ProxmoxConnectionTestResult',
    description: '連線測試結果'
} as const;

export const ProxmoxNodePublicSchema = {
    properties: {
        id: {
            anyOf: [
                {
                    type: 'integer'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Id'
        },
        name: {
            type: 'string',
            title: 'Name'
        },
        host: {
            type: 'string',
            title: 'Host'
        },
        port: {
            type: 'integer',
            title: 'Port'
        },
        is_primary: {
            type: 'boolean',
            title: 'Is Primary'
        },
        is_online: {
            type: 'boolean',
            title: 'Is Online'
        },
        last_checked: {
            anyOf: [
                {
                    type: 'string',
                    format: 'date-time'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Last Checked'
        },
        priority: {
            type: 'integer',
            title: 'Priority',
            default: 5
        }
    },
    type: 'object',
    required: [
        'name',
        'host',
        'port',
        'is_primary',
        'is_online'
    ],
    title: 'ProxmoxNodePublic',
    description: '回傳給前端的節點資訊'
} as const;

export const ProxmoxNodeUpdateSchema = {
    properties: {
        host: {
            type: 'string',
            title: 'Host'
        },
        port: {
            type: 'integer',
            maximum: 65535,
            minimum: 1,
            title: 'Port',
            default: 8006
        },
        priority: {
            type: 'integer',
            maximum: 10,
            minimum: 1,
            title: 'Priority',
            default: 5
        }
    },
    type: 'object',
    required: [
        'host'
    ],
    title: 'ProxmoxNodeUpdate',
    description: '更新單一節點設定的請求 schema'
} as const;

export const ProxmoxStoragePublicSchema = {
    properties: {
        id: {
            type: 'integer',
            title: 'Id'
        },
        node_name: {
            type: 'string',
            title: 'Node Name'
        },
        storage: {
            type: 'string',
            title: 'Storage'
        },
        storage_type: {
            anyOf: [
                {
                    type: 'string'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Storage Type'
        },
        total_gb: {
            type: 'number',
            title: 'Total Gb'
        },
        used_gb: {
            type: 'number',
            title: 'Used Gb'
        },
        avail_gb: {
            type: 'number',
            title: 'Avail Gb'
        },
        can_vm: {
            type: 'boolean',
            title: 'Can Vm'
        },
        can_lxc: {
            type: 'boolean',
            title: 'Can Lxc'
        },
        can_iso: {
            type: 'boolean',
            title: 'Can Iso'
        },
        can_backup: {
            type: 'boolean',
            title: 'Can Backup'
        },
        is_shared: {
            type: 'boolean',
            title: 'Is Shared'
        },
        active: {
            type: 'boolean',
            title: 'Active'
        },
        enabled: {
            type: 'boolean',
            title: 'Enabled'
        },
        speed_tier: {
            type: 'string',
            title: 'Speed Tier'
        },
        user_priority: {
            type: 'integer',
            title: 'User Priority'
        }
    },
    type: 'object',
    required: [
        'id',
        'node_name',
        'storage',
        'total_gb',
        'used_gb',
        'avail_gb',
        'can_vm',
        'can_lxc',
        'can_iso',
        'can_backup',
        'is_shared',
        'active',
        'enabled',
        'speed_tier',
        'user_priority'
    ],
    title: 'ProxmoxStoragePublic',
    description: '回傳給前端的 Storage 資訊'
} as const;

export const ProxmoxStorageUpdateSchema = {
    properties: {
        enabled: {
            type: 'boolean',
            title: 'Enabled'
        },
        speed_tier: {
            type: 'string',
            pattern: '^(nvme|ssd|hdd|unknown)$',
            title: 'Speed Tier'
        },
        user_priority: {
            type: 'integer',
            maximum: 10,
            minimum: 1,
            title: 'User Priority'
        }
    },
    type: 'object',
    required: [
        'enabled',
        'speed_tier',
        'user_priority'
    ],
    title: 'ProxmoxStorageUpdate',
    description: '更新 Storage 使用者設定的請求 schema'
} as const;

export const RRDDataPointSchema = {
    properties: {
        time: {
            type: 'integer',
            title: 'Time',
            description: 'Timestamp'
        },
        cpu: {
            anyOf: [
                {
                    type: 'number'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Cpu'
        },
        maxcpu: {
            anyOf: [
                {
                    type: 'integer'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Maxcpu'
        },
        mem: {
            anyOf: [
                {
                    type: 'number'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Mem'
        },
        maxmem: {
            anyOf: [
                {
                    type: 'number'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Maxmem'
        },
        disk: {
            anyOf: [
                {
                    type: 'number'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Disk'
        },
        maxdisk: {
            anyOf: [
                {
                    type: 'number'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Maxdisk'
        },
        netin: {
            anyOf: [
                {
                    type: 'number'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Netin'
        },
        netout: {
            anyOf: [
                {
                    type: 'number'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Netout'
        }
    },
    type: 'object',
    required: [
        'time'
    ],
    title: 'RRDDataPoint',
    description: 'RRD 數據點'
} as const;

export const RRDDataResponseSchema = {
    properties: {
        timeframe: {
            type: 'string',
            title: 'Timeframe',
            description: 'Time range'
        },
        data: {
            items: {
                $ref: '#/components/schemas/RRDDataPoint'
            },
            type: 'array',
            title: 'Data',
            description: 'Data points'
        }
    },
    type: 'object',
    required: [
        'timeframe',
        'data'
    ],
    title: 'RRDDataResponse',
    description: 'RRD 歷史數據'
} as const;

export const RateLimitStatusResponseSchema = {
    properties: {
        limit_per_minute: {
            type: 'integer',
            title: 'Limit Per Minute'
        },
        current_usage: {
            type: 'integer',
            title: 'Current Usage'
        },
        remaining: {
            type: 'integer',
            title: 'Remaining'
        },
        reset_at: {
            type: 'string',
            format: 'date-time',
            title: 'Reset At'
        },
        disabled: {
            type: 'boolean',
            title: 'Disabled',
            default: false
        },
        error: {
            anyOf: [
                {
                    type: 'string'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Error'
        }
    },
    type: 'object',
    required: [
        'limit_per_minute',
        'current_usage',
        'remaining',
        'reset_at'
    ],
    title: 'RateLimitStatusResponse',
    description: '速率限制状态'
} as const;

export const RecommendationFormContextSchema = {
    properties: {
        resource_type: {
            anyOf: [
                {
                    type: 'string',
                    enum: [
                        'lxc',
                        'vm'
                    ]
                },
                {
                    type: 'null'
                }
            ],
            title: 'Resource Type'
        },
        mode: {
            anyOf: [
                {
                    type: 'string',
                    enum: [
                        'immediate',
                        'scheduled'
                    ]
                },
                {
                    type: 'null'
                }
            ],
            title: 'Mode'
        },
        start_at: {
            anyOf: [
                {
                    type: 'string',
                    format: 'date-time'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Start At'
        },
        end_at: {
            anyOf: [
                {
                    type: 'string',
                    format: 'date-time'
                },
                {
                    type: 'null'
                }
            ],
            title: 'End At'
        },
        selected_gpu_mapping_id: {
            anyOf: [
                {
                    type: 'string'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Selected Gpu Mapping Id'
        },
        gpu_options: {
            items: {
                $ref: '#/components/schemas/GPUOptionContext'
            },
            type: 'array',
            title: 'Gpu Options'
        }
    },
    type: 'object',
    title: 'RecommendationFormContext'
} as const;

export const RecommendedMachineSchema = {
    properties: {
        node: {
            type: 'string',
            title: 'Node'
        },
        resource_type: {
            type: 'string',
            enum: [
                'lxc',
                'vm'
            ],
            title: 'Resource Type'
        },
        instance_count: {
            type: 'integer',
            minimum: 0,
            title: 'Instance Count',
            default: 0
        },
        reason: {
            type: 'string',
            title: 'Reason'
        }
    },
    type: 'object',
    required: [
        'node',
        'resource_type',
        'reason'
    ],
    title: 'RecommendedMachine'
} as const;

export const RecurrencePreviewSchema = {
    properties: {
        windows: {
            items: {
                prefixItems: [
                    {
                        type: 'string',
                        format: 'date-time'
                    },
                    {
                        type: 'string',
                        format: 'date-time'
                    }
                ],
                type: 'array',
                maxItems: 2,
                minItems: 2
            },
            type: 'array',
            title: 'Windows'
        }
    },
    type: 'object',
    required: [
        'windows'
    ],
    title: 'RecurrencePreview',
    description: 'Next few computed windows for a candidate RRULE — used by the review UI\nto confirm the schedule does what the teacher intended before approving.'
} as const;

export const RefreshTokenRequestSchema = {
    properties: {
        refresh_token: {
            type: 'string',
            title: 'Refresh Token'
        }
    },
    type: 'object',
    required: [
        'refresh_token'
    ],
    title: 'RefreshTokenRequest'
} as const;

export const ResourceConfigSchema = {
    properties: {
        vmid: {
            type: 'integer',
            title: 'Vmid'
        },
        node: {
            type: 'string',
            title: 'Node'
        },
        resource_type: {
            type: 'string',
            title: 'Resource Type'
        },
        name: {
            anyOf: [
                {
                    type: 'string'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Name'
        },
        cpu_cores: {
            anyOf: [
                {
                    type: 'integer'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Cpu Cores'
        },
        cpu_type: {
            anyOf: [
                {
                    type: 'string'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Cpu Type'
        },
        memory_mb: {
            anyOf: [
                {
                    type: 'integer'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Memory Mb'
        },
        disk_info: {
            anyOf: [
                {
                    type: 'string'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Disk Info'
        },
        disk_size_gb: {
            anyOf: [
                {
                    type: 'integer'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Disk Size Gb'
        },
        os_type: {
            anyOf: [
                {
                    type: 'string'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Os Type'
        },
        net0: {
            anyOf: [
                {
                    type: 'string'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Net0'
        },
        description: {
            anyOf: [
                {
                    type: 'string'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Description'
        },
        tags: {
            anyOf: [
                {
                    type: 'string'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Tags'
        },
        onboot: {
            type: 'boolean',
            title: 'Onboot',
            default: false
        },
        protection: {
            type: 'boolean',
            title: 'Protection',
            default: false
        },
        raw: {
            additionalProperties: true,
            type: 'object',
            title: 'Raw'
        }
    },
    type: 'object',
    required: [
        'vmid',
        'node',
        'resource_type'
    ],
    title: 'ResourceConfig'
} as const;

export const ResourcePublicSchema = {
    properties: {
        vmid: {
            anyOf: [
                {
                    type: 'integer'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Vmid'
        },
        request_id: {
            anyOf: [
                {
                    type: 'string',
                    format: 'uuid'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Request Id'
        },
        name: {
            type: 'string',
            title: 'Name'
        },
        status: {
            type: 'string',
            title: 'Status'
        },
        node: {
            type: 'string',
            title: 'Node'
        },
        type: {
            type: 'string',
            title: 'Type'
        },
        is_placeholder: {
            type: 'boolean',
            title: 'Is Placeholder',
            default: false
        },
        can_control: {
            type: 'boolean',
            title: 'Can Control',
            default: true
        },
        environment_type: {
            anyOf: [
                {
                    type: 'string'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Environment Type'
        },
        os_info: {
            anyOf: [
                {
                    type: 'string'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Os Info'
        },
        expiry_date: {
            anyOf: [
                {
                    type: 'string',
                    format: 'date'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Expiry Date'
        },
        ip_address: {
            anyOf: [
                {
                    type: 'string'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Ip Address'
        },
        ssh_public_key: {
            anyOf: [
                {
                    type: 'string'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Ssh Public Key'
        },
        service_template_slug: {
            anyOf: [
                {
                    type: 'string'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Service Template Slug'
        },
        cpu: {
            anyOf: [
                {
                    type: 'number'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Cpu'
        },
        maxcpu: {
            anyOf: [
                {
                    type: 'integer'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Maxcpu'
        },
        mem: {
            anyOf: [
                {
                    type: 'integer'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Mem'
        },
        maxmem: {
            anyOf: [
                {
                    type: 'integer'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Maxmem'
        },
        uptime: {
            anyOf: [
                {
                    type: 'integer'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Uptime'
        },
        auto_stop_at: {
            anyOf: [
                {
                    type: 'string',
                    format: 'date-time'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Auto Stop At'
        },
        auto_stop_reason: {
            anyOf: [
                {
                    type: 'string',
                    enum: [
                        'window_grace',
                        'practice_quota'
                    ]
                },
                {
                    type: 'null'
                }
            ],
            title: 'Auto Stop Reason'
        }
    },
    type: 'object',
    required: [
        'vmid',
        'name',
        'status',
        'node',
        'type'
    ],
    title: 'ResourcePublic',
    description: '公開的資源資訊（合併 Proxmox + DB）'
} as const;

export const ResourceStatusSchema = {
    properties: {
        vmid: {
            type: 'integer',
            title: 'Vmid'
        },
        node: {
            type: 'string',
            title: 'Node'
        },
        resource_type: {
            type: 'string',
            title: 'Resource Type'
        },
        status: {
            type: 'string',
            title: 'Status'
        },
        cpu_usage: {
            type: 'number',
            title: 'Cpu Usage'
        },
        cpu_cores: {
            type: 'integer',
            title: 'Cpu Cores'
        },
        mem_used_bytes: {
            type: 'integer',
            title: 'Mem Used Bytes'
        },
        mem_total_bytes: {
            type: 'integer',
            title: 'Mem Total Bytes'
        },
        mem_used_pct: {
            type: 'number',
            title: 'Mem Used Pct'
        },
        disk_read_bytes: {
            type: 'integer',
            title: 'Disk Read Bytes'
        },
        disk_write_bytes: {
            type: 'integer',
            title: 'Disk Write Bytes'
        },
        disk_total_bytes: {
            type: 'integer',
            title: 'Disk Total Bytes'
        },
        net_in_bytes: {
            type: 'integer',
            title: 'Net In Bytes'
        },
        net_out_bytes: {
            type: 'integer',
            title: 'Net Out Bytes'
        },
        uptime_seconds: {
            anyOf: [
                {
                    type: 'integer'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Uptime Seconds'
        },
        pid: {
            anyOf: [
                {
                    type: 'integer'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Pid'
        }
    },
    type: 'object',
    required: [
        'vmid',
        'node',
        'resource_type',
        'status',
        'cpu_usage',
        'cpu_cores',
        'mem_used_bytes',
        'mem_total_bytes',
        'mem_used_pct',
        'disk_read_bytes',
        'disk_write_bytes',
        'disk_total_bytes',
        'net_in_bytes',
        'net_out_bytes'
    ],
    title: 'ResourceStatus'
} as const;

export const ResourceSummarySchema = {
    properties: {
        vmid: {
            type: 'integer',
            title: 'Vmid'
        },
        name: {
            type: 'string',
            title: 'Name'
        },
        resource_type: {
            type: 'string',
            title: 'Resource Type'
        },
        node: {
            type: 'string',
            title: 'Node'
        },
        status: {
            type: 'string',
            title: 'Status'
        },
        pool: {
            anyOf: [
                {
                    type: 'string'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Pool'
        },
        cpu_usage: {
            type: 'number',
            title: 'Cpu Usage'
        },
        cpu_cores: {
            type: 'integer',
            title: 'Cpu Cores'
        },
        mem_used_bytes: {
            type: 'integer',
            title: 'Mem Used Bytes'
        },
        mem_total_bytes: {
            type: 'integer',
            title: 'Mem Total Bytes'
        },
        mem_used_pct: {
            type: 'number',
            title: 'Mem Used Pct'
        },
        disk_used_bytes: {
            type: 'integer',
            title: 'Disk Used Bytes'
        },
        disk_total_bytes: {
            type: 'integer',
            title: 'Disk Total Bytes'
        },
        disk_used_pct: {
            type: 'number',
            title: 'Disk Used Pct'
        },
        net_in_bytes: {
            type: 'integer',
            title: 'Net In Bytes'
        },
        net_out_bytes: {
            type: 'integer',
            title: 'Net Out Bytes'
        },
        uptime_seconds: {
            anyOf: [
                {
                    type: 'integer'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Uptime Seconds'
        },
        is_template: {
            type: 'boolean',
            title: 'Is Template'
        }
    },
    type: 'object',
    required: [
        'vmid',
        'name',
        'resource_type',
        'node',
        'status',
        'cpu_usage',
        'cpu_cores',
        'mem_used_bytes',
        'mem_total_bytes',
        'mem_used_pct',
        'disk_used_bytes',
        'disk_total_bytes',
        'disk_used_pct',
        'net_in_bytes',
        'net_out_bytes',
        'is_template'
    ],
    title: 'ResourceSummary'
} as const;

export const ReverseProxyRuleCreateSchema = {
    properties: {
        vmid: {
            type: 'integer',
            exclusiveMinimum: 0,
            title: 'Vmid'
        },
        zone_id: {
            type: 'string',
            maxLength: 64,
            minLength: 1,
            title: 'Zone Id'
        },
        hostname_prefix: {
            type: 'string',
            maxLength: 190,
            title: 'Hostname Prefix',
            default: ''
        },
        internal_port: {
            type: 'integer',
            maximum: 65535,
            minimum: 1,
            title: 'Internal Port'
        },
        enable_https: {
            type: 'boolean',
            title: 'Enable Https',
            default: true
        }
    },
    type: 'object',
    required: [
        'vmid',
        'zone_id',
        'internal_port'
    ],
    title: 'ReverseProxyRuleCreate'
} as const;

export const ReverseProxyRulePublicSchema = {
    properties: {
        id: {
            type: 'string',
            format: 'uuid',
            title: 'Id'
        },
        vmid: {
            type: 'integer',
            title: 'Vmid'
        },
        vm_ip: {
            type: 'string',
            title: 'Vm Ip'
        },
        domain: {
            type: 'string',
            title: 'Domain'
        },
        zone_id: {
            anyOf: [
                {
                    type: 'string'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Zone Id'
        },
        internal_port: {
            type: 'integer',
            title: 'Internal Port'
        },
        enable_https: {
            type: 'boolean',
            title: 'Enable Https'
        },
        dns_provider: {
            type: 'string',
            title: 'Dns Provider'
        },
        created_at: {
            type: 'string',
            format: 'date-time',
            title: 'Created At'
        }
    },
    type: 'object',
    required: [
        'id',
        'vmid',
        'vm_ip',
        'domain',
        'internal_port',
        'enable_https',
        'dns_provider',
        'created_at'
    ],
    title: 'ReverseProxyRulePublic',
    description: '反向代理規則（回應）'
} as const;

export const ReverseProxyRuleUpdateSchema = {
    properties: {
        vmid: {
            type: 'integer',
            exclusiveMinimum: 0,
            title: 'Vmid'
        },
        zone_id: {
            type: 'string',
            maxLength: 64,
            minLength: 1,
            title: 'Zone Id'
        },
        hostname_prefix: {
            type: 'string',
            maxLength: 190,
            title: 'Hostname Prefix',
            default: ''
        },
        internal_port: {
            type: 'integer',
            maximum: 65535,
            minimum: 1,
            title: 'Internal Port'
        },
        enable_https: {
            type: 'boolean',
            title: 'Enable Https',
            default: true
        }
    },
    type: 'object',
    required: [
        'vmid',
        'zone_id',
        'internal_port'
    ],
    title: 'ReverseProxyRuleUpdate'
} as const;

export const ReverseProxyRuntimeSectionSchema = {
    properties: {
        routers: {
            items: {
                additionalProperties: true,
                type: 'object'
            },
            type: 'array',
            title: 'Routers'
        },
        services: {
            items: {
                additionalProperties: true,
                type: 'object'
            },
            type: 'array',
            title: 'Services'
        },
        middlewares: {
            items: {
                additionalProperties: true,
                type: 'object'
            },
            type: 'array',
            title: 'Middlewares'
        }
    },
    type: 'object',
    title: 'ReverseProxyRuntimeSection'
} as const;

export const ReverseProxyRuntimeSnapshotSchema = {
    properties: {
        runtime_error: {
            anyOf: [
                {
                    type: 'string'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Runtime Error'
        },
        version: {
            anyOf: [
                {
                    additionalProperties: true,
                    type: 'object'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Version'
        },
        overview: {
            anyOf: [
                {
                    additionalProperties: true,
                    type: 'object'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Overview'
        },
        entrypoints: {
            items: {
                additionalProperties: true,
                type: 'object'
            },
            type: 'array',
            title: 'Entrypoints'
        },
        http: {
            $ref: '#/components/schemas/ReverseProxyRuntimeSection'
        },
        tcp: {
            $ref: '#/components/schemas/ReverseProxyRuntimeSection'
        },
        udp: {
            $ref: '#/components/schemas/ReverseProxyRuntimeSection'
        }
    },
    type: 'object',
    title: 'ReverseProxyRuntimeSnapshot'
} as const;

export const ReverseProxySetupContextSchema = {
    properties: {
        enabled: {
            type: 'boolean',
            title: 'Enabled'
        },
        gateway_ready: {
            type: 'boolean',
            title: 'Gateway Ready'
        },
        cloudflare_ready: {
            type: 'boolean',
            title: 'Cloudflare Ready'
        },
        reasons: {
            items: {
                type: 'string'
            },
            type: 'array',
            title: 'Reasons'
        },
        zones: {
            items: {
                $ref: '#/components/schemas/ReverseProxyZoneOption'
            },
            type: 'array',
            title: 'Zones'
        },
        default_dns_target_type: {
            anyOf: [
                {
                    type: 'string'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Default Dns Target Type'
        },
        default_dns_target_value: {
            anyOf: [
                {
                    type: 'string'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Default Dns Target Value'
        }
    },
    type: 'object',
    required: [
        'enabled',
        'gateway_ready',
        'cloudflare_ready'
    ],
    title: 'ReverseProxySetupContext'
} as const;

export const ReverseProxyZoneOptionSchema = {
    properties: {
        id: {
            type: 'string',
            title: 'Id'
        },
        name: {
            type: 'string',
            title: 'Name'
        }
    },
    type: 'object',
    required: [
        'id',
        'name'
    ],
    title: 'ReverseProxyZoneOption'
} as const;

export const SSHConfirmRequestSchema = {
    properties: {
        token: {
            type: 'string',
            title: 'Token',
            description: '執行時取得的 confirm_token'
        },
        confirm_token: {
            anyOf: [
                {
                    type: 'string'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Confirm Token',
            description: '相容欄位：可改以 confirm_token 傳入'
        },
        approved: {
            type: 'boolean',
            title: 'Approved',
            description: '是否允許執行'
        },
        command: {
            anyOf: [
                {
                    type: 'string'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Command',
            description: '可選：允許前覆寫要執行的指令內容'
        }
    },
    type: 'object',
    required: [
        'token',
        'approved'
    ],
    title: 'SSHConfirmRequest'
} as const;

export const SSHExecRequestSchema = {
    properties: {
        vmid: {
            type: 'integer',
            title: 'Vmid',
            description: '目標 VM 或 LXC 的 ID'
        },
        command: {
            type: 'string',
            minLength: 1,
            title: 'Command',
            description: '要執行的指令內容'
        },
        ssh_user: {
            type: 'string',
            title: 'Ssh User',
            description: 'SSH 登入帳號（預設 root）',
            default: 'root'
        },
        ssh_port: {
            type: 'integer',
            maximum: 65535,
            minimum: 1,
            title: 'Ssh Port',
            description: 'SSH 埠號（預設 22）',
            default: 22
        },
        require_confirm: {
            type: 'boolean',
            title: 'Require Confirm',
            description: '是否需要等待使用者確認（回傳 pending 狀態）',
            default: false
        }
    },
    type: 'object',
    required: [
        'vmid',
        'command'
    ],
    title: 'SSHExecRequest'
} as const;

export const SSHExecResultSchema = {
    properties: {
        vmid: {
            type: 'integer',
            title: 'Vmid',
            default: 0
        },
        host: {
            type: 'string',
            title: 'Host',
            default: ''
        },
        ssh_user: {
            type: 'string',
            title: 'Ssh User',
            default: ''
        },
        command: {
            type: 'string',
            title: 'Command',
            default: ''
        },
        stdout: {
            type: 'string',
            title: 'Stdout',
            default: ''
        },
        stderr: {
            type: 'string',
            title: 'Stderr',
            default: ''
        },
        exit_code: {
            type: 'integer',
            title: 'Exit Code',
            default: 0
        },
        error: {
            anyOf: [
                {
                    type: 'string'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Error'
        },
        blocked: {
            type: 'boolean',
            title: 'Blocked',
            description: '是否因安全黑名單被攔截',
            default: false
        },
        block_reason: {
            anyOf: [
                {
                    type: 'string'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Block Reason'
        },
        pending: {
            type: 'boolean',
            title: 'Pending',
            description: '是否在等待使用者確認',
            default: false
        },
        confirm_token: {
            anyOf: [
                {
                    type: 'string'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Confirm Token',
            description: '等待確認時的 Token（TTL 5 分鐘）'
        }
    },
    type: 'object',
    title: 'SSHExecResult'
} as const;

export const SSHKeyResponseSchema = {
    properties: {
        vmid: {
            type: 'integer',
            title: 'Vmid'
        },
        ssh_public_key: {
            anyOf: [
                {
                    type: 'string'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Ssh Public Key'
        },
        ssh_private_key: {
            anyOf: [
                {
                    type: 'string'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Ssh Private Key'
        }
    },
    type: 'object',
    required: [
        'vmid'
    ],
    title: 'SSHKeyResponse',
    description: 'SSH 金鑰回應'
} as const;

export const ScriptDeployLogDetailSchema = {
    properties: {
        id: {
            type: 'string',
            format: 'uuid',
            title: 'Id'
        },
        task_id: {
            type: 'string',
            title: 'Task Id'
        },
        user_id: {
            anyOf: [
                {
                    type: 'string',
                    format: 'uuid'
                },
                {
                    type: 'null'
                }
            ],
            title: 'User Id'
        },
        vmid: {
            anyOf: [
                {
                    type: 'integer'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Vmid'
        },
        template_slug: {
            type: 'string',
            title: 'Template Slug'
        },
        template_name: {
            anyOf: [
                {
                    type: 'string'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Template Name'
        },
        hostname: {
            anyOf: [
                {
                    type: 'string'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Hostname'
        },
        status: {
            type: 'string',
            title: 'Status'
        },
        progress: {
            anyOf: [
                {
                    type: 'string'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Progress'
        },
        message: {
            anyOf: [
                {
                    type: 'string'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Message'
        },
        created_at: {
            type: 'string',
            format: 'date-time',
            title: 'Created At'
        },
        updated_at: {
            type: 'string',
            format: 'date-time',
            title: 'Updated At'
        },
        completed_at: {
            anyOf: [
                {
                    type: 'string',
                    format: 'date-time'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Completed At'
        },
        script_path: {
            anyOf: [
                {
                    type: 'string'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Script Path'
        },
        error: {
            anyOf: [
                {
                    type: 'string'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Error'
        },
        output: {
            anyOf: [
                {
                    type: 'string'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Output'
        }
    },
    type: 'object',
    required: [
        'id',
        'task_id',
        'template_slug',
        'status',
        'created_at',
        'updated_at'
    ],
    title: 'ScriptDeployLogDetail',
    description: '部署日誌詳細內容（含完整 output 與 error）'
} as const;

export const ScriptDeployLogListSchema = {
    properties: {
        items: {
            items: {
                $ref: '#/components/schemas/ScriptDeployLogListItem'
            },
            type: 'array',
            title: 'Items'
        },
        total: {
            type: 'integer',
            title: 'Total'
        },
        limit: {
            type: 'integer',
            title: 'Limit'
        },
        offset: {
            type: 'integer',
            title: 'Offset'
        }
    },
    type: 'object',
    required: [
        'items',
        'total',
        'limit',
        'offset'
    ],
    title: 'ScriptDeployLogList',
    description: '部署日誌列表回應'
} as const;

export const ScriptDeployLogListItemSchema = {
    properties: {
        id: {
            type: 'string',
            format: 'uuid',
            title: 'Id'
        },
        task_id: {
            type: 'string',
            title: 'Task Id'
        },
        user_id: {
            anyOf: [
                {
                    type: 'string',
                    format: 'uuid'
                },
                {
                    type: 'null'
                }
            ],
            title: 'User Id'
        },
        vmid: {
            anyOf: [
                {
                    type: 'integer'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Vmid'
        },
        template_slug: {
            type: 'string',
            title: 'Template Slug'
        },
        template_name: {
            anyOf: [
                {
                    type: 'string'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Template Name'
        },
        hostname: {
            anyOf: [
                {
                    type: 'string'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Hostname'
        },
        status: {
            type: 'string',
            title: 'Status'
        },
        progress: {
            anyOf: [
                {
                    type: 'string'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Progress'
        },
        message: {
            anyOf: [
                {
                    type: 'string'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Message'
        },
        created_at: {
            type: 'string',
            format: 'date-time',
            title: 'Created At'
        },
        updated_at: {
            type: 'string',
            format: 'date-time',
            title: 'Updated At'
        },
        completed_at: {
            anyOf: [
                {
                    type: 'string',
                    format: 'date-time'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Completed At'
        }
    },
    type: 'object',
    required: [
        'id',
        'task_id',
        'template_slug',
        'status',
        'created_at',
        'updated_at'
    ],
    title: 'ScriptDeployLogListItem',
    description: '部署日誌列表項'
} as const;

export const ScriptDeployRequestSchema = {
    properties: {
        template_slug: {
            type: 'string',
            title: 'Template Slug',
            description: '模板 slug（如 docker）'
        },
        script_path: {
            type: 'string',
            title: 'Script Path',
            description: '腳本路徑（如 ct/docker.sh）'
        },
        hostname: {
            type: 'string',
            maxLength: 63,
            pattern: '^[a-z0-9]([a-z0-9-]*[a-z0-9])?$',
            title: 'Hostname'
        },
        password: {
            type: 'string',
            maxLength: 128,
            minLength: 5,
            title: 'Password'
        },
        cpu: {
            type: 'integer',
            maximum: 16,
            minimum: 1,
            title: 'Cpu',
            default: 2
        },
        ram: {
            type: 'integer',
            maximum: 65536,
            minimum: 256,
            title: 'Ram',
            default: 2048
        },
        disk: {
            type: 'integer',
            maximum: 500,
            minimum: 1,
            title: 'Disk',
            default: 4
        },
        unprivileged: {
            type: 'boolean',
            title: 'Unprivileged',
            default: true
        },
        ssh: {
            type: 'boolean',
            title: 'Ssh',
            default: false
        },
        environment_type: {
            type: 'string',
            title: 'Environment Type',
            default: '服務模板'
        },
        os_info: {
            anyOf: [
                {
                    type: 'string'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Os Info'
        },
        expiry_date: {
            anyOf: [
                {
                    type: 'string',
                    format: 'date'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Expiry Date'
        }
    },
    type: 'object',
    required: [
        'template_slug',
        'script_path',
        'hostname',
        'password'
    ],
    title: 'ScriptDeployRequest',
    description: '服務模板無人值守部署請求'
} as const;

export const ScriptDeployResponseSchema = {
    properties: {
        task_id: {
            type: 'string',
            title: 'Task Id'
        },
        message: {
            type: 'string',
            title: 'Message'
        }
    },
    type: 'object',
    required: [
        'task_id',
        'message'
    ],
    title: 'ScriptDeployResponse',
    description: '部署任務建立回應'
} as const;

export const ScriptDeployStatusSchema = {
    properties: {
        task_id: {
            type: 'string',
            title: 'Task Id'
        },
        status: {
            type: 'string',
            title: 'Status'
        },
        progress: {
            anyOf: [
                {
                    type: 'string'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Progress'
        },
        vmid: {
            anyOf: [
                {
                    type: 'integer'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Vmid'
        },
        message: {
            anyOf: [
                {
                    type: 'string'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Message'
        },
        error: {
            anyOf: [
                {
                    type: 'string'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Error'
        },
        output: {
            anyOf: [
                {
                    type: 'string'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Output'
        }
    },
    type: 'object',
    required: [
        'task_id',
        'status'
    ],
    title: 'ScriptDeployStatus',
    description: '部署任務狀態'
} as const;

export const ServiceActionResultSchema = {
    properties: {
        service: {
            type: 'string',
            title: 'Service'
        },
        action: {
            type: 'string',
            title: 'Action'
        },
        success: {
            type: 'boolean',
            title: 'Success'
        },
        output: {
            type: 'string',
            title: 'Output'
        }
    },
    type: 'object',
    required: [
        'service',
        'action',
        'success',
        'output'
    ],
    title: 'ServiceActionResult'
} as const;

export const ServiceConfigReadSchema = {
    properties: {
        service: {
            type: 'string',
            title: 'Service'
        },
        content: {
            type: 'string',
            title: 'Content'
        }
    },
    type: 'object',
    required: [
        'service',
        'content'
    ],
    title: 'ServiceConfigRead'
} as const;

export const ServiceConfigWriteSchema = {
    properties: {
        content: {
            type: 'string',
            title: 'Content',
            description: '設定檔內容'
        }
    },
    type: 'object',
    required: [
        'content'
    ],
    title: 'ServiceConfigWrite'
} as const;

export const ServiceStatusResultSchema = {
    properties: {
        service: {
            type: 'string',
            title: 'Service'
        },
        active: {
            type: 'boolean',
            title: 'Active'
        },
        status_text: {
            type: 'string',
            title: 'Status Text'
        }
    },
    type: 'object',
    required: [
        'service',
        'active',
        'status_text'
    ],
    title: 'ServiceStatusResult'
} as const;

export const SessionStatusResponseSchema = {
    properties: {
        vmid: {
            type: 'integer',
            title: 'Vmid'
        },
        running: {
            type: 'boolean',
            title: 'Running'
        },
        auto_stop_at: {
            anyOf: [
                {
                    type: 'string',
                    format: 'date-time'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Auto Stop At'
        },
        auto_stop_reason: {
            anyOf: [
                {
                    type: 'string',
                    enum: [
                        'window_grace',
                        'practice_quota'
                    ]
                },
                {
                    type: 'null'
                }
            ],
            title: 'Auto Stop Reason'
        },
        minutes_until_stop: {
            anyOf: [
                {
                    type: 'integer'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Minutes Until Stop'
        },
        expiry_at: {
            anyOf: [
                {
                    type: 'string',
                    format: 'date-time'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Expiry At'
        },
        hours_until_expiry: {
            anyOf: [
                {
                    type: 'integer'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Hours Until Expiry'
        },
        should_warn: {
            type: 'boolean',
            title: 'Should Warn',
            default: false
        },
        warn_reason: {
            anyOf: [
                {
                    type: 'string',
                    enum: [
                        'auto_stop',
                        'expiry'
                    ]
                },
                {
                    type: 'null'
                }
            ],
            title: 'Warn Reason'
        },
        can_extend: {
            type: 'boolean',
            title: 'Can Extend',
            default: false
        }
    },
    type: 'object',
    required: [
        'vmid',
        'running'
    ],
    title: 'SessionStatusResponse',
    description: 'Live status of a VM\'s auto-stop / expiry warnings, polled by the\nstudent UI. ``warn_reason`` distinguishes the two cases so the dialog can\nshow appropriate copy and actions.'
} as const;

export const SnapshotCreateRequestSchema = {
    properties: {
        snapname: {
            type: 'string',
            maxLength: 40,
            minLength: 1,
            title: 'Snapname',
            description: 'Snapshot name'
        },
        description: {
            anyOf: [
                {
                    type: 'string',
                    maxLength: 255
                },
                {
                    type: 'null'
                }
            ],
            title: 'Description',
            description: 'Snapshot description'
        },
        vmstate: {
            type: 'boolean',
            title: 'Vmstate',
            description: 'Include RAM state (VM only)',
            default: false
        }
    },
    type: 'object',
    required: [
        'snapname'
    ],
    title: 'SnapshotCreateRequest',
    description: '建立快照'
} as const;

export const SnapshotInfoSchema = {
    properties: {
        name: {
            type: 'string',
            title: 'Name',
            description: 'Snapshot name'
        },
        description: {
            anyOf: [
                {
                    type: 'string'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Description',
            description: 'Snapshot description'
        },
        snaptime: {
            anyOf: [
                {
                    type: 'integer'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Snaptime',
            description: 'Creation timestamp'
        },
        vmstate: {
            anyOf: [
                {
                    type: 'integer'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Vmstate',
            description: 'Includes VM state (0/1)'
        }
    },
    type: 'object',
    required: [
        'name'
    ],
    title: 'SnapshotInfo',
    description: '快照資訊'
} as const;

export const SnapshotResponseSchema = {
    properties: {
        message: {
            type: 'string',
            title: 'Message'
        },
        task_id: {
            anyOf: [
                {
                    type: 'string'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Task Id'
        }
    },
    type: 'object',
    required: [
        'message'
    ],
    title: 'SnapshotResponse',
    description: '快照操作回應'
} as const;

export const SpecChangeRequestCreateSchema = {
    properties: {
        vmid: {
            type: 'integer',
            title: 'Vmid'
        },
        change_type: {
            $ref: '#/components/schemas/SpecChangeType'
        },
        reason: {
            type: 'string',
            minLength: 10,
            title: 'Reason',
            description: '調整原因至少10字'
        },
        requested_cpu: {
            anyOf: [
                {
                    type: 'integer',
                    maximum: 32,
                    minimum: 1
                },
                {
                    type: 'null'
                }
            ],
            title: 'Requested Cpu'
        },
        requested_memory: {
            anyOf: [
                {
                    type: 'integer',
                    maximum: 65536,
                    minimum: 512
                },
                {
                    type: 'null'
                }
            ],
            title: 'Requested Memory'
        },
        requested_disk: {
            anyOf: [
                {
                    type: 'integer',
                    maximum: 1000,
                    minimum: 1
                },
                {
                    type: 'null'
                }
            ],
            title: 'Requested Disk'
        }
    },
    type: 'object',
    required: [
        'vmid',
        'change_type',
        'reason'
    ],
    title: 'SpecChangeRequestCreate',
    description: '建立規格調整申請'
} as const;

export const SpecChangeRequestPublicSchema = {
    properties: {
        id: {
            type: 'string',
            format: 'uuid',
            title: 'Id'
        },
        vmid: {
            type: 'integer',
            title: 'Vmid'
        },
        user_id: {
            type: 'string',
            format: 'uuid',
            title: 'User Id'
        },
        user_email: {
            anyOf: [
                {
                    type: 'string'
                },
                {
                    type: 'null'
                }
            ],
            title: 'User Email'
        },
        user_full_name: {
            anyOf: [
                {
                    type: 'string'
                },
                {
                    type: 'null'
                }
            ],
            title: 'User Full Name'
        },
        change_type: {
            $ref: '#/components/schemas/SpecChangeType'
        },
        reason: {
            type: 'string',
            title: 'Reason'
        },
        current_cpu: {
            anyOf: [
                {
                    type: 'integer'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Current Cpu'
        },
        current_memory: {
            anyOf: [
                {
                    type: 'integer'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Current Memory'
        },
        current_disk: {
            anyOf: [
                {
                    type: 'integer'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Current Disk'
        },
        requested_cpu: {
            anyOf: [
                {
                    type: 'integer'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Requested Cpu'
        },
        requested_memory: {
            anyOf: [
                {
                    type: 'integer'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Requested Memory'
        },
        requested_disk: {
            anyOf: [
                {
                    type: 'integer'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Requested Disk'
        },
        status: {
            $ref: '#/components/schemas/SpecChangeRequestStatus'
        },
        reviewer_id: {
            anyOf: [
                {
                    type: 'string',
                    format: 'uuid'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Reviewer Id'
        },
        review_comment: {
            anyOf: [
                {
                    type: 'string'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Review Comment'
        },
        reviewed_at: {
            anyOf: [
                {
                    type: 'string',
                    format: 'date-time'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Reviewed At'
        },
        applied_at: {
            anyOf: [
                {
                    type: 'string',
                    format: 'date-time'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Applied At'
        },
        created_at: {
            type: 'string',
            format: 'date-time',
            title: 'Created At'
        }
    },
    type: 'object',
    required: [
        'id',
        'vmid',
        'user_id',
        'change_type',
        'reason',
        'current_cpu',
        'current_memory',
        'current_disk',
        'requested_cpu',
        'requested_memory',
        'requested_disk',
        'status',
        'reviewer_id',
        'review_comment',
        'reviewed_at',
        'applied_at',
        'created_at'
    ],
    title: 'SpecChangeRequestPublic',
    description: '公開的規格調整申請資訊'
} as const;

export const SpecChangeRequestReviewSchema = {
    properties: {
        status: {
            $ref: '#/components/schemas/SpecChangeRequestStatus'
        },
        review_comment: {
            anyOf: [
                {
                    type: 'string'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Review Comment'
        }
    },
    type: 'object',
    required: [
        'status'
    ],
    title: 'SpecChangeRequestReview',
    description: '審核規格調整申請'
} as const;

export const SpecChangeRequestStatusSchema = {
    type: 'string',
    enum: [
        'pending',
        'approved',
        'rejected'
    ],
    title: 'SpecChangeRequestStatus',
    description: '規格調整申請狀態'
} as const;

export const SpecChangeRequestsPublicSchema = {
    properties: {
        data: {
            items: {
                $ref: '#/components/schemas/SpecChangeRequestPublic'
            },
            type: 'array',
            title: 'Data'
        },
        count: {
            type: 'integer',
            title: 'Count'
        }
    },
    type: 'object',
    required: [
        'data',
        'count'
    ],
    title: 'SpecChangeRequestsPublic',
    description: '規格調整申請列表'
} as const;

export const SpecChangeTypeSchema = {
    type: 'string',
    enum: [
        'cpu',
        'memory',
        'disk',
        'combined'
    ],
    title: 'SpecChangeType',
    description: '規格調整類型'
} as const;

export const StorageInfoSchema = {
    properties: {
        node: {
            type: 'string',
            title: 'Node'
        },
        storage: {
            type: 'string',
            title: 'Storage'
        },
        storage_type: {
            type: 'string',
            title: 'Storage Type'
        },
        content: {
            type: 'string',
            title: 'Content'
        },
        avail_bytes: {
            type: 'integer',
            title: 'Avail Bytes'
        },
        used_bytes: {
            type: 'integer',
            title: 'Used Bytes'
        },
        total_bytes: {
            type: 'integer',
            title: 'Total Bytes'
        },
        used_pct: {
            type: 'number',
            title: 'Used Pct'
        },
        active: {
            type: 'boolean',
            title: 'Active'
        },
        enabled: {
            type: 'boolean',
            title: 'Enabled'
        },
        shared: {
            type: 'boolean',
            title: 'Shared'
        }
    },
    type: 'object',
    required: [
        'node',
        'storage',
        'storage_type',
        'content',
        'avail_bytes',
        'used_bytes',
        'total_bytes',
        'used_pct',
        'active',
        'enabled',
        'shared'
    ],
    title: 'StorageInfo'
} as const;

export const SubnetConfigCreateSchema = {
    properties: {
        cidr: {
            type: 'string',
            title: 'Cidr'
        },
        gateway: {
            type: 'string',
            title: 'Gateway'
        },
        bridge_name: {
            type: 'string',
            title: 'Bridge Name'
        },
        gateway_vm_ip: {
            type: 'string',
            title: 'Gateway Vm Ip'
        },
        dns_servers: {
            anyOf: [
                {
                    type: 'string'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Dns Servers'
        },
        extra_blocked_subnets: {
            items: {
                type: 'string'
            },
            type: 'array',
            title: 'Extra Blocked Subnets',
            default: []
        }
    },
    type: 'object',
    required: [
        'cidr',
        'gateway',
        'bridge_name',
        'gateway_vm_ip'
    ],
    title: 'SubnetConfigCreate',
    description: '設定/更新子網配置'
} as const;

export const SubnetConfigPublicSchema = {
    properties: {
        cidr: {
            type: 'string',
            title: 'Cidr'
        },
        gateway: {
            type: 'string',
            title: 'Gateway'
        },
        bridge_name: {
            type: 'string',
            title: 'Bridge Name'
        },
        gateway_vm_ip: {
            type: 'string',
            title: 'Gateway Vm Ip'
        },
        dns_servers: {
            anyOf: [
                {
                    type: 'string'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Dns Servers'
        },
        extra_blocked_subnets: {
            items: {
                type: 'string'
            },
            type: 'array',
            title: 'Extra Blocked Subnets',
            default: []
        },
        updated_at: {
            type: 'string',
            format: 'date-time',
            title: 'Updated At'
        },
        total_ips: {
            type: 'integer',
            title: 'Total Ips'
        },
        used_ips: {
            type: 'integer',
            title: 'Used Ips'
        },
        available_ips: {
            type: 'integer',
            title: 'Available Ips'
        }
    },
    type: 'object',
    required: [
        'cidr',
        'gateway',
        'bridge_name',
        'gateway_vm_ip',
        'dns_servers',
        'updated_at',
        'total_ips',
        'used_ips',
        'available_ips'
    ],
    title: 'SubnetConfigPublic',
    description: '子網配置公開回傳格式'
} as const;

export const SubnetStatusResponseSchema = {
    properties: {
        configured: {
            type: 'boolean',
            title: 'Configured'
        },
        cidr: {
            anyOf: [
                {
                    type: 'string'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Cidr'
        },
        bridge_name: {
            anyOf: [
                {
                    type: 'string'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Bridge Name'
        },
        total_ips: {
            type: 'integer',
            title: 'Total Ips',
            default: 0
        },
        used_ips: {
            type: 'integer',
            title: 'Used Ips',
            default: 0
        },
        available_ips: {
            type: 'integer',
            title: 'Available Ips',
            default: 0
        }
    },
    type: 'object',
    required: [
        'configured'
    ],
    title: 'SubnetStatusResponse',
    description: '子網狀態摘要'
} as const;

export const SyncNowResultSchema = {
    properties: {
        success: {
            type: 'boolean',
            title: 'Success'
        },
        nodes: {
            items: {
                $ref: '#/components/schemas/ProxmoxNodePublic'
            },
            type: 'array',
            title: 'Nodes'
        },
        storage_count: {
            type: 'integer',
            title: 'Storage Count'
        },
        error: {
            anyOf: [
                {
                    type: 'string'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Error'
        }
    },
    type: 'object',
    required: [
        'success',
        'nodes',
        'storage_count'
    ],
    title: 'SyncNowResult',
    description: '同步節點與 Storage 結果'
} as const;

export const SystemSnapshotSchema = {
    properties: {
        collected_at: {
            type: 'string',
            format: 'date-time',
            title: 'Collected At'
        },
        collection_duration_seconds: {
            type: 'number',
            title: 'Collection Duration Seconds'
        },
        cluster: {
            $ref: '#/components/schemas/ClusterInfo'
        },
        nodes: {
            items: {
                $ref: '#/components/schemas/NodeInfo'
            },
            type: 'array',
            title: 'Nodes'
        },
        storages: {
            items: {
                $ref: '#/components/schemas/StorageInfo'
            },
            type: 'array',
            title: 'Storages'
        },
        resources: {
            items: {
                $ref: '#/components/schemas/ResourceSummary'
            },
            type: 'array',
            title: 'Resources'
        },
        resource_statuses: {
            items: {
                $ref: '#/components/schemas/ResourceStatus'
            },
            type: 'array',
            title: 'Resource Statuses'
        },
        resource_configs: {
            items: {
                $ref: '#/components/schemas/ResourceConfig'
            },
            type: 'array',
            title: 'Resource Configs'
        },
        network_interfaces: {
            items: {
                $ref: '#/components/schemas/NetworkInterface'
            },
            type: 'array',
            title: 'Network Interfaces'
        },
        errors: {
            items: {
                type: 'string'
            },
            type: 'array',
            title: 'Errors'
        },
        total_nodes: {
            type: 'integer',
            title: 'Total Nodes'
        },
        online_nodes: {
            type: 'integer',
            title: 'Online Nodes'
        },
        total_vms: {
            type: 'integer',
            title: 'Total Vms'
        },
        total_lxc: {
            type: 'integer',
            title: 'Total Lxc'
        },
        running_vms: {
            type: 'integer',
            title: 'Running Vms'
        },
        running_lxc: {
            type: 'integer',
            title: 'Running Lxc'
        }
    },
    type: 'object',
    required: [
        'collected_at',
        'collection_duration_seconds',
        'cluster',
        'nodes',
        'storages',
        'resources',
        'resource_statuses',
        'resource_configs',
        'network_interfaces',
        'total_nodes',
        'online_nodes',
        'total_vms',
        'total_lxc',
        'running_vms',
        'running_lxc'
    ],
    title: 'SystemSnapshot'
} as const;

export const TeacherJudgeFileAnalysisUpdateRequestSchema = {
    properties: {
        analysis: {
            $ref: '#/components/schemas/TeacherJudgeRubricAnalysis'
        }
    },
    type: 'object',
    required: [
        'analysis'
    ],
    title: 'TeacherJudgeFileAnalysisUpdateRequest'
} as const;

export const TeacherJudgeFilePublicSchema = {
    properties: {
        id: {
            type: 'string',
            title: 'Id'
        },
        group_id: {
            type: 'string',
            title: 'Group Id'
        },
        uploaded_by: {
            anyOf: [
                {
                    type: 'string'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Uploaded By'
        },
        original_filename: {
            type: 'string',
            title: 'Original Filename'
        },
        file_hash: {
            type: 'string',
            title: 'File Hash'
        },
        template_key: {
            type: 'string',
            title: 'Template Key'
        },
        analysis_json: {
            additionalProperties: true,
            type: 'object',
            title: 'Analysis Json'
        },
        status: {
            type: 'string',
            enum: [
                'active',
                'replaced'
            ],
            title: 'Status'
        },
        created_at: {
            type: 'string',
            title: 'Created At'
        },
        updated_at: {
            type: 'string',
            title: 'Updated At'
        }
    },
    type: 'object',
    required: [
        'id',
        'group_id',
        'uploaded_by',
        'original_filename',
        'file_hash',
        'template_key',
        'analysis_json',
        'status',
        'created_at',
        'updated_at'
    ],
    title: 'TeacherJudgeFilePublic'
} as const;

export const TeacherJudgeFileUploadResponseSchema = {
    properties: {
        file: {
            $ref: '#/components/schemas/TeacherJudgeFilePublic'
        },
        analysis: {
            $ref: '#/components/schemas/TeacherJudgeRubricAnalysis'
        },
        ai_metrics: {
            additionalProperties: true,
            type: 'object',
            title: 'Ai Metrics'
        },
        template_key: {
            type: 'string',
            title: 'Template Key',
            default: 'linux'
        }
    },
    type: 'object',
    required: [
        'file',
        'analysis',
        'ai_metrics'
    ],
    title: 'TeacherJudgeFileUploadResponse'
} as const;

export const TeacherJudgeRubricAnalysisSchema = {
    properties: {
        items: {
            items: {
                $ref: '#/components/schemas/TeacherJudgeRubricItem'
            },
            type: 'array',
            title: 'Items'
        },
        total_items: {
            type: 'integer',
            title: 'Total Items',
            default: 0
        },
        checked_count: {
            type: 'integer',
            title: 'Checked Count',
            default: 0
        },
        auto_count: {
            type: 'integer',
            title: 'Auto Count',
            default: 0
        },
        partial_count: {
            type: 'integer',
            title: 'Partial Count',
            default: 0
        },
        manual_count: {
            type: 'integer',
            title: 'Manual Count',
            default: 0
        },
        summary: {
            type: 'string',
            title: 'Summary',
            description: 'AI 整體說明（繁體中文）',
            default: ''
        },
        raw_text: {
            type: 'string',
            title: 'Raw Text',
            description: '解析後的原始文件文字（供後續對話使用）',
            default: ''
        }
    },
    type: 'object',
    title: 'TeacherJudgeRubricAnalysis',
    description: 'AI 分析評分表後的結構化結果。'
} as const;

export const TeacherJudgeRubricChatMessageSchema = {
    properties: {
        role: {
            type: 'string',
            enum: [
                'user',
                'assistant'
            ],
            title: 'Role',
            description: '\'user\' 或 \'assistant\''
        },
        content: {
            type: 'string',
            title: 'Content',
            description: '訊息內容'
        }
    },
    type: 'object',
    required: [
        'role',
        'content'
    ],
    title: 'TeacherJudgeRubricChatMessage',
    description: '對話訊息。'
} as const;

export const TeacherJudgeRubricChatRequestSchema = {
    properties: {
        messages: {
            items: {
                $ref: '#/components/schemas/TeacherJudgeRubricChatMessage'
            },
            type: 'array',
            minItems: 1,
            title: 'Messages'
        },
        rubric_context: {
            type: 'string',
            title: 'Rubric Context',
            description: '目前評分表的 JSON 字串（作為背景知識）',
            default: ''
        },
        is_refine: {
            type: 'boolean',
            title: 'Is Refine',
            description: 'True = 老師手動調整後觸發的全表潤飾模式',
            default: false
        },
        template_key: {
            type: 'string',
            title: 'Template Key',
            description: '目前評分環境 template key，用於驗證 check_steps',
            default: 'linux'
        }
    },
    type: 'object',
    required: [
        'messages'
    ],
    title: 'TeacherJudgeRubricChatRequest',
    description: '對話請求。'
} as const;

export const TeacherJudgeRubricChatResponseSchema = {
    properties: {
        reply: {
            type: 'string',
            title: 'Reply'
        },
        updated_items: {
            anyOf: [
                {
                    items: {
                        additionalProperties: true,
                        type: 'object'
                    },
                    type: 'array'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Updated Items'
        },
        prompt_tokens: {
            type: 'integer',
            title: 'Prompt Tokens'
        },
        completion_tokens: {
            type: 'integer',
            title: 'Completion Tokens'
        },
        total_tokens: {
            type: 'integer',
            title: 'Total Tokens'
        },
        elapsed_seconds: {
            type: 'number',
            title: 'Elapsed Seconds'
        },
        tokens_per_second: {
            type: 'number',
            title: 'Tokens Per Second'
        }
    },
    type: 'object',
    required: [
        'reply',
        'prompt_tokens',
        'completion_tokens',
        'total_tokens',
        'elapsed_seconds',
        'tokens_per_second'
    ],
    title: 'TeacherJudgeRubricChatResponse',
    description: '對話回應。'
} as const;

export const TeacherJudgeRubricCheckStepSchema = {
    properties: {
        template_key: {
            type: 'string',
            title: 'Template Key',
            description: '評分環境 template key'
        },
        command_key: {
            type: 'string',
            title: 'Command Key',
            description: 'template command catalog 的穩定 ID'
        },
        command_label: {
            anyOf: [
                {
                    type: 'string'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Command Label',
            description: 'template command catalog 的顯示名稱'
        }
    },
    type: 'object',
    required: [
        'template_key',
        'command_key'
    ],
    title: 'TeacherJudgeRubricCheckStep',
    description: '評分計劃書中的 command catalog 引用。'
} as const;

export const TeacherJudgeRubricExportRequestSchema = {
    properties: {
        items: {
            items: {
                additionalProperties: true,
                type: 'object'
            },
            type: 'array',
            minItems: 1,
            title: 'Items'
        },
        summary: {
            type: 'string',
            title: 'Summary',
            default: ''
        }
    },
    type: 'object',
    required: [
        'items'
    ],
    title: 'TeacherJudgeRubricExportRequest',
    description: '匯出 Excel 請求。'
} as const;

export const TeacherJudgeRubricItemSchema = {
    properties: {
        id: {
            type: 'string',
            title: 'Id',
            description: '評分項目唯一 ID'
        },
        title: {
            type: 'string',
            title: 'Title',
            description: '評分項目名稱'
        },
        description: {
            type: 'string',
            title: 'Description',
            description: '評分說明',
            default: ''
        },
        checked: {
            type: 'boolean',
            title: 'Checked',
            description: '是否已達成（有做到就打勾）',
            default: false
        },
        detectable: {
            type: 'string',
            enum: [
                'auto',
                'partial',
                'manual'
            ],
            title: 'Detectable',
            description: '可偵測性：auto | partial | manual',
            default: 'manual'
        },
        detection_method: {
            anyOf: [
                {
                    type: 'string'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Detection Method',
            description: '自動偵測方式說明（detectable=auto/partial 時填寫）'
        },
        fallback: {
            anyOf: [
                {
                    type: 'string'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Fallback',
            description: '無法自動偵測時的替代建議'
        },
        check_steps: {
            items: {
                $ref: '#/components/schemas/TeacherJudgeRubricCheckStep'
            },
            type: 'array',
            title: 'Check Steps',
            description: '本階段只產生計劃書，僅引用既有 command_key，不代表已執行。'
        }
    },
    type: 'object',
    required: [
        'id',
        'title'
    ],
    title: 'TeacherJudgeRubricItem',
    description: '單一評分項目。'
} as const;

export const TeacherJudgeRubricUploadResponseSchema = {
    properties: {
        analysis: {
            $ref: '#/components/schemas/TeacherJudgeRubricAnalysis'
        },
        ai_metrics: {
            additionalProperties: true,
            type: 'object',
            title: 'Ai Metrics'
        },
        template_key: {
            type: 'string',
            title: 'Template Key',
            default: 'linux'
        }
    },
    type: 'object',
    required: [
        'analysis',
        'ai_metrics'
    ],
    title: 'TeacherJudgeRubricUploadResponse',
    description: '上傳評分表回應。'
} as const;

export const TeacherJudgeScriptArtifactPublicSchema = {
    properties: {
        id: {
            type: 'string',
            title: 'Id'
        },
        group_id: {
            type: 'string',
            title: 'Group Id'
        },
        name: {
            type: 'string',
            title: 'Name'
        },
        template_key: {
            type: 'string',
            title: 'Template Key'
        },
        rubric_snapshot_json: {
            additionalProperties: true,
            type: 'object',
            title: 'Rubric Snapshot Json'
        },
        source_file_id: {
            anyOf: [
                {
                    type: 'string'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Source File Id'
        },
        source_file_snapshot_json: {
            additionalProperties: true,
            type: 'object',
            title: 'Source File Snapshot Json'
        },
        script_language: {
            type: 'string',
            enum: [
                'python',
                'shell',
                'bat'
            ],
            title: 'Script Language'
        },
        script_content: {
            type: 'string',
            title: 'Script Content'
        },
        source: {
            type: 'string',
            enum: [
                'ai_generated',
                'regenerated'
            ],
            title: 'Source'
        },
        version: {
            type: 'integer',
            title: 'Version'
        },
        status: {
            type: 'string',
            enum: [
                'draft',
                'review_failed',
                'reviewed',
                'approved',
                'archived'
            ],
            title: 'Status'
        },
        policy_check_result_json: {
            additionalProperties: true,
            type: 'object',
            title: 'Policy Check Result Json'
        },
        ai_review_result_json: {
            additionalProperties: true,
            type: 'object',
            title: 'Ai Review Result Json'
        },
        created_by: {
            anyOf: [
                {
                    type: 'string'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Created By'
        },
        approved_by: {
            anyOf: [
                {
                    type: 'string'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Approved By'
        },
        created_at: {
            type: 'string',
            title: 'Created At'
        },
        updated_at: {
            type: 'string',
            title: 'Updated At'
        },
        approved_at: {
            anyOf: [
                {
                    type: 'string'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Approved At'
        }
    },
    type: 'object',
    required: [
        'id',
        'group_id',
        'name',
        'template_key',
        'rubric_snapshot_json',
        'source_file_id',
        'source_file_snapshot_json',
        'script_language',
        'script_content',
        'source',
        'version',
        'status',
        'policy_check_result_json',
        'ai_review_result_json',
        'created_by',
        'approved_by',
        'created_at',
        'updated_at',
        'approved_at'
    ],
    title: 'TeacherJudgeScriptArtifactPublic'
} as const;

export const TeacherJudgeScriptCreateRequestSchema = {
    properties: {
        name: {
            type: 'string',
            maxLength: 255,
            minLength: 1,
            title: 'Name'
        },
        template_key: {
            type: 'string',
            maxLength: 50,
            title: 'Template Key',
            default: 'linux'
        },
        rubric_snapshot: {
            $ref: '#/components/schemas/TeacherJudgeRubricAnalysis'
        },
        source_file_id: {
            anyOf: [
                {
                    type: 'string',
                    format: 'uuid'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Source File Id'
        }
    },
    type: 'object',
    required: [
        'name',
        'rubric_snapshot'
    ],
    title: 'TeacherJudgeScriptCreateRequest',
    description: 'Create a managed script artifact from the current rubric analysis.'
} as const;

export const TeacherJudgeScriptRegenerateRequestSchema = {
    properties: {
        rubric_snapshot: {
            anyOf: [
                {
                    $ref: '#/components/schemas/TeacherJudgeRubricAnalysis'
                },
                {
                    type: 'null'
                }
            ]
        }
    },
    type: 'object',
    title: 'TeacherJudgeScriptRegenerateRequest',
    description: 'Regenerate a managed script artifact.'
} as const;

export const TeacherJudgeScriptRunCreateRequestSchema = {
    properties: {
        target_scope: {
            type: 'string',
            enum: [
                'all_with_vm',
                'running_only',
                'manual'
            ],
            title: 'Target Scope',
            default: 'manual'
        },
        target_vmids: {
            items: {
                type: 'integer'
            },
            type: 'array',
            title: 'Target Vmids'
        }
    },
    type: 'object',
    title: 'TeacherJudgeScriptRunCreateRequest',
    description: 'Create an execution run for an approved managed script.'
} as const;

export const TeacherJudgeScriptRunPublicSchema = {
    properties: {
        id: {
            type: 'string',
            title: 'Id'
        },
        group_id: {
            type: 'string',
            title: 'Group Id'
        },
        artifact_id: {
            type: 'string',
            title: 'Artifact Id'
        },
        target_scope: {
            type: 'string',
            enum: [
                'all_with_vm',
                'running_only',
                'manual'
            ],
            title: 'Target Scope'
        },
        target_snapshot_json: {
            additionalProperties: true,
            type: 'object',
            title: 'Target Snapshot Json'
        },
        status: {
            type: 'string',
            enum: [
                'pending',
                'running',
                'completed',
                'failed',
                'cancelled'
            ],
            title: 'Status'
        },
        progress_json: {
            additionalProperties: true,
            type: 'object',
            title: 'Progress Json'
        },
        result_summary_json: {
            additionalProperties: true,
            type: 'object',
            title: 'Result Summary Json'
        },
        target_results_json: {
            additionalProperties: true,
            type: 'object',
            title: 'Target Results Json'
        },
        started_by: {
            anyOf: [
                {
                    type: 'string'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Started By'
        },
        started_at: {
            anyOf: [
                {
                    type: 'string'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Started At'
        },
        finished_at: {
            anyOf: [
                {
                    type: 'string'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Finished At'
        },
        created_at: {
            type: 'string',
            title: 'Created At'
        },
        updated_at: {
            type: 'string',
            title: 'Updated At'
        }
    },
    type: 'object',
    required: [
        'id',
        'group_id',
        'artifact_id',
        'target_scope',
        'target_snapshot_json',
        'status',
        'progress_json',
        'result_summary_json',
        'target_results_json',
        'started_by',
        'started_at',
        'finished_at',
        'created_at',
        'updated_at'
    ],
    title: 'TeacherJudgeScriptRunPublic'
} as const;

export const TemplateSchemaSchema = {
    properties: {
        volid: {
            type: 'string',
            title: 'Volid'
        },
        format: {
            type: 'string',
            title: 'Format'
        },
        size: {
            type: 'integer',
            title: 'Size'
        }
    },
    type: 'object',
    required: [
        'volid',
        'format',
        'size'
    ],
    title: 'TemplateSchema',
    description: 'LXC OS template 資訊'
} as const;

export const TerminalInfoSchemaSchema = {
    properties: {
        vmid: {
            type: 'integer',
            title: 'Vmid'
        },
        ws_url: {
            type: 'string',
            title: 'Ws Url'
        },
        ticket: {
            anyOf: [
                {
                    type: 'string'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Ticket'
        },
        message: {
            type: 'string',
            title: 'Message'
        }
    },
    type: 'object',
    required: [
        'vmid',
        'ws_url',
        'message'
    ],
    title: 'TerminalInfoSchema',
    description: 'LXC Terminal 連線資訊'
} as const;

export const TokenSchema = {
    properties: {
        access_token: {
            type: 'string',
            title: 'Access Token'
        },
        refresh_token: {
            anyOf: [
                {
                    type: 'string'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Refresh Token'
        },
        token_type: {
            type: 'string',
            title: 'Token Type',
            default: 'bearer'
        }
    },
    type: 'object',
    required: [
        'access_token'
    ],
    title: 'Token',
    description: 'JWT 存取權杖回應'
} as const;

export const ToolCallRecordSchema = {
    properties: {
        name: {
            type: 'string',
            title: 'Name'
        },
        args: {
            additionalProperties: true,
            type: 'object',
            title: 'Args'
        },
        result: {
            anyOf: [
                {
                    additionalProperties: true,
                    type: 'object'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Result'
        }
    },
    type: 'object',
    required: [
        'name'
    ],
    title: 'ToolCallRecord'
} as const;

export const TopologyEdgeSchema = {
    properties: {
        source_vmid: {
            anyOf: [
                {
                    type: 'integer'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Source Vmid'
        },
        target_vmid: {
            anyOf: [
                {
                    type: 'integer'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Target Vmid'
        },
        ports: {
            items: {
                $ref: '#/components/schemas/PortSpec'
            },
            type: 'array',
            title: 'Ports',
            default: []
        },
        direction: {
            type: 'string',
            enum: [
                'one_way',
                'bidirectional'
            ],
            title: 'Direction',
            default: 'one_way'
        }
    },
    type: 'object',
    title: 'TopologyEdge',
    description: '拓撲圖中的連線'
} as const;

export const TopologyNodeSchema = {
    properties: {
        vmid: {
            anyOf: [
                {
                    type: 'integer'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Vmid'
        },
        name: {
            type: 'string',
            title: 'Name'
        },
        node_type: {
            type: 'string',
            enum: [
                'vm',
                'gateway'
            ],
            title: 'Node Type'
        },
        vm_type: {
            anyOf: [
                {
                    type: 'string',
                    enum: [
                        'qemu',
                        'lxc'
                    ]
                },
                {
                    type: 'null'
                }
            ],
            title: 'Vm Type'
        },
        status: {
            anyOf: [
                {
                    type: 'string'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Status'
        },
        ip_address: {
            anyOf: [
                {
                    type: 'string'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Ip Address'
        },
        firewall_enabled: {
            type: 'boolean',
            title: 'Firewall Enabled',
            default: false
        },
        position_x: {
            type: 'number',
            title: 'Position X',
            default: 100
        },
        position_y: {
            type: 'number',
            title: 'Position Y',
            default: 100
        }
    },
    type: 'object',
    required: [
        'name',
        'node_type'
    ],
    title: 'TopologyNode',
    description: '拓撲圖中的節點'
} as const;

export const TopologyResponseSchema = {
    properties: {
        nodes: {
            items: {
                $ref: '#/components/schemas/TopologyNode'
            },
            type: 'array',
            title: 'Nodes'
        },
        edges: {
            items: {
                $ref: '#/components/schemas/TopologyEdge'
            },
            type: 'array',
            title: 'Edges'
        }
    },
    type: 'object',
    required: [
        'nodes',
        'edges'
    ],
    title: 'TopologyResponse',
    description: '完整拓撲資料（節點 + 連線）'
} as const;

export const UpdatePasswordSchema = {
    properties: {
        current_password: {
            type: 'string',
            maxLength: 128,
            minLength: 8,
            title: 'Current Password'
        },
        new_password: {
            type: 'string',
            maxLength: 128,
            minLength: 8,
            title: 'New Password'
        }
    },
    type: 'object',
    required: [
        'current_password',
        'new_password'
    ],
    title: 'UpdatePassword',
    description: '更新密碼'
} as const;

export const UsageByModelSchema = {
    properties: {
        requests: {
            type: 'integer',
            title: 'Requests'
        },
        input_tokens: {
            type: 'integer',
            title: 'Input Tokens'
        },
        output_tokens: {
            type: 'integer',
            title: 'Output Tokens'
        }
    },
    type: 'object',
    required: [
        'requests',
        'input_tokens',
        'output_tokens'
    ],
    title: 'UsageByModel',
    description: '按模型分組的使用量'
} as const;

export const UsageInfoSchema = {
    properties: {
        prompt_tokens: {
            type: 'integer',
            title: 'Prompt Tokens',
            default: 0
        },
        completion_tokens: {
            type: 'integer',
            title: 'Completion Tokens',
            default: 0
        },
        total_tokens: {
            type: 'integer',
            title: 'Total Tokens',
            default: 0
        }
    },
    type: 'object',
    title: 'UsageInfo',
    description: '使用量信息'
} as const;

export const UsageStatsResponseSchema = {
    properties: {
        total_requests: {
            type: 'integer',
            title: 'Total Requests'
        },
        total_input_tokens: {
            type: 'integer',
            title: 'Total Input Tokens'
        },
        total_output_tokens: {
            type: 'integer',
            title: 'Total Output Tokens'
        },
        by_model: {
            additionalProperties: {
                $ref: '#/components/schemas/UsageByModel'
            },
            type: 'object',
            title: 'By Model'
        },
        start_date: {
            type: 'string',
            format: 'date-time',
            title: 'Start Date'
        },
        end_date: {
            type: 'string',
            format: 'date-time',
            title: 'End Date'
        }
    },
    type: 'object',
    required: [
        'total_requests',
        'total_input_tokens',
        'total_output_tokens',
        'by_model',
        'start_date',
        'end_date'
    ],
    title: 'UsageStatsResponse',
    description: 'Proxy 使用量統計回應'
} as const;

export const UserCreateSchema = {
    properties: {
        email: {
            type: 'string',
            maxLength: 255,
            format: 'email',
            title: 'Email'
        },
        password: {
            type: 'string',
            maxLength: 128,
            minLength: 8,
            title: 'Password'
        },
        is_active: {
            type: 'boolean',
            title: 'Is Active',
            default: true
        },
        role: {
            $ref: '#/components/schemas/UserRole',
            default: 'student'
        },
        is_superuser: {
            type: 'boolean',
            title: 'Is Superuser',
            default: false
        },
        full_name: {
            anyOf: [
                {
                    type: 'string',
                    maxLength: 255
                },
                {
                    type: 'null'
                }
            ],
            title: 'Full Name'
        },
        avatar_url: {
            anyOf: [
                {
                    type: 'string',
                    maxLength: 2048
                },
                {
                    type: 'null'
                }
            ],
            title: 'Avatar Url'
        }
    },
    type: 'object',
    required: [
        'email',
        'password'
    ],
    title: 'UserCreate',
    description: '建立使用者'
} as const;

export const UserPublicSchema = {
    properties: {
        id: {
            type: 'string',
            format: 'uuid',
            title: 'Id'
        },
        email: {
            type: 'string',
            format: 'email',
            title: 'Email'
        },
        is_active: {
            type: 'boolean',
            title: 'Is Active'
        },
        role: {
            $ref: '#/components/schemas/UserRole'
        },
        is_superuser: {
            type: 'boolean',
            title: 'Is Superuser'
        },
        full_name: {
            anyOf: [
                {
                    type: 'string'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Full Name'
        },
        avatar_url: {
            anyOf: [
                {
                    type: 'string'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Avatar Url'
        },
        created_at: {
            anyOf: [
                {
                    type: 'string',
                    format: 'date-time'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Created At'
        }
    },
    type: 'object',
    required: [
        'id',
        'email',
        'is_active',
        'role',
        'is_superuser'
    ],
    title: 'UserPublic',
    description: 'API 回傳的使用者資料'
} as const;

export const UserRegisterSchema = {
    properties: {
        email: {
            type: 'string',
            maxLength: 255,
            format: 'email',
            title: 'Email'
        },
        password: {
            type: 'string',
            maxLength: 128,
            minLength: 8,
            title: 'Password'
        },
        full_name: {
            anyOf: [
                {
                    type: 'string',
                    maxLength: 255
                },
                {
                    type: 'null'
                }
            ],
            title: 'Full Name'
        },
        avatar_url: {
            anyOf: [
                {
                    type: 'string',
                    maxLength: 2048
                },
                {
                    type: 'null'
                }
            ],
            title: 'Avatar Url'
        }
    },
    type: 'object',
    required: [
        'email',
        'password'
    ],
    title: 'UserRegister',
    description: '使用者自行註冊'
} as const;

export const UserRoleSchema = {
    type: 'string',
    enum: [
        'student',
        'teacher',
        'admin'
    ],
    title: 'UserRole'
} as const;

export const UserUpdateSchema = {
    properties: {
        email: {
            anyOf: [
                {
                    type: 'string',
                    maxLength: 255,
                    format: 'email'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Email'
        },
        password: {
            anyOf: [
                {
                    type: 'string',
                    maxLength: 128,
                    minLength: 8
                },
                {
                    type: 'null'
                }
            ],
            title: 'Password'
        },
        is_active: {
            anyOf: [
                {
                    type: 'boolean'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Is Active'
        },
        role: {
            anyOf: [
                {
                    $ref: '#/components/schemas/UserRole'
                },
                {
                    type: 'null'
                }
            ]
        },
        is_superuser: {
            anyOf: [
                {
                    type: 'boolean'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Is Superuser'
        },
        full_name: {
            anyOf: [
                {
                    type: 'string',
                    maxLength: 255
                },
                {
                    type: 'null'
                }
            ],
            title: 'Full Name'
        },
        avatar_url: {
            anyOf: [
                {
                    type: 'string',
                    maxLength: 2048
                },
                {
                    type: 'null'
                }
            ],
            title: 'Avatar Url'
        }
    },
    type: 'object',
    title: 'UserUpdate',
    description: '管理員更新使用者'
} as const;

export const UserUpdateMeSchema = {
    properties: {
        full_name: {
            anyOf: [
                {
                    type: 'string',
                    maxLength: 255
                },
                {
                    type: 'null'
                }
            ],
            title: 'Full Name'
        },
        email: {
            anyOf: [
                {
                    type: 'string',
                    maxLength: 255,
                    format: 'email'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Email'
        },
        avatar_url: {
            anyOf: [
                {
                    type: 'string',
                    maxLength: 2048
                },
                {
                    type: 'null'
                }
            ],
            title: 'Avatar Url'
        }
    },
    type: 'object',
    title: 'UserUpdateMe',
    description: '使用者更新自己資料'
} as const;

export const UsersPublicSchema = {
    properties: {
        data: {
            items: {
                $ref: '#/components/schemas/UserPublic'
            },
            type: 'array',
            title: 'Data'
        },
        count: {
            type: 'integer',
            title: 'Count'
        }
    },
    type: 'object',
    required: [
        'data',
        'count'
    ],
    title: 'UsersPublic',
    description: '使用者列表回應'
} as const;

export const VMCreateRequestSchema = {
    properties: {
        hostname: {
            type: 'string',
            maxLength: 63,
            minLength: 1,
            title: 'Hostname'
        },
        template_id: {
            type: 'integer',
            title: 'Template Id'
        },
        username: {
            type: 'string',
            maxLength: 32,
            minLength: 1,
            title: 'Username'
        },
        password: {
            type: 'string',
            minLength: 6,
            title: 'Password'
        },
        cores: {
            type: 'integer',
            maximum: 32,
            minimum: 1,
            title: 'Cores',
            default: 2
        },
        memory: {
            type: 'integer',
            maximum: 65536,
            minimum: 512,
            title: 'Memory',
            default: 2048
        },
        disk_size: {
            type: 'integer',
            maximum: 1000,
            minimum: 10,
            title: 'Disk Size',
            default: 20
        },
        storage: {
            type: 'string',
            title: 'Storage',
            default: 'local-lvm'
        },
        environment_type: {
            type: 'string',
            title: 'Environment Type'
        },
        os_info: {
            anyOf: [
                {
                    type: 'string'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Os Info'
        },
        expiry_date: {
            anyOf: [
                {
                    type: 'string',
                    format: 'date'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Expiry Date'
        },
        start: {
            type: 'boolean',
            title: 'Start',
            default: true
        },
        service_template_slug: {
            anyOf: [
                {
                    type: 'string'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Service Template Slug'
        }
    },
    type: 'object',
    required: [
        'hostname',
        'template_id',
        'username',
        'password',
        'environment_type'
    ],
    title: 'VMCreateRequest',
    description: '建立 VM（cloud-init template）'
} as const;

export const VMCreateResponseSchema = {
    properties: {
        vmid: {
            type: 'integer',
            title: 'Vmid'
        },
        upid: {
            type: 'string',
            title: 'Upid'
        },
        message: {
            type: 'string',
            title: 'Message'
        }
    },
    type: 'object',
    required: [
        'vmid',
        'upid',
        'message'
    ],
    title: 'VMCreateResponse',
    description: '建立 VM 回應'
} as const;

export const VMMigrationJobStatusSchema = {
    type: 'string',
    enum: [
        'pending',
        'running',
        'completed',
        'failed',
        'blocked',
        'cancelled'
    ],
    title: 'VMMigrationJobStatus'
} as const;

export const VMMigrationStatusSchema = {
    type: 'string',
    enum: [
        'idle',
        'pending',
        'running',
        'completed',
        'failed',
        'blocked'
    ],
    title: 'VMMigrationStatus'
} as const;

export const VMRequestAvailabilityDaySchema = {
    properties: {
        date: {
            type: 'string',
            format: 'date',
            title: 'Date'
        },
        available_hours: {
            items: {
                type: 'integer'
            },
            type: 'array',
            title: 'Available Hours'
        },
        limited_hours: {
            items: {
                type: 'integer'
            },
            type: 'array',
            title: 'Limited Hours'
        },
        blocked_hours: {
            items: {
                type: 'integer'
            },
            type: 'array',
            title: 'Blocked Hours'
        },
        unavailable_hours: {
            items: {
                type: 'integer'
            },
            type: 'array',
            title: 'Unavailable Hours'
        },
        best_hours: {
            items: {
                type: 'integer'
            },
            type: 'array',
            title: 'Best Hours'
        },
        slots: {
            items: {
                $ref: '#/components/schemas/VMRequestAvailabilitySlot'
            },
            type: 'array',
            title: 'Slots'
        }
    },
    type: 'object',
    required: [
        'date'
    ],
    title: 'VMRequestAvailabilityDay'
} as const;

export const VMRequestAvailabilityNodeSnapshotSchema = {
    properties: {
        node: {
            type: 'string',
            title: 'Node'
        },
        status: {
            type: 'string',
            title: 'Status'
        },
        candidate: {
            type: 'boolean',
            title: 'Candidate'
        },
        priority: {
            type: 'integer',
            maximum: 10,
            minimum: 1,
            title: 'Priority',
            default: 5
        },
        is_target: {
            type: 'boolean',
            title: 'Is Target',
            default: false
        },
        placement_count: {
            type: 'integer',
            minimum: 0,
            title: 'Placement Count',
            default: 0
        },
        running_resources: {
            type: 'integer',
            minimum: 0,
            title: 'Running Resources',
            default: 0
        },
        projected_running_resources: {
            type: 'integer',
            minimum: 0,
            title: 'Projected Running Resources',
            default: 0
        },
        dominant_share: {
            type: 'number',
            minimum: 0,
            title: 'Dominant Share',
            default: 0
        },
        average_share: {
            type: 'number',
            minimum: 0,
            title: 'Average Share',
            default: 0
        },
        cpu_share: {
            type: 'number',
            minimum: 0,
            title: 'Cpu Share',
            default: 0
        },
        memory_share: {
            type: 'number',
            minimum: 0,
            title: 'Memory Share',
            default: 0
        },
        disk_share: {
            type: 'number',
            minimum: 0,
            title: 'Disk Share',
            default: 0
        },
        remaining_cpu_cores: {
            type: 'number',
            minimum: 0,
            title: 'Remaining Cpu Cores',
            default: 0
        },
        remaining_memory_gb: {
            type: 'number',
            minimum: 0,
            title: 'Remaining Memory Gb',
            default: 0
        },
        remaining_disk_gb: {
            type: 'number',
            minimum: 0,
            title: 'Remaining Disk Gb',
            default: 0
        },
        vm_stack: {
            items: {
                $ref: '#/components/schemas/VMRequestAvailabilityStackItem'
            },
            type: 'array',
            title: 'Vm Stack'
        }
    },
    type: 'object',
    required: [
        'node',
        'status',
        'candidate'
    ],
    title: 'VMRequestAvailabilityNodeSnapshot'
} as const;

export const VMRequestAvailabilityRequestSchema = {
    properties: {
        resource_type: {
            type: 'string',
            enum: [
                'lxc',
                'vm'
            ],
            title: 'Resource Type',
            default: 'lxc'
        },
        cores: {
            type: 'integer',
            maximum: 256,
            minimum: 1,
            title: 'Cores',
            default: 2
        },
        memory: {
            type: 'integer',
            maximum: 1048576,
            minimum: 128,
            title: 'Memory',
            description: 'MB',
            default: 2048
        },
        disk_size: {
            anyOf: [
                {
                    type: 'integer',
                    maximum: 65536,
                    minimum: 1
                },
                {
                    type: 'null'
                }
            ],
            title: 'Disk Size'
        },
        rootfs_size: {
            anyOf: [
                {
                    type: 'integer',
                    maximum: 65536,
                    minimum: 1
                },
                {
                    type: 'null'
                }
            ],
            title: 'Rootfs Size'
        },
        instance_count: {
            type: 'integer',
            maximum: 100,
            minimum: 1,
            title: 'Instance Count',
            default: 1
        },
        gpu_required: {
            type: 'integer',
            maximum: 16,
            minimum: 0,
            title: 'Gpu Required',
            default: 0
        },
        days: {
            type: 'integer',
            maximum: 14,
            minimum: 1,
            title: 'Days',
            default: 7
        },
        timezone: {
            type: 'string',
            maxLength: 64,
            minLength: 1,
            title: 'Timezone',
            default: 'Asia/Taipei'
        },
        policy_role: {
            anyOf: [
                {
                    $ref: '#/components/schemas/UserRole'
                },
                {
                    type: 'null'
                }
            ]
        }
    },
    type: 'object',
    title: 'VMRequestAvailabilityRequest'
} as const;

export const VMRequestAvailabilityResponseSchema = {
    properties: {
        summary: {
            $ref: '#/components/schemas/VMRequestAvailabilitySummary'
        },
        recommended_slots: {
            items: {
                $ref: '#/components/schemas/VMRequestAvailabilitySlot'
            },
            type: 'array',
            title: 'Recommended Slots'
        },
        days: {
            items: {
                $ref: '#/components/schemas/VMRequestAvailabilityDay'
            },
            type: 'array',
            title: 'Days'
        }
    },
    type: 'object',
    required: [
        'summary'
    ],
    title: 'VMRequestAvailabilityResponse'
} as const;

export const VMRequestAvailabilitySlotSchema = {
    properties: {
        start_at: {
            type: 'string',
            format: 'date-time',
            title: 'Start At'
        },
        end_at: {
            type: 'string',
            format: 'date-time',
            title: 'End At'
        },
        date: {
            type: 'string',
            format: 'date',
            title: 'Date'
        },
        hour: {
            type: 'integer',
            maximum: 23,
            minimum: 0,
            title: 'Hour'
        },
        within_policy: {
            type: 'boolean',
            title: 'Within Policy'
        },
        feasible: {
            type: 'boolean',
            title: 'Feasible'
        },
        status: {
            type: 'string',
            enum: [
                'available',
                'limited',
                'unavailable',
                'policy_blocked'
            ],
            title: 'Status'
        },
        label: {
            type: 'string',
            title: 'Label'
        },
        summary: {
            type: 'string',
            title: 'Summary'
        },
        reasons: {
            items: {
                type: 'string'
            },
            type: 'array',
            title: 'Reasons'
        },
        recommended_nodes: {
            items: {
                type: 'string'
            },
            type: 'array',
            title: 'Recommended Nodes'
        },
        target_node: {
            anyOf: [
                {
                    type: 'string'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Target Node'
        },
        placement_strategy: {
            anyOf: [
                {
                    type: 'string'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Placement Strategy'
        },
        node_snapshots: {
            items: {
                $ref: '#/components/schemas/VMRequestAvailabilityNodeSnapshot'
            },
            type: 'array',
            title: 'Node Snapshots'
        }
    },
    type: 'object',
    required: [
        'start_at',
        'end_at',
        'date',
        'hour',
        'within_policy',
        'feasible',
        'status',
        'label',
        'summary'
    ],
    title: 'VMRequestAvailabilitySlot'
} as const;

export const VMRequestAvailabilityStackItemSchema = {
    properties: {
        name: {
            type: 'string',
            title: 'Name'
        },
        count: {
            type: 'integer',
            minimum: 0,
            title: 'Count',
            default: 0
        },
        pending: {
            type: 'boolean',
            title: 'Pending',
            default: false
        }
    },
    type: 'object',
    required: [
        'name'
    ],
    title: 'VMRequestAvailabilityStackItem'
} as const;

export const VMRequestAvailabilitySummarySchema = {
    properties: {
        timezone: {
            type: 'string',
            title: 'Timezone'
        },
        role: {
            type: 'string',
            title: 'Role'
        },
        role_label: {
            type: 'string',
            title: 'Role Label'
        },
        policy_window: {
            type: 'string',
            title: 'Policy Window'
        },
        checked_days: {
            type: 'integer',
            maximum: 14,
            minimum: 1,
            title: 'Checked Days'
        },
        feasible_slot_count: {
            type: 'integer',
            minimum: 0,
            title: 'Feasible Slot Count',
            default: 0
        },
        recommended_slot_count: {
            type: 'integer',
            minimum: 0,
            title: 'Recommended Slot Count',
            default: 0
        },
        current_status: {
            type: 'string',
            title: 'Current Status'
        }
    },
    type: 'object',
    required: [
        'timezone',
        'role',
        'role_label',
        'policy_window',
        'checked_days',
        'current_status'
    ],
    title: 'VMRequestAvailabilitySummary'
} as const;

export const VMRequestCreateSchema = {
    properties: {
        reason: {
            type: 'string',
            minLength: 10,
            title: 'Reason'
        },
        resource_type: {
            type: 'string',
            title: 'Resource Type'
        },
        hostname: {
            type: 'string',
            maxLength: 63,
            minLength: 1,
            title: 'Hostname'
        },
        cores: {
            type: 'integer',
            title: 'Cores',
            default: 2
        },
        memory: {
            type: 'integer',
            title: 'Memory',
            default: 2048
        },
        password: {
            type: 'string',
            maxLength: 128,
            minLength: 8,
            title: 'Password'
        },
        storage: {
            type: 'string',
            title: 'Storage',
            default: 'local-lvm'
        },
        environment_type: {
            type: 'string',
            title: 'Environment Type',
            default: 'Custom'
        },
        os_info: {
            anyOf: [
                {
                    type: 'string'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Os Info'
        },
        expiry_date: {
            anyOf: [
                {
                    type: 'string',
                    format: 'date'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Expiry Date'
        },
        mode: {
            type: 'string',
            enum: [
                'quick_template',
                'immediate',
                'scheduled'
            ],
            title: 'Mode',
            default: 'scheduled'
        },
        start_at: {
            anyOf: [
                {
                    type: 'string',
                    format: 'date-time'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Start At'
        },
        end_at: {
            anyOf: [
                {
                    type: 'string',
                    format: 'date-time'
                },
                {
                    type: 'null'
                }
            ],
            title: 'End At'
        },
        ostemplate: {
            anyOf: [
                {
                    type: 'string'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Ostemplate'
        },
        rootfs_size: {
            anyOf: [
                {
                    type: 'integer'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Rootfs Size'
        },
        template_id: {
            anyOf: [
                {
                    type: 'integer'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Template Id'
        },
        disk_size: {
            anyOf: [
                {
                    type: 'integer'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Disk Size'
        },
        username: {
            anyOf: [
                {
                    type: 'string'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Username'
        },
        gpu_mapping_id: {
            anyOf: [
                {
                    type: 'string'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Gpu Mapping Id'
        },
        service_template_slug: {
            anyOf: [
                {
                    type: 'string'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Service Template Slug'
        },
        service_template_script_path: {
            anyOf: [
                {
                    type: 'string'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Service Template Script Path'
        }
    },
    type: 'object',
    required: [
        'reason',
        'resource_type',
        'hostname',
        'password'
    ],
    title: 'VMRequestCreate'
} as const;

export const VMRequestPublicSchema = {
    properties: {
        id: {
            type: 'string',
            format: 'uuid',
            title: 'Id'
        },
        user_id: {
            type: 'string',
            format: 'uuid',
            title: 'User Id'
        },
        user_email: {
            anyOf: [
                {
                    type: 'string'
                },
                {
                    type: 'null'
                }
            ],
            title: 'User Email'
        },
        user_full_name: {
            anyOf: [
                {
                    type: 'string'
                },
                {
                    type: 'null'
                }
            ],
            title: 'User Full Name'
        },
        reason: {
            type: 'string',
            title: 'Reason'
        },
        resource_type: {
            type: 'string',
            title: 'Resource Type'
        },
        request_kind: {
            type: 'string',
            title: 'Request Kind',
            default: 'research'
        },
        hostname: {
            type: 'string',
            title: 'Hostname'
        },
        cores: {
            type: 'integer',
            title: 'Cores'
        },
        memory: {
            type: 'integer',
            title: 'Memory'
        },
        storage: {
            type: 'string',
            title: 'Storage'
        },
        environment_type: {
            type: 'string',
            title: 'Environment Type'
        },
        os_info: {
            anyOf: [
                {
                    type: 'string'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Os Info'
        },
        expiry_date: {
            anyOf: [
                {
                    type: 'string',
                    format: 'date'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Expiry Date'
        },
        start_at: {
            anyOf: [
                {
                    type: 'string',
                    format: 'date-time'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Start At'
        },
        end_at: {
            anyOf: [
                {
                    type: 'string',
                    format: 'date-time'
                },
                {
                    type: 'null'
                }
            ],
            title: 'End At'
        },
        ostemplate: {
            anyOf: [
                {
                    type: 'string'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Ostemplate'
        },
        rootfs_size: {
            anyOf: [
                {
                    type: 'integer'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Rootfs Size'
        },
        template_id: {
            anyOf: [
                {
                    type: 'integer'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Template Id'
        },
        disk_size: {
            anyOf: [
                {
                    type: 'integer'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Disk Size'
        },
        username: {
            anyOf: [
                {
                    type: 'string'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Username'
        },
        gpu_mapping_id: {
            anyOf: [
                {
                    type: 'string'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Gpu Mapping Id'
        },
        service_template_slug: {
            anyOf: [
                {
                    type: 'string'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Service Template Slug'
        },
        service_template_script_path: {
            anyOf: [
                {
                    type: 'string'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Service Template Script Path'
        },
        status: {
            $ref: '#/components/schemas/VMRequestStatus'
        },
        reviewer_id: {
            anyOf: [
                {
                    type: 'string',
                    format: 'uuid'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Reviewer Id'
        },
        review_comment: {
            anyOf: [
                {
                    type: 'string'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Review Comment'
        },
        reviewed_at: {
            anyOf: [
                {
                    type: 'string',
                    format: 'date-time'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Reviewed At'
        },
        vmid: {
            anyOf: [
                {
                    type: 'integer'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Vmid'
        },
        assigned_node: {
            anyOf: [
                {
                    type: 'string'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Assigned Node'
        },
        desired_node: {
            anyOf: [
                {
                    type: 'string'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Desired Node'
        },
        actual_node: {
            anyOf: [
                {
                    type: 'string'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Actual Node'
        },
        placement_strategy_used: {
            anyOf: [
                {
                    type: 'string'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Placement Strategy Used'
        },
        migration_status: {
            $ref: '#/components/schemas/VMMigrationStatus',
            default: 'idle'
        },
        migration_error: {
            anyOf: [
                {
                    type: 'string'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Migration Error'
        },
        migration_pinned: {
            type: 'boolean',
            title: 'Migration Pinned',
            default: false
        },
        resource_warning: {
            anyOf: [
                {
                    type: 'string'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Resource Warning'
        },
        rebalance_epoch: {
            type: 'integer',
            title: 'Rebalance Epoch',
            default: 0
        },
        last_rebalanced_at: {
            anyOf: [
                {
                    type: 'string',
                    format: 'date-time'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Last Rebalanced At'
        },
        last_migrated_at: {
            anyOf: [
                {
                    type: 'string',
                    format: 'date-time'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Last Migrated At'
        },
        recurrence_rule: {
            anyOf: [
                {
                    type: 'string'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Recurrence Rule'
        },
        recurrence_duration_minutes: {
            anyOf: [
                {
                    type: 'integer'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Recurrence Duration Minutes'
        },
        schedule_timezone: {
            anyOf: [
                {
                    type: 'string'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Schedule Timezone'
        },
        next_window_start: {
            anyOf: [
                {
                    type: 'string',
                    format: 'date-time'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Next Window Start'
        },
        next_window_end: {
            anyOf: [
                {
                    type: 'string',
                    format: 'date-time'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Next Window End'
        },
        batch_job_id: {
            anyOf: [
                {
                    type: 'string',
                    format: 'uuid'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Batch Job Id'
        },
        created_at: {
            type: 'string',
            format: 'date-time',
            title: 'Created At'
        }
    },
    type: 'object',
    required: [
        'id',
        'user_id',
        'reason',
        'resource_type',
        'hostname',
        'cores',
        'memory',
        'storage',
        'environment_type',
        'status',
        'created_at'
    ],
    title: 'VMRequestPublic'
} as const;

export const VMRequestReviewSchema = {
    properties: {
        status: {
            type: 'string',
            enum: [
                'approved',
                'rejected'
            ],
            title: 'Status'
        },
        review_comment: {
            anyOf: [
                {
                    type: 'string'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Review Comment'
        }
    },
    type: 'object',
    required: [
        'status'
    ],
    title: 'VMRequestReview'
} as const;

export const VMRequestReviewContextSchema = {
    properties: {
        request: {
            $ref: '#/components/schemas/VMRequestPublic'
        },
        window_start: {
            type: 'string',
            format: 'date-time',
            title: 'Window Start'
        },
        window_end: {
            type: 'string',
            format: 'date-time',
            title: 'Window End'
        },
        window_active_now: {
            type: 'boolean',
            title: 'Window Active Now',
            default: false
        },
        feasible: {
            type: 'boolean',
            title: 'Feasible',
            default: false
        },
        placement_strategy: {
            anyOf: [
                {
                    type: 'string'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Placement Strategy'
        },
        projected_node: {
            anyOf: [
                {
                    type: 'string'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Projected Node'
        },
        summary: {
            type: 'string',
            title: 'Summary'
        },
        reasons: {
            items: {
                type: 'string'
            },
            type: 'array',
            title: 'Reasons'
        },
        warnings: {
            items: {
                type: 'string'
            },
            type: 'array',
            title: 'Warnings'
        },
        resource_warnings: {
            items: {
                type: 'string'
            },
            type: 'array',
            title: 'Resource Warnings'
        },
        cluster_nodes: {
            items: {
                type: 'string'
            },
            type: 'array',
            title: 'Cluster Nodes'
        },
        current_running_resources: {
            items: {
                $ref: '#/components/schemas/VMRequestReviewRuntimeResource'
            },
            type: 'array',
            title: 'Current Running Resources'
        },
        overlapping_approved_requests: {
            items: {
                $ref: '#/components/schemas/VMRequestReviewOverlapItem'
            },
            type: 'array',
            title: 'Overlapping Approved Requests'
        },
        projected_nodes: {
            items: {
                $ref: '#/components/schemas/VMRequestReviewProjectedNode'
            },
            type: 'array',
            title: 'Projected Nodes'
        },
        node_scores: {
            items: {
                $ref: '#/components/schemas/VMRequestReviewNodeScore'
            },
            type: 'array',
            title: 'Node Scores'
        }
    },
    type: 'object',
    required: [
        'request',
        'window_start',
        'window_end',
        'summary'
    ],
    title: 'VMRequestReviewContext'
} as const;

export const VMRequestReviewNodeScoreSchema = {
    properties: {
        node: {
            type: 'string',
            title: 'Node'
        },
        balance_score: {
            type: 'number',
            title: 'Balance Score',
            default: 0
        },
        cpu_share: {
            type: 'number',
            title: 'Cpu Share',
            default: 0
        },
        memory_share: {
            type: 'number',
            title: 'Memory Share',
            default: 0
        },
        disk_share: {
            type: 'number',
            title: 'Disk Share',
            default: 0
        },
        peak_penalty: {
            type: 'number',
            title: 'Peak Penalty',
            default: 0
        },
        loadavg_penalty: {
            type: 'number',
            title: 'Loadavg Penalty',
            default: 0
        },
        storage_penalty: {
            type: 'number',
            title: 'Storage Penalty',
            default: 0
        },
        migration_cost: {
            type: 'number',
            title: 'Migration Cost',
            default: 0
        },
        priority: {
            type: 'integer',
            title: 'Priority',
            default: 5
        },
        is_selected: {
            type: 'boolean',
            title: 'Is Selected',
            default: false
        },
        reason: {
            anyOf: [
                {
                    type: 'string'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Reason'
        }
    },
    type: 'object',
    required: [
        'node'
    ],
    title: 'VMRequestReviewNodeScore'
} as const;

export const VMRequestReviewOverlapItemSchema = {
    properties: {
        request_id: {
            type: 'string',
            format: 'uuid',
            title: 'Request Id'
        },
        hostname: {
            type: 'string',
            title: 'Hostname'
        },
        resource_type: {
            type: 'string',
            title: 'Resource Type'
        },
        start_at: {
            anyOf: [
                {
                    type: 'string',
                    format: 'date-time'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Start At'
        },
        end_at: {
            anyOf: [
                {
                    type: 'string',
                    format: 'date-time'
                },
                {
                    type: 'null'
                }
            ],
            title: 'End At'
        },
        vmid: {
            anyOf: [
                {
                    type: 'integer'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Vmid'
        },
        status: {
            $ref: '#/components/schemas/VMRequestStatus'
        },
        assigned_node: {
            anyOf: [
                {
                    type: 'string'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Assigned Node'
        },
        desired_node: {
            anyOf: [
                {
                    type: 'string'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Desired Node'
        },
        actual_node: {
            anyOf: [
                {
                    type: 'string'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Actual Node'
        },
        projected_node: {
            anyOf: [
                {
                    type: 'string'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Projected Node'
        },
        projected_strategy: {
            anyOf: [
                {
                    type: 'string'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Projected Strategy'
        },
        migration_status: {
            $ref: '#/components/schemas/VMMigrationStatus',
            default: 'idle'
        },
        last_migrated_at: {
            anyOf: [
                {
                    type: 'string',
                    format: 'date-time'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Last Migrated At'
        },
        is_current_request: {
            type: 'boolean',
            title: 'Is Current Request',
            default: false
        },
        is_running_now: {
            type: 'boolean',
            title: 'Is Running Now',
            default: false
        },
        is_provisioned: {
            type: 'boolean',
            title: 'Is Provisioned',
            default: false
        }
    },
    type: 'object',
    required: [
        'request_id',
        'hostname',
        'resource_type',
        'status'
    ],
    title: 'VMRequestReviewOverlapItem'
} as const;

export const VMRequestReviewProjectedNodeSchema = {
    properties: {
        node: {
            type: 'string',
            title: 'Node'
        },
        request_count: {
            type: 'integer',
            minimum: 0,
            title: 'Request Count',
            default: 0
        },
        includes_current_request: {
            type: 'boolean',
            title: 'Includes Current Request',
            default: false
        },
        hostnames: {
            items: {
                type: 'string'
            },
            type: 'array',
            title: 'Hostnames'
        }
    },
    type: 'object',
    required: [
        'node'
    ],
    title: 'VMRequestReviewProjectedNode'
} as const;

export const VMRequestReviewRuntimeResourceSchema = {
    properties: {
        vmid: {
            type: 'integer',
            title: 'Vmid'
        },
        name: {
            type: 'string',
            title: 'Name'
        },
        node: {
            type: 'string',
            title: 'Node'
        },
        resource_type: {
            type: 'string',
            title: 'Resource Type'
        },
        status: {
            type: 'string',
            title: 'Status'
        },
        linked_request_id: {
            anyOf: [
                {
                    type: 'string',
                    format: 'uuid'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Linked Request Id'
        },
        linked_hostname: {
            anyOf: [
                {
                    type: 'string'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Linked Hostname'
        },
        linked_actual_node: {
            anyOf: [
                {
                    type: 'string'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Linked Actual Node'
        },
        linked_desired_node: {
            anyOf: [
                {
                    type: 'string'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Linked Desired Node'
        }
    },
    type: 'object',
    required: [
        'vmid',
        'name',
        'node',
        'resource_type',
        'status'
    ],
    title: 'VMRequestReviewRuntimeResource'
} as const;

export const VMRequestStatusSchema = {
    type: 'string',
    enum: [
        'pending',
        'approved',
        'rejected',
        'cancelled'
    ],
    title: 'VMRequestStatus'
} as const;

export const VMRequestWindowAvailabilityRequestSchema = {
    properties: {
        resource_type: {
            type: 'string',
            enum: [
                'lxc',
                'vm'
            ],
            title: 'Resource Type',
            default: 'lxc'
        },
        cores: {
            type: 'integer',
            maximum: 256,
            minimum: 1,
            title: 'Cores',
            default: 2
        },
        memory: {
            type: 'integer',
            maximum: 1048576,
            minimum: 128,
            title: 'Memory',
            description: 'MB',
            default: 2048
        },
        disk_size: {
            anyOf: [
                {
                    type: 'integer',
                    maximum: 65536,
                    minimum: 1
                },
                {
                    type: 'null'
                }
            ],
            title: 'Disk Size'
        },
        rootfs_size: {
            anyOf: [
                {
                    type: 'integer',
                    maximum: 65536,
                    minimum: 1
                },
                {
                    type: 'null'
                }
            ],
            title: 'Rootfs Size'
        },
        gpu_required: {
            type: 'integer',
            maximum: 16,
            minimum: 0,
            title: 'Gpu Required',
            default: 0
        },
        start_at: {
            type: 'string',
            format: 'date-time',
            title: 'Start At'
        },
        end_at: {
            type: 'string',
            format: 'date-time',
            title: 'End At'
        },
        mode: {
            type: 'string',
            enum: [
                'quick_template',
                'research',
                'scheduled'
            ],
            title: 'Mode',
            default: 'research'
        }
    },
    type: 'object',
    required: [
        'start_at',
        'end_at'
    ],
    title: 'VMRequestWindowAvailabilityRequest'
} as const;

export const VMRequestWindowAvailabilityResponseSchema = {
    properties: {
        status: {
            type: 'string',
            enum: [
                'available',
                'limited',
                'unavailable'
            ],
            title: 'Status'
        },
        feasible: {
            type: 'boolean',
            title: 'Feasible'
        },
        start_at: {
            type: 'string',
            format: 'date-time',
            title: 'Start At'
        },
        end_at: {
            type: 'string',
            format: 'date-time',
            title: 'End At'
        },
        duration_hours: {
            type: 'integer',
            minimum: 0,
            title: 'Duration Hours'
        },
        duration_days: {
            type: 'number',
            minimum: 0,
            title: 'Duration Days'
        },
        summary: {
            type: 'string',
            title: 'Summary'
        },
        reason: {
            type: 'string',
            title: 'Reason'
        },
        selected_node: {
            anyOf: [
                {
                    type: 'string'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Selected Node'
        },
        placement_strategy: {
            anyOf: [
                {
                    type: 'string'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Placement Strategy'
        },
        checked_checkpoint_count: {
            type: 'integer',
            minimum: 0,
            title: 'Checked Checkpoint Count',
            default: 0
        },
        warnings: {
            items: {
                type: 'string'
            },
            type: 'array',
            title: 'Warnings'
        }
    },
    type: 'object',
    required: [
        'status',
        'feasible',
        'start_at',
        'end_at',
        'duration_hours',
        'duration_days',
        'summary',
        'reason'
    ],
    title: 'VMRequestWindowAvailabilityResponse'
} as const;

export const VMRequestsPublicSchema = {
    properties: {
        data: {
            items: {
                $ref: '#/components/schemas/VMRequestPublic'
            },
            type: 'array',
            title: 'Data'
        },
        count: {
            type: 'integer',
            title: 'Count'
        }
    },
    type: 'object',
    required: [
        'data',
        'count'
    ],
    title: 'VMRequestsPublic'
} as const;

export const VMTemplateSchemaSchema = {
    properties: {
        vmid: {
            type: 'integer',
            title: 'Vmid'
        },
        name: {
            type: 'string',
            title: 'Name'
        },
        node: {
            type: 'string',
            title: 'Node'
        }
    },
    type: 'object',
    required: [
        'vmid',
        'name',
        'node'
    ],
    title: 'VMTemplateSchema',
    description: 'VM template 資訊'
} as const;

export const VNCInfoSchemaSchema = {
    properties: {
        vmid: {
            type: 'integer',
            title: 'Vmid'
        },
        ws_url: {
            type: 'string',
            title: 'Ws Url'
        },
        ticket: {
            anyOf: [
                {
                    type: 'string'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Ticket'
        },
        port: {
            anyOf: [
                {
                    type: 'string'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Port'
        },
        message: {
            type: 'string',
            title: 'Message'
        }
    },
    type: 'object',
    required: [
        'vmid',
        'ws_url',
        'message'
    ],
    title: 'VNCInfoSchema',
    description: 'VNC 連線資訊'
} as const;

export const ValidationErrorSchema = {
    properties: {
        loc: {
            items: {
                anyOf: [
                    {
                        type: 'string'
                    },
                    {
                        type: 'integer'
                    }
                ]
            },
            type: 'array',
            title: 'Location'
        },
        msg: {
            type: 'string',
            title: 'Message'
        },
        type: {
            type: 'string',
            title: 'Error Type'
        },
        input: {
            title: 'Input'
        },
        ctx: {
            type: 'object',
            title: 'Context'
        }
    },
    type: 'object',
    required: [
        'loc',
        'msg',
        'type'
    ],
    title: 'ValidationError'
} as const;

export const app__ai__pve_log__schemas__ChatRequestSchema = {
    properties: {
        message: {
            anyOf: [
                {
                    type: 'string',
                    maxLength: 2000
                },
                {
                    type: 'null'
                }
            ],
            title: 'Message',
            description: '使用者輸入的自然語言問題'
        },
        group_id: {
            type: 'string',
            format: 'uuid',
            title: 'Group Id',
            description: '目前所在群組 ID'
        },
        messages: {
            anyOf: [
                {
                    items: {
                        additionalProperties: true,
                        type: 'object'
                    },
                    type: 'array'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Messages',
            description: '完整的對話歷史（用於中斷與接續對話）'
        }
    },
    type: 'object',
    required: [
        'group_id'
    ],
    title: 'ChatRequest'
} as const;

export const app__ai__pve_log__schemas__ChatResponseSchema = {
    properties: {
        reply: {
            type: 'string',
            title: 'Reply'
        },
        tools_called: {
            items: {
                $ref: '#/components/schemas/ToolCallRecord'
            },
            type: 'array',
            title: 'Tools Called'
        },
        needs_confirmation: {
            type: 'boolean',
            title: 'Needs Confirmation',
            default: false
        },
        messages: {
            items: {
                additionalProperties: true,
                type: 'object'
            },
            type: 'array',
            title: 'Messages'
        },
        error: {
            anyOf: [
                {
                    type: 'string'
                },
                {
                    type: 'null'
                }
            ],
            title: 'Error'
        }
    },
    type: 'object',
    required: [
        'reply'
    ],
    title: 'ChatResponse'
} as const;

export const app__ai__template_recommendation__schemas__ChatMessageSchema = {
    properties: {
        role: {
            type: 'string',
            title: 'Role',
            description: 'Role of the message sender, usually \'user\' or \'assistant\'.'
        },
        content: {
            type: 'string',
            title: 'Content',
            description: 'Content of the message.'
        }
    },
    type: 'object',
    required: [
        'role',
        'content'
    ],
    title: 'ChatMessage'
} as const;

export const app__ai__template_recommendation__schemas__ChatRequestSchema = {
    properties: {
        messages: {
            items: {
                $ref: '#/components/schemas/app__ai__template_recommendation__schemas__ChatMessage'
            },
            type: 'array',
            minItems: 1,
            title: 'Messages',
            description: 'List of previous chat messages.'
        },
        top_k: {
            type: 'integer',
            maximum: 10,
            minimum: 1,
            title: 'Top K',
            default: 5
        },
        device_nodes: {
            items: {
                $ref: '#/components/schemas/DeviceNode'
            },
            type: 'array',
            title: 'Device Nodes'
        },
        form_context: {
            anyOf: [
                {
                    $ref: '#/components/schemas/RecommendationFormContext'
                },
                {
                    type: 'null'
                }
            ]
        }
    },
    type: 'object',
    required: [
        'messages'
    ],
    title: 'ChatRequest'
} as const;

export const app__ai__template_recommendation__schemas__ChatResponseSchema = {
    properties: {
        reply: {
            type: 'string',
            title: 'Reply',
            description: 'AI text reply.'
        },
        prompt_tokens: {
            type: 'integer',
            title: 'Prompt Tokens',
            default: 0
        },
        completion_tokens: {
            type: 'integer',
            title: 'Completion Tokens',
            default: 0
        },
        total_tokens: {
            type: 'integer',
            title: 'Total Tokens',
            default: 0
        },
        elapsed_seconds: {
            type: 'number',
            title: 'Elapsed Seconds',
            default: 0
        },
        tokens_per_second: {
            type: 'number',
            title: 'Tokens Per Second',
            default: 0
        }
    },
    type: 'object',
    required: [
        'reply'
    ],
    title: 'ChatResponse'
} as const;

export const app__schemas__ai_proxy__ChatMessageSchema = {
    properties: {
        role: {
            type: 'string',
            title: 'Role',
            description: '消息角色: system, user, assistant'
        },
        content: {
            type: 'string',
            title: 'Content',
            description: '消息内容'
        }
    },
    type: 'object',
    required: [
        'role',
        'content'
    ],
    title: 'ChatMessage',
    description: '聊天消息'
} as const;
