...
const MessageBubble = ({ type, text, image, document, video, isStreaming }) => {
  ...
  const parseThinkingContent = (content) => {
    ...
    // 如果完全沒有 thinking 標籤，就當作普通輸出
    if (startThinkIndex === -1 && endThinkIndex === -1) {
      return [{ type: 'output', content }]
    }
    ...
    if (endThinkIndex !== -1) {
      ...
      // 清理思考部分的開始標籤（如果有的話）
      const actualStartThinkIndex = thinkingPart.indexOf('<think>')
      if (actualStartThinkIndex !== -1) {
        thinkingPart = thinkingPart.slice(actualStartThinkIndex + '<think>'.length)
      }

      const parts = []
      // 就算有 <think>，可能前面也有一些文字
      if (actualStartThinkIndex > 0) {
        parts.push({ type: 'output', content: content.slice(0, actualStartThinkIndex) })
      }
    ...
    const parts = []
    if (actualStartThinkIndex > 0) {
      parts.push({ type: 'output', content: thinkingPart.slice(0, actualStartThinkIndex) })
    }
